...
/datum/controller/subsystem/cargo/proc/add_junk()
	...
	if(clear_turfs.len)
		...
		var/obj/structure/closet/crate/cargo = new(pick_n_take(clear_turfs))
		for(var/datum/cargo_item/item in shuffle(SScargo.cargo_items))
			if(item.container_type == "box")
				continue
			if(limit <= 0)
				break
			new item.path(cargo)
	...
	if(clear_turfs.len)
		new /obj/structure/closet/crate/maint_loot(pick_n_take(clear_turfs))
	...
	if(clear_turfs.len)
		new /obj/structure/closet/crate/secure/special(pick_n_take(clear_turfs))
