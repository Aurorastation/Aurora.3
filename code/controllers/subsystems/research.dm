	...
	while(loadconcepts.NextRow())
		for(var/datum/research_concepts/A in concepts)
			if(A.id == loadconcepts.item[1])
				A.level = loadconcepts.item[2]
				A.progress = loadconcepts.item[3]
	//load unlocked items
	var/DBQuery/loaditems = dbcon.NewQuery("SELECT id, level, progress FROM ss13_research_items WHERE deleted_at IS NULL ORDER BY order_by")
	init_subtypes(/datum/research_items, techitems)
	loaditems.Execute()
	while(loaditems.NextRow())
		for(var/datum/research_items/A in techitems)
			if(A.id == loaditems.item[1])
				A.unlocked = loaditems.item[2]
	sortitems()
