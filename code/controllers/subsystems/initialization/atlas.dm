...
/datum/controller/subsystem/atlas/Initialize(timeofday)
...
#endif
	if (!map_override)
		map_override = "event/nikal_sahira"
	...
	var/maps_loaded
	if(istype(current_map, /datum/map/event/nikal_sahira))
		...
		// for CI tests so they don't load hardcoded map repeatedly
		maps_loaded = load_map_directory("maps/[current_map.path]/", TRUE)
	...
	// Sector load order: Database < Config file < Default map
	var/fallback = GLOB.config.current_space_sector ? GLOB.config.current_space_sector : current_map.default_sector
	var/chosen_sector = SSregistry.getValue(REGISTRY_CURRENT_SECTOR, fallback)
