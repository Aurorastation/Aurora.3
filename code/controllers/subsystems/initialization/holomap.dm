...
/datum/controller/subsystem/holomap/proc/generate_minimap(zlevel = 1)
	...
	for(var/turf/T as anything in Z_TURFS(zlevel))
		...
		if(A.area_flags & AREA_FLAG_HIDE_FROM_HOLOMAP)
			...
			if(!istype(A, /area/horizon/maintenance) && !istype(A, /area/horizon/gunnery) && !istype(A, /area/horizon/ai) && !istype(A, /area/horizon/command/bridge/aibunker) && !istype(A, /area/horizon/command/bridge/selfdestruct) && !istype(A, /area/port_volturno/unsimulated))
