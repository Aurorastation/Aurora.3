...
/datum/controller/subsystem/finalize/Initialize(timeofday)
	...
	log_subsystem("map_finalization", "Finalized map in [(world.time - time)/10] seconds.")
	...
	if(config.generate_asteroid)
		...
		log_subsystem("map_finalization", "Generated asteroid in [(world.time - time)/10] seconds.")
...
/datum/controller/subsystem/finalize/proc/select_ruin()
	...
	for(var/folder in subfolders)
		...
		if(!fexists("[folder]config.json"))
			log_subsystem("map_finalization", "Found no cofiguration file in [folder] - Skipping")
			continue

		try
			mission_config = json_decode(file2text("[folder]config.json"))
		catch(var/exception/ed)
			log_subsystem("map_finalization", "Invalid config.json in [folder]: [ed]")
			continue

		try
			am = new(mission_config, folder)
		catch(var/exception/ec)
			log_subsystem("map_finalization", "Error while creating away mission datum: [ec]")
			continue

		if(length(am.valid_sectors))
			if(!(SSatlas.current_sector.name in am.valid_sectors))
				log_subsystem("map_finalization", "[SSatlas.current_sector.name] is not a valid map for [am.name]")
				continue
	...
	if(!length(mission_list))
		log_subsystem("map_finalization", "Found no valid ruins for the current map.")
		return

	log_subsystem("map_finalization", "Loaded ruin config.")
	...
	if(SSpersist_config.forced_awaymission)
		if(SSpersist_config.forced_awaymission in mission_list)
			...
			log_subsystem("map_finalization", "Selected enforced away mission.")
			admin_notice(SPAN_DANGER("Selected enforced away mission."), R_DEBUG)
			return
		else
			log_subsystem("map_finalization", "Failed to selected enforced away mission. Fallback to weighted selection.")
...
/datum/controller/subsystem/finalize/proc/load_space_ruin()
	...
	if(!selected_mission)
		log_subsystem("map_finalization", "Not loading away mission, because no mission has been selected.")
	...
	for(var/map in selected_mission.map_files)
		...
		if (!maploader.load_map(file(mfile), 0, 0, no_changeturf = TRUE))
			log_subsystem("map_finalization", "Failed to load '[mfile]'!")
			admin_notice(SPAN_DANGER("Failed to load '[mfile]'!"), R_DEBUG)
		else
			log_subsystem("map_finalization", "Loaded away mission on z [world.maxz] in [(world.time - time)/10] seconds.")
...
/datum/controller/subsystem/finalize/proc/place_dungeon_spawns()
	...
	log_subsystem("map_finalization","Attempting to create asteroid dungeons for [length(asteroid_spawn)] different areas, with [length(files) - 1] possible dungeons, with a [dungeon_chance]% chance to spawn a dungeon per area.")

	for(var/turf/spawn_location in asteroid_spawn)

		if(length(files) <= 0) //Sanity
			log_subsystem("map_finalization","There aren't enough dungeon map files to fill the entire dungeon map. There may be less dungeons than expected.")
			break
		...
		if(prob(dungeon_chance))
			...
			if(!dd_hassuffix(chosen_dungeon,".dmm")) //Don't read anything that isn't a map file
				...
				log_subsystem("map_finalization","ALERT: [chosen_dungeon] is not a .dmm file! Skipping!")
				continue

			var/map_file = file("[map_directory][chosen_dungeon]")

			if(isfile(map_file)) //Sanity
				log_subsystem("map_finalization","Loading dungeon '[chosen_dungeon]' at coordinates [spawn_location.x], [spawn_location.y], [spawn_location.z].")
				maploader.load_map(map_file,spawn_location.x,spawn_location.y,spawn_location.z)
				dungeons_placed += 1
			else
				log_subsystem("map_finalization","ERROR: Something weird happened with the file: [chosen_dungeon].")

			if(dd_hassuffix(chosen_dungeon,"_unique.dmm")) //Unique dungeons should only spawn once.
				files -= chosen_dungeon

	log_subsystem("map_finalization","Loaded [dungeons_placed] asteroid dungeons in [(world.time - start_time)/10] seconds.")
