...
proc/place_dungeon_spawns_generic(var/map_directory, var/turf/spawn_location)
	//var/map_directory = "maps/dungeon_spawns/"
	var/list/files = flist(map_directory)
	var/start_time = world.time
	var/dungeons_placed = 0
	...
	// log_ss("map_finalization","Attempting to create asteroid dungeons for [length(spawn_landmarks)] different areas, \
	// 		with [length(files) - 1] possible dungeons, \
	// 		with a [dungeon_chance]% chance to spawn a dungeon per area.")


	if(length(files) <= 0) //Sanity
		log_ss("place_dungeon_spawns_generic","There aren't enough dungeon map files to fill the entire dungeon map. There may be less dungeons than expected.")
	...
	if(isfile(map_file)) //Sanity
		log_ss("place_dungeon_spawns_generic","Loading dungeon '[chosen_dungeon]' at coordinates [spawn_location.x], [spawn_location.y], [spawn_location.z].")
		maploader.load_map(map_file,spawn_location.x,spawn_location.y,spawn_location.z)
		dungeons_placed += 1
	else
		log_ss("place_dungeon_spawns_generic","ERROR: Something weird happened with the file: [chosen_dungeon].")

	if(dd_hassuffix(chosen_dungeon,"_unique.dmm")) //Unique dungeons should only spawn once.
		files -= chosen_dungeon

	log_ss("place_dungeon_spawns_generic","Loaded [dungeons_placed] asteroid dungeons in [(world.time - start_time)/10] seconds.")
