...
/datum/controller/subsystem/fail2topic/Initialize(timeofday)
	...
	if (world.system_type == UNIX && enabled)
		enabled = FALSE
		log_subsystem("fail2topic", "Subsystem disabled due to it not supporting UNIX.", log_world = FALSE, severity = SEVERITY_NOTICE)
...
/datum/controller/subsystem/fail2topic/proc/BanFromFirewall(ip)
	...
	if (.)
		log_subsystem("fail2topic", "Failed to ban [ip]. Exit code: [.].", log_world = FALSE, severity = SEVERITY_ERROR)
	else if (isnull(.))
		log_subsystem("fail2topic", "Failed to invoke ban script.", log_world = FALSE, severity = SEVERITY_ERROR)
	else
		log_subsystem("fail2topic", "Banned [ip].", log_world = FALSE, severity = SEVERITY_NOTICE)
...
/datum/controller/subsystem/fail2topic/proc/DropFirewallRule()
	...
	if (.)
		log_subsystem("fail2topic", "Failed to drop firewall rule. Exit code: [.].", log_world = FALSE, severity = SEVERITY_ERROR)
	else if (isnull(.))
		log_subsystem("fail2topic", "Failed to invoke ban script.", log_world = FALSE, severity = SEVERITY_ERROR)
	else
		log_subsystem("fail2topic", "Firewall rule dropped.", log_world = FALSE, severity = SEVERITY_INFO)
