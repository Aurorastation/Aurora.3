...
/datum/controller/subsystem/distress/Initialize(start_timeofday)
	...
	for(var/team in all_teams)
		...
		if(SSmapping.current_sector.name in ert.possible_space_sector)
			available_teams += ert
		all_ert_teams += ert
...
/datum/controller/subsystem/distress/proc/trigger_armed_response_team(var/forced_choice = null)
	...
	command_announcement.Announce("An emergency response team has picked up the distress signal. A specialized relief team will arrive shortly.", "[SSmapping.current_map.station_name] Distress Suite", 'sound/misc/announcements/security_level_old.ogg')
...
/datum/controller/subsystem/distress/proc/trigger_overmap_distress_beacon(var/obj/effect/overmap/visitable/requester, var/distress_message, var/mob/user)
	...
	command_announcement.Announce("A distress beacon has been broadcasted to nearby vessels in the sector. Please remain calm and make preparations for the arrival of third parties.", "[SSmapping.current_map.station_name] Distress Suite", 'sound/misc/announcements/security_level_old.ogg', zlevels = requester.map_z)
...
/client/proc/response_team()
	...
	if(SSdistress.send_emergency_team)
		to_chat(usr, SPAN_DANGER("[SSmapping.current_map.boss_name] has already dispatched an emergency response team!"))
		return
