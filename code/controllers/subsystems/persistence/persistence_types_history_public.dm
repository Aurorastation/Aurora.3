...
/**
 * Attempts to calculate a total sum for the specified type and attribute.
 * NOTE:
 *  If the type features non-number values, these will just be ignored.
 *  Ideally the return value can be cached to reduce load.
 * PARAMS:
 * 	target_type =	Singleton persistent type definition. See /singleton/persistent_type and subtypes.
 *  attribute =		Custom attribute of the record, can be null if the type definition doesn't require it.
 * RETURN:
 * 	Number, defaults to zero if no records or only non-number records are available.
 */
/datum/controller/subsystem/persistence/proc/historyGetTotalSum(var/singleton/persistent_type/history/target_type, attribute)
	if(!target_type)
		log_subsystem_persistence_warning("Attempted to get history records total sum with null target type.")
		return list()

	if(target_type.requires_attribute && !attribute)
		log_subsystem_persistence_warning("Attempted to get history records total sum of type [target_type] without required attribute.")
		return list()

	// Because the amount of records for the specified type is unkown or possibly a large count, cache is bypassed.
	// Retrieve all values from database, include cache values that are new, aggregate.
	...
	// Because the history table allows generic values to be saved, we need to verify each value is in fact a number, otherwise discard value.
	var/sum = 0
