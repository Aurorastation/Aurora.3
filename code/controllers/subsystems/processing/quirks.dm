...
PROCESSING_SUBSYSTEM_DEF(quirks)
	...
	var/list/quirk_categories = list() //List of quirk categories for character setup
...
/datum/controller/subsystem/processing/quirks/proc/SetupQuirks()
	...
	for(var/type in quirk_list)
		...
		var/quirk_category = quirk_type.category
		if(!(quirk_category in quirk_categories))
			quirk_categories += quirk_category
...
/datum/controller/subsystem/processing/quirks/proc/randomise_quirks(mob/living/user)
	...
	var/max_positive_quirks = 3 //CONFIG_GET(number/max_positive_quirks)
...
/datum/controller/subsystem/processing/quirks/proc/filter_invalid_quirks(list/quirks)
	...
	var/points_enabled = TRUE //!CONFIG_GET(flag/disable_quirk_points)
	var/max_positive_quirks = 3 //CONFIG_GET(number/max_positive_quirks)
	var/balance = 0 //-CONFIG_GET(number/default_quirk_points)
...
//			continue
		...
		if (value > 0)
			if (max_positive_quirks >= 0 && length(quirks) == max_positive_quirks)
				continue
	...
	if (length(new_quirks) == length(quirks))
		return quirks
