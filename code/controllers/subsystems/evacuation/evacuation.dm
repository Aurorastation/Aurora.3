		...
		if(TRANSFER_EMERGENCY)
			for(var/area/A in get_sorted_areas())
				if(istype(A, /area/horizon/hallway))
					A.readyalert()
			if(!skip_announce)
				priority_announcement.Announce(replacetext(replacetext(SSmapping.current_map.emergency_shuttle_called_message, "%dock%", "[SSmapping.current_map.dock_name]"),  "%ETA%", "[round(get_eta()/60)] minute\s"), new_sound = 'sound/AI/emergency_shuttle_called_message.ogg')
		if(TRANSFER_CREW)
			if(!skip_announce)
				priority_announcement.Announce(replacetext(replacetext(SSmapping.current_map.shuttle_called_message, "%dock%", "[SSmapping.current_map.dock_name]"),  "%ETA%", "[round(get_eta()/60)] minute\s"), new_sound = 'sound/AI/shuttle_called_message.ogg')
		if(TRANSFER_JUMP)
			if(!skip_announce)
				priority_announcement.Announce(replacetext(replacetext(SSmapping.current_map.bluespace_called_message, "%dock%", "[SSmapping.current_map.dock_name]"),  "%ETA%", "[round(get_eta()/60)] minute\s"), new_sound = 'sound/AI/bluespace_jump_called.ogg')
	return TRUE
...
/datum/evacuation_controller/proc/cancel_evacuation()
	...
	switch(evacuation_type)
		if(TRANSFER_EMERGENCY)
			evac_recalled.Announce(SSmapping.current_map.emergency_shuttle_recall_message, new_sound = 'sound/AI/emergency_shuttle_recall_message.ogg')
			for(var/area/A in get_sorted_areas())
				if(istype(A, /area/horizon/hallway))
					A.readyreset()
		if(TRANSFER_JUMP)
			priority_announcement.Announce(SSmapping.current_map.bluespace_recall_message, new_sound = 'sound/AI/bluespace_jump_recalled.ogg')
		if(TRANSFER_CREW)
			priority_announcement.Announce(SSmapping.current_map.shuttle_recall_message, new_sound = 'sound/AI/shuttle_recall_message.ogg')
...
/datum/evacuation_controller/proc/finish_preparing_evac()
	...
	switch(evacuation_type)
		if(TRANSFER_EMERGENCY)
			evac_waiting.Announce(replacetext(SSmapping.current_map.emergency_shuttle_docked_message, "%ETA%", "[estimated_time] minute\s"), new_sound = sound('sound/AI/emergency_shuttle_docked.ogg'))
		if(TRANSFER_JUMP)
			priority_announcement.Announce(replacetext(replacetext(SSmapping.current_map.bluespace_docked_message, "%dock%", "[SSmapping.current_map.dock_name]"), "%ETA%", "[estimated_time] minute\s"), new_sound = sound('sound/AI/bluespace_jump_docked.ogg'))
		if(TRANSFER_CREW)
			priority_announcement.Announce(replacetext(replacetext(SSmapping.current_map.shuttle_docked_message, "%dock%", "[SSmapping.current_map.dock_name]"), "%ETA%", "[estimated_time] minute\s"), new_sound = sound('sound/AI/shuttle_docked_message.ogg'))
...
/datum/evacuation_controller/proc/launch_evacuation()
	...
	switch(evacuation_type)
		if(TRANSFER_EMERGENCY)
			priority_announcement.Announce(replacetext(replacetext(SSmapping.current_map.emergency_shuttle_leaving_dock, "%dock%", "[SSmapping.current_map.dock_name]"),  "%ETA%", "[round(get_eta()/60,1)] minute\s"), new_sound = sound('sound/AI/emergency_shuttle_leaving_dock.ogg'))
		if(TRANSFER_JUMP)
			priority_announcement.Announce(replacetext(replacetext(SSmapping.current_map.bluespace_leaving_dock, "%dock%", "[SSmapping.current_map.dock_name]"),  "%ETA%", "[round(get_eta()/60,1)] minute\s"), new_sound = sound('sound/AI/bluespace_jump_leaving.ogg'))
		if(TRANSFER_CREW)
			priority_announcement.Announce(replacetext(replacetext(SSmapping.current_map.shuttle_leaving_dock, "%dock%", "[SSmapping.current_map.dock_name]"),  "%ETA%", "[round(get_eta()/60,1)] minute\s"), new_sound = sound('sound/AI/shuttle_leaving_dock.ogg'))
