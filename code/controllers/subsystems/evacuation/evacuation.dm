...
/datum/evacuation_controller/proc/finish_preparing_evac()
	...
	if (emergency_evacuation)
		...
		priority_announcement.Announce(replacetext(replacetext(current_map.shuttle_docked_message, "%dock_name%", "[current_map.dock_name]"),  "%ETD%", "[estimated_time] minute\s"), new_sound = sound('sound/AI/shuttledock.ogg')
