	...
	if(emergency_evacuation)
		for(var/area/A in world)
			if(istype(A, /area/hallway))
				A.readyalert()
		if(!skip_announce)
			priority_announcement.Announce(replacetext(replacetext(current_map.emergency_shuttle_called_message, "%dock_name%", "[current_map.dock_name]"),  "%ETA%", "[round(get_eta()/60)] minute\s"))
	else
		if(!skip_announce)
			priority_announcement.Announce(replacetext(replacetext(current_map.shuttle_called_message, "%dock_name%", "[current_map.dock_name]"),  "%ETA%", "[round(get_eta()/60)] minute\s"))
...
/datum/evacuation_controller/proc/finish_preparing_evac()
	...
	if (emergency_evacuation)
		evac_waiting.Announce(replacetext(current_map.emergency_shuttle_docked_message, "%ETD%", "[estimated_time] minute\s"), new_sound = sound('sound/effects/Evacuation.ogg', volume = 35))
	else
		priority_announcement.Announce(replacetext(replacetext(current_map.shuttle_docked_message, "%dock_name%", "[current_map.dock_name]"),  "%ETD%", "[estimated_time] minute\s"))
