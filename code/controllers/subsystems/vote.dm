...
/datum/controller/subsystem/vote/fire(resumed = FALSE)
	if (mode)
		...
		// plus vote duration
		time_remaining = round((started_time + config.vote_period - round_duration())/10)
	...
	if (round_duration() >= next_transfer_time - 600)
		autotransfer()
...
/datum/controller/subsystem/vote/proc/get_result()
	...
	if(!config.vote_no_default && choices.len)
		...
		if(non_voters > 0)
			if(mode == "restart")
				...
				var/factor = 0.5
				switch(round_duration() / (10 * 60)) // minutes
					if(0 to 60)
						factor = 0.5
					if(61 to 120)
						factor = 0.8
	...
	if(mode == "crew_transfer")
		if(round(round_duration() / 36000)+12 <= 14)
...
/datum/controller/subsystem/vote/proc/initiate_vote(vote_type, initiator_key, automatic = FALSE)
	if(!mode)
		if(started_time != null && !(check_rights(R_ADMIN|R_MOD) || automatic))
			...
			if(next_allowed_time > round_duration())
				return 0
