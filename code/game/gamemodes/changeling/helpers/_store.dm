...
/datum/changeling/proc/purchasePower(var/datum/mind/M, var/power_name, var/remake_verbs = 1)
	...
	if(absorbed_dna < power.required_dna)
		to_chat(M.current, SPAN_WARNING("Our DNA structure is not complex enough to evolve this. We need at least [power.required_dna] more assimilated DNA structures to buy it."))
		return
