...
/datum/universal_state/bluespace_jump/OnEnter()
	var/space_zlevel = SSmapping.current_map.get_empty_zlevel() //get a place for stragglers
	for(var/mob/living/M in GLOB.mob_list)
		if(M.z in affected_levels)
			...
			if(istype(A,/area/space)) //straggler
				...
				apply_bluespaced(M)
	old_accessible_z_levels = SSmapping.current_map.accessible_z_levels.Copy()
	for(var/z in affected_levels)
		SSmapping.current_map.accessible_z_levels -= "[z]" //not accessible during the jump
...
/datum/universal_state/bluespace_jump/OnExit()
	...
	bluespaced.Cut()
	SSmapping.current_map.accessible_z_levels = old_accessible_z_levels
