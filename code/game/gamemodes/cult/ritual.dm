...
//		usr << browse(null, "window=tank")
	...
	attack(mob/living/M as mob, mob/living/user as mob)
		...
		msg_admin_attack(text="[user.name] ([user.ckey]) used [name] on [M.name] ([M.ckey]) (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[user.x];Y=[user.y];Z=[user.z]'>JMP</a>)",ckey=key_name(user),ckey_target=key_name(M))
