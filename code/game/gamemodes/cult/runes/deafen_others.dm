	...
	if(istype(O, /obj/effect/rune))
		...
		for(var/mob/living/carbon/C in range(7, get_turf(O)))
			...
			if(N)
				continue
			C.ear_deaf += 50
			to_chat(C, SPAN_WARNING("The world around you suddenly becomes quiet."))
			affected += C
			if(prob(1))
				C.sdisabilities |= DEAF
		if(affected.len)
			user.say("Sti[pick("'","`")] kaliedir!")
			to_chat(user, SPAN_CULT("The world becomes quiet as the deafening rune dissipates into fine dust."))
		...
		if(affected.len)
			...
			for(var/mob/V in orange(1, get_turf(O)))
				if(!iscultist(V))
					to_chat(V, SPAN_WARNING("Dust flows from [user]'s hands for a moment, and the world suddenly becomes quiet.."))
	return TRUE