...
/obj/item/book/tome/proc/attack_admins(var/mob/living/target_mob, var/mob/living/user)
	target_mob.log_message("Has had the [name] used on them by [user.name] ([user.ckey])", LOG_VICTIM, log_globally = FALSE)
	user.log_message("Used [name] on [target_mob.name] ([target_mob.ckey])", LOG_ATTACK, log_globally = FALSE)
