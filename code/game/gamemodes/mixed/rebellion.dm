/datum/game_mode/rebellion
	name = "rebellion (rev+cultist)"
	round_description = "Some crewmembers are attempting to start a revolution while a cult gains followers to take over the [current_map.station_type] in the shadows!"
	extended_round_description = "Revolutionaries and cultists spawn during this round."
	config_tag = "rebellion"
	required_players = 30
	required_enemies = 6
	antag_tags = list(MODE_REVOLUTIONARY, MODE_LOYALIST, MODE_CULTIST)
	require_all_templates = TRUE
