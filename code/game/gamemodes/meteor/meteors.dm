	...
	if (!done)
		spawn(0)
			...
			if (istype(A, /obj/effect/energy_field))//If a normal/small meteor impacts an energy field, then it makes a widely audible impact sound and qdels
				...
				msg_admin_attack(text="Meteor impacted energy field at coords (<a href='?_src_=holder;adminplayerobservecoodjump=1;X=[x];Y=[y];Z=[z]'>JMP</a>)")
			...
			if (--src.hits == 0 && !done)
				...
				if(!istype(A,/obj/machinery/power/emitter) && \
					...
					msg_admin_attack(text="Meteor exploded at coords (<a href='?_src_=holder;adminplayerobservecoodjump=1;X=[x];Y=[y];Z=[z]'>JMP</a>)")
				else
					msg_admin_attack(text="Meteor dissipated without exploding at coords (<a href='?_src_=holder;adminplayerobservecoodjump=1;X=[x];Y=[y];Z=[z]'>JMP</a>)")
				spawn()
					qdel(src)
			...
			spawn(0)
				...
				if (istype(A, /obj/effect/energy_field))//If a big meteor impacts an energy field, then it detonates immediately with reduced power
					...
					if (T)
						meteor_shield_impact_sound(T, shieldsoundrange)
					explosion(loc, power, power + power_step, power + power_step * 2, power + power_step * 3, 0)
					msg_admin_attack(text="Large Meteor impacted energy field and then exploded at coords (<a href='?_src_=holder;adminplayerobservecoodjump=1;X=[x];Y=[y];Z=[z]'>JMP</a>)")
				...
				if (--src.hits == 0 && !done)
					...
					if(prob(detonation_chance) && !istype(A, /obj/structure/grille))
						explosion(loc, power, power + power_step, power + power_step * 2, power + power_step * 3, 0)
						msg_admin_attack(text="Large Meteor exploded at coords (<a href='?_src_=holder;adminplayerobservecoodjump=1;X=[x];Y=[y];Z=[z]'>JMP</a>)")
					else
						msg_admin_attack(text="Large Meteor dissipated without a final explosion at coords (<a href='?_src_=holder;adminplayerobservecoodjump=1;X=[x];Y=[y];Z=[z]'>JMP</a>)")
					spawn()
						qdel(src)
