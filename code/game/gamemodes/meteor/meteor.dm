...
/datum/game_mode/meteor/declare_completion()
	...
	for(var/mob/living/player in player_list)
		if(player.stat != DEAD)
			...
			if(!location)
				continue
			if(istype(location.loc.type, /area/shuttle/escape))
				text += "<br><b><font size=2>[player.real_name] escaped on the emergency shuttle</font></b>"
			if(istype(location.loc.type, /area/shuttle/escape_pod))
	...
	if(survivors)
		to_world(SPAN_NOTICE("<B>The following survived the meteor storm</B>") + ":[text]")
	else
