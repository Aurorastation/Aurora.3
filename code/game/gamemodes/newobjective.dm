	...
	objective
		...
		protection
			New(var/text,var/joba,var/datum/mind/targeta)
				...
				job = joba
				weight = get_points(job)
				explanation_text = "[target.current.real_name], the [target.assigned_role] is a [pick("relative of a","friend of a","") + pick("high ranking","important","well-liked")] mercenary [pick("Leader","Officer","Agent","sympathiser")].  Make sure they get off the facility safely, while minimizing intervention."
		...
		steal
			...
			*/
			...
			RCD
				steal_target = /obj/item/rfd/construction
			...
			blueprints
				...
				explanation_text = "Steal the facility's blueprints."
				weight = 20
			...
			nuke_disk
				...
				explanation_text = "Steal the facility's nuclear authentication disk."
				weight = 20
