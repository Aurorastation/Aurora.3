...
/mob/living/carbon/human/proc/vampire_dominate()
	...
	if (!command)
		to_chat(src, SPAN_ALERT("Cancelled."))
		return
...
/mob/living/carbon/human/proc/vampire_enthrall()
	...
	if (!istype(G))
		...
		return
	if (G.state == GRAB_PASSIVE || G.state == GRAB_UPGRADING)
		to_chat(src, SPAN_WARNING("You must have the victim pinned to the ground to enthrall them."))
		return
...
/mob/living/carbon/human/proc/vampire_diseasedtouch()
	...
	to_chat(src, SPAN_NOTICE("You infect [T] with a deadly disease. They will soon fade away."))
...
/mob/living/carbon/human/proc/vampire_embrace()
	...
	while (do_mob(src, T, 50))
		if (!mind.vampire)
			to_chat(src, SPAN_ALERT("Your fangs have disappeared!"))
			return
		if (!T.vessel.get_reagent_amount("blood"))
