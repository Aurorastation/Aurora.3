...
/datum/game_mode/malfunction/verb/advanced_encryption_hack()
	...
	switch(alert("Should this be announced to the general population?",,"Yes","No"))
		...
		if("No")
			...
			//	return
			log_ability_use(user, "advanced encryption hack (SUCCESS - title: [reporttitle])")
			to_world(SPAN_ALERT("New [SSmapping.current_map.company_name] Update available at all communication consoles."))
...
/datum/game_mode/malfunction/verb/system_override()
	...
	if(duration > 1500)								// Two types of announcements. Short hacks trigger immediate warnings. Long hacks are more "progressive".
		spawn(0)
			...
			if(!user || user.stat == DEAD)
				return
			command_announcement.Announce("Notice: Trace Update. Abnormal network activity originating from: Network terminal aboard [SSmapping.current_map.station_name]. External network connections disabled. Trace cancelled.", "Network Monitoring")
			sleep(duration/5)
			if(!user || user.stat == DEAD)
				return
