	...
	for(var/mob/living/carbon/human/H in living_mob_list)
		...
		if (prob(25))
			if (prob(75))
				...
				domutcheck(H,null,MUTCHK_FORCED)
	sleep(100)
	command_announcement.Announce("High levels of radiation detected near the ship. Please report to the Med-bay if you feel strange.", "Anomaly Alert")
...
/proc/prison_break() // -- Callagan
	...
	if(areas && areas.len > 0)
		...
		sleep(150)
		command_announcement.Announce("Gr3y.T1d3 virus detected in [station_name()] imprisonment subroutines. Recommend Ship AI involvement.", "Security Alert")
	else
