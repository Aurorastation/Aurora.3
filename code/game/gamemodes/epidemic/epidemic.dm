...
/datum/game_mode/epidemic/send_intercept()
	...
	intercepttext += "<B>* A cure is to be researched immediately, but NanoTarsen intellectual property must be respected. To prevent knowledge of [virus_name] from falling into unauthorized hands, all medical staff that work with the pathogen must be enhanced with a NanoTarsen loyality implant.</B><BR>"
