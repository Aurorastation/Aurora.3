...
/datum/antagonist/ninja/proc/generate_ninja_directive(side)
	var/directive = "[side=="face"?"[SSmapping.current_map.company_name]":"A criminal syndicate"] is your employer. "//Let them know which side they're on.
	switch(rand(1,19))
		if(1)
			directive += "The Spider Clan must not be linked to this operation. Remain hidden and covert when possible."
		if(2)
			directive += "[SSmapping.current_map.station_name] is financed by an enemy of the Spider Clan. Cause as much structural damage as desired."
		if(3)
		...
		if(5)
			directive += "We are currently negotiating with [SSmapping.current_map.company_name] [SSmapping.current_map.boss_name]. Prioritize saving human lives over ending them."
		if(6)
			directive += "We are engaged in a legal dispute over [SSmapping.current_map.station_name]. If a lawyer is present on board, force their cooperation in the matter."
		if(7)
		...
		if(9)
			directive += "A free agent has proposed a lucrative business deal. Implicate [SSmapping.current_map.company_name] involvement in the operation."
		if(10)
		...
		if(13)
			directive += "Some disgruntled [SSmapping.current_map.company_name] employees have been supportive of our operations. Be wary of any mistreatment by command staff."
		if(14)
		...
		if(16)
			directive += "The Spider Clan has been bargaining with a competing prosthetics manufacturer. Try to shine [SSmapping.current_map.company_name] prosthetics in a bad light."
		if(17)
