...
/obj/machinery/newscaster
	name = "newscaster"
	desc = "A standard newsfeed handler for use on commercial space vessels. All the news you absolutely have no use for, in one place!"
...
/obj/machinery/newscaster/attack_hand(mob/user as mob)            //########### THE MAIN BEEF IS HERE! And in the proc below this...############
	...
	if(istype(user, /mob/living/carbon/human) || istype(user,/mob/living/silicon) )
		...
		switch(screen)
			...
			if(1)
				dat+= "[current_map.station_type] Feed Channels<HR>"
			...
			if(9)
				...
				if(src.viewing_channel.censored)
					dat+="<span class='warning'><B>ATTENTION:</B></span> This channel has been deemed as threatening to the welfare of the [current_map.station_type], and marked with a [current_map.company_name] D-Notice.<BR>"
			...
			if(11)
				...
				dat+="<FONT SIZE=1>A D-Notice is to be bestowed upon the channel if the handling Authority deems it as harmful for the [current_map.station_type]'s"
			...
			if(13)
				dat+="<B>[src.viewing_channel.channel_name]: </B><FONT SIZE=1>\[ created by: <span class='boldannounce'>[src.viewing_channel.author]</span> \]</font><BR>"
				dat+="Channel messages listed below. If you deem them dangerous to the [current_map.station_type], you can <A href='?src=\ref[src];toggle_d_notice=\ref[src.viewing_channel]'>Bestow a D-Notice upon the channel</A>.<HR>"
				if(src.viewing_channel.censored)
					dat+="<span class='warning'><B>ATTENTION:</B></span> This channel has been deemed as threatening to the welfare of the [current_map.station_type], and marked with a [current_map.company_name] D-Notice.<BR>"
					dat+="No further feed story additions are allowed while the D-Notice is in effect.<BR><BR>"
				else
			...
			if(17)
				...
				dat+="<BR><A href='?src=\ref[src];setScreen=[0]'>Return</A><BR>"
			if(18)
				dat+="<B><span class='boldannounce'>-- SHIPWIDE WANTED ISSUE --</B></span><BR><FONT SIZE=2>\[Submitted by: <span class='good'>[SSnews.wanted_issue.backup_author]</span>\]</font><HR>"
...
/obj/item/newspaper
	name = "newspaper"
	desc = "An issue of The Griffon, the newspaper circulating aboard most facilities."
	...
	if(ishuman(user))
		...
		switch(screen)
			...
			if(1) // X channel pages inbetween.
				...
				if(C.censored)
					dat+="This channel was deemed dangerous to the general welfare of the [current_map.station_type] and therefore marked with a <B><span class='warning'>D-Notice</B></span>. Its contents were not transferred to the newspaper at the time of printing."
				else
