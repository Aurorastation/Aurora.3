...
/obj/machinery/optable/attack_hand(mob/user)
	...
	if(user != victim && !use_check_and_message(user)) // Skip checks if you're doing it to yourself or turning it off, this is an anti-griefing mechanic more than anything.
		...
		user.visible_message("<b>[user]</b> switches [suppressing ? "on" : "off"] \the [src]'s neural suppressor.")
...
/obj/machinery/optable/MouseDrop_T(mob/target, mob/user)
	...
	if(istype(M))
		...
		if(L == user)
			user.visible_message("<b>[user]</b> starts climbing onto \the [src].", SPAN_NOTICE("You start climbing onto \the [src]."), range = 3)
		else
			user.visible_message("<b>[user]</b> starts putting [L] onto \the [src].", SPAN_NOTICE("You start putting \the [L] onto \the [src]."), range = 3)
...
/obj/machinery/optable/attackby(obj/item/W, mob/living/carbon/user)
	if(istype(W, /obj/item/grab))
		...
		if(L == user)
			...
			user.visible_message("<b>[user]</b> starts putting \the [L] onto \the [src].", SPAN_NOTICE("You start putting \the [L] onto \the [src]."), range = 3)
		if(do_mob(user, L, 10, needhand = FALSE))
			if(bucklestatus == 2)
				var/obj/structure/LB = L.buckled
				LB.user_unbuckle_mob(user)
