...
/obj/machinery/alarm
	...
	var/list/TLV = list()
	var/list/trace_gas = list("sleeping_agent", "hydrogen") //list of other gases that this air alarm is able to detect
	...
	switch(screen)
		...
		if(AALARM_SCREEN_SCRUB)
			...
			for(var/id_tag in alarm_area.air_scrub_names)
				...
				scrubbers[scrubbers.len]["filters"] += list(list("name" = "Hydrogen", 		"command" = "h2_scrub","val"  = info["filter_hydrogen"]))
