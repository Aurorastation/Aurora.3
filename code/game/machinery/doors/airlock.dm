	...
	switch (href_list["command"])
		...
		if("bolts")
			if(src.isWireCut(AIRLOCK_WIRE_DOOR_BOLTS))
				...
				if(issilicon(usr) && density && !player_is_antag(usr.mind))
					to_chat(usr, SPAN_WARNING("Nanotrasen Safety Protocol #328 prevents you from lowering the door bolts of a closed airlock."))
				else if(lock())
