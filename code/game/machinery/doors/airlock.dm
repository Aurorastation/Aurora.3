	...
	switch (href_list["command"])
		...
		if("bolts")
			if(src.isWireCut(AIRLOCK_WIRE_DOOR_BOLTS))
				...
				if(issilicon(src) && !player_is_antag(src.mob.mind))
					to_chat(usr, span("notice", "The door bolts will drop in five seconds."))
					src.visible_message("\icon[src.icon] <b>[src]</b> announces, <span class='notice'>\"Bolts set to drop in FIVE SECONDS.\"</span>")
					addtimer(CALLBACK(src, .proc/lock), 50)
				else
					lock()
			else if(!activate)
				if(issilicon(src) && !player_is_antag(src.mob.mind))
					to_chat(usr, span("notice", "The door bolts will raise in five seconds."))
		...
		if("electrify_temporary")
			if(issilicon(src) && !player_is_antag(src.mob.mind))
				to_chat(usr, span("notice", "The door will electrify in five seconds."))
				src.visible_message("\icon[src.icon] <b>[src]</b> announces, <span class='notice'>\"Airlock set to electrify in FIVE SECONDS.\"</span>")
				addtimer(CALLBACK(src, .proc/electrify, 30 * activate, 1), 50)
			else
		...
		if("electrify_permanently")
			if(issilicon(src) && !player_is_antag(src.mob.mind))
				to_chat(usr, span("notice", "The door will electrify in five seconds."))
				src.visible_message("\icon[src.icon] <b>[src]</b> announces, <span class='notice'>\"Airlock set to electrify in FIVE SECONDS.\"</span>")
				addtimer(CALLBACK(src, .proc/electrify, -1 * activate, 1), 50)
			else
