...
/obj/machinery/door_timer/attackby(obj/item/O as obj, var/mob/user as mob)
	if( istype( O, /obj/item/paper/incident ))
		if( !incident )
			...
			to_chat(user,  SPAN_ALERT("<b>[src]</b> buzzes, \"There's already an active sentence!\""))
		return
	else if( istype( O, /obj/item/paper ))
		to_chat(user,  SPAN_ALERT("<b>[src]</b> buzzes, \"This console only accepts authentic incident reports. Copies are invalid.\""))
		return
...
/obj/machinery/door_timer/proc/import( var/obj/item/paper/incident/I, var/user )
	if( !istype( I ))
		to_chat(user,  SPAN_ALERT("<b>[src]</b> buzzes, \"Could not import the incident report.\""))
		return 0

	if( !istype( I.incident ))
		to_chat(user,  SPAN_ALERT("<b>[src]</b> buzzes, \"Report has no incident encoded!\""))
		return 0

	if( !I.sentence )
		to_chat(user,  SPAN_ALERT("<b>[src]</b> buzzes, \"Report does not contain a guilty sentence!\""))
		return 0

	var/datum/crime_incident/crime = I.incident

	if( !istype( crime.criminal ))
		to_chat(user,  SPAN_ALERT("<b>[src]</b> buzzes, \"Report has no criminal encoded!\""))
		return 0

	if( !crime.brig_sentence )
		to_chat(user,  SPAN_ALERT("<b>[src]</b> buzzes, \"Report had no brig sentence.\""))
		return 0

	if( crime.brig_sentence >= PERMABRIG_SENTENCE )
		to_chat(user,  SPAN_ALERT("<b>[src]</b> buzzes, \"The criminal has a HUT sentence and needs to be detained until transfer.\""))
		return 0
