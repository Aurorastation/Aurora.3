...
/obj/machinery/requests_console/Topic(href, href_list)
	...
	if( href_list["department"] && message )
		...
		if(pass)
			...
			audible_message(text("[icon2html(src, usr)] *The Requests Console beeps: 'NOTICE: No server detected!'"),,4)
	...
	if (istype(O, /obj/item/card/id))
		...
		if(lid)	//More of that restocking business
			...
			for (var/mob/U in hearers(4, src.loc))
				U.show_message(text("[icon2html(src, usr)] *The Requests Console beeps: 'Paper added.'"))
		else
				...
				for(var/cc in allConsoles)
					...
					if(Console == sendto)
						...
						for (var/mob/player in hearers(4, Console.loc))
							player.show_message(text("[icon2html(Console, usr)] *The Requests Console beeps: 'Fax received'"))
						Console.paperstock--
				if(sent == 1)
					user.show_message(text("[icon2html(src, usr)] *The Requests Console beeps: 'Message Sent.'"))
			else
				user.show_message(text("[icon2html(src, usr)] *The Requests Console beeps: 'NOTICE: No server detected!'"))
