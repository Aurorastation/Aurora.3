...
/obj/machinery/suit_cycler/update_icon()
	...
	if(boots)
		var/image/boot_image = boots.get_mob_overlay(null, boots.icon, boots.icon_state, slot_shoes_str)
		if(boot_image)
			add_overlay(boot_image)
	if(mask)
		var/image/mask_image = mask.get_mob_overlay(null, mask.icon, mask.icon_state, slot_wear_mask_str)
		if(mask_image)
			add_overlay(mask_image)
	if(occupant)
