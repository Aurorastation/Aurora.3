...
/obj/structure/barricade/attackby(obj/item/W as obj, mob/user as mob)
	if (istype(W, /obj/item/stack))
		...
		if (health < maxhealth)
			...
			if(do_after(user, 20 - 20 * skill_time_reduction("construction", 0.05, user)) && health < maxhealth)
				if (D.use(1))
					health = maxhealth
...
/obj/item/deployable_kit/attack_self(mob/user)
	...
	if(do_after(user, assembly_time - assembly_time * skill_time_reduction("construction", 0.1, user)))
		assemble_kit(user)
		qdel(src)
