...
/obj/machinery/suit_storage_unit/verb/move_inside()
	...
	if ( (src.OCCUPANT) || (src.HELMET) || (src.SUIT) )
		...
		return
	usr.visible_message("<span class='notice'>[usr] starts squeezing into [src]!</span>", "<span class='notice'>You start squeezing into [src]!</span>")
	if(do_after(usr, 10))
		usr.stop_pulling()
...
/obj/machinery/suit_storage_unit/attackby(obj/item/I as obj, mob/user as mob)
	...
	if ( istype(I, /obj/item/weapon/grab) )
		...
		if ( (src.OCCUPANT) || (src.HELMET) || (src.SUIT) ) //Unit needs to be absolutely empty
			...
			return
		user.visible_message("<span class='notice'>[user] starts putting [G.affecting] into [src].</span>", "<span class='notice'>You start putting [G.affecting] into [src].</span>")
		if(do_after(user, 20))
