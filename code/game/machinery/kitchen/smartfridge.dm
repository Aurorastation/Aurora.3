...
/obj/machinery/smartfridge/attackby(var/obj/item/O as obj, var/mob/user as mob)
	...
	if(accept_check(O))
		if(contents.len >= max_n_of_items)
			...
			user.visible_message("<b>[user]</b> adds \a [O] to \the [src].", SPAN_NOTICE("You add \the [O] to \the [src]."))
	...
	if(istype(O, /obj/item/storage/bag) || istype(O, /obj/item/storage/box/produce))
		...
		if(plants_loaded)
			...
			user.visible_message("<b>[user]</b> loads \the [src] with \the [P].", SPAN_NOTICE("You load \the [src] with \the [P]."))
			if(P.contents.len > 0)
