...
/obj/machinery/cryopod/attackby(var/obj/item/grab/G, var/mob/user)
	if(istype(G))
		...
		if(willing)
			user.visible_message("<b>[user]</b> starts putting \the [G.affecting] into \the [src].", SPAN_NOTICE("You start putting [G.affecting] into [src]."), range = 3)
...
/obj/machinery/cryopod/MouseDrop_T(atom/movable/O, mob/living/user)
	...
	if(willing)
		if(L == user)
			...
			user.visible_message("<b>[user]</b> starts putting \the [L] into \the [src].", SPAN_NOTICE("You start putting \the [L] into \the [src]."), range = 3)
