...
/obj/machinery/iv_drip/machinery_process()
	...
	if(src.attached)
		...
		if(!(get_dist(src, src.attached) <= 1 && isturf(src.attached.loc)))
			var/obj/item/organ/external/affecting = src.attached:get_organ(pick(TARGET_R_ARM, TARGET_L_ARM))
			src.attached.visible_message("<span class='warning'>The needle is ripped out of [src.attached]'s [affecting.limb_name == TARGET_R_ARM ? "right arm" : "left arm"].</span>", "<span class='danger'>The needle <B>painfully</B> rips out of your [affecting.limb_name == TARGET_R_ARM ? "right arm" : "left arm"].</span>")
