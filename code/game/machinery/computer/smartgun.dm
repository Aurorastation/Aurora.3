...
/obj/machinery/computer/smartguncontrol/attack_ai(var/mob/user as mob)
		return src.attack_hand(user)

/obj/machinery/computer/smartguncontrol/attack_hand(var/mob/user as mob)
	if(..())
		return
	...
	if(screen == 0)
		...
		for(var/obj/item/device/firing_pin/security_pin/P in smartguns)
			...
			if(!P.gun) continue
			dat += "[P.gun.name] - [P.registered_user] - [get_area(Tr)]"
			if(P.lockstatus)
...
/obj/machinery/computer/smartguncontrol/process()
	if(!..())
		src.updateDialog()
	return
