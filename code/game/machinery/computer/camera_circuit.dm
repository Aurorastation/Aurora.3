...
/obj/item/circuitboard/camera/New()
...
/obj/item/circuitboard/camera/proc/updateBuildPath()
	build_path = null
...
/obj/item/circuitboard/camera/attackby(var/obj/item/I, var/mob/user)//if(health > 50)
	..()
...
/obj/item/circuitboard/camera/attack_self(var/mob/user)
	if(!secured && ishuman(user))
		user.machine = src
		interact(user, 0)

/obj/item/circuitboard/camera/proc/interact(var/mob/user, var/ai=0)
	if(secured)
		return
	if (!ishuman(user))
		return ..(user)
...
/obj/item/circuitboard/camera/Topic(href, href_list)
	..()
	if( href_list["close"] )
...
/obj/item/circuitboard/camera/updateDialog()
