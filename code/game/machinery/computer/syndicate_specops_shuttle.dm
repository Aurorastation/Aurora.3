...
/proc/syndicate_elite_process()
	...
	for(var/turf/T in get_area_turfs(end_location) )
		...
		to_chat(M, SPAN_WARNING("You have arrived to [current_map.station_name]. Commence operation!"))
...
/obj/machinery/computer/syndicate_elite_shuttle/Topic(href, href_list)
	...
	if (href_list["sendtodock"])
		...
		to_chat(usr, SPAN_NOTICE("The Syndicate Elite shuttle will arrive on [current_map.station_name] in [(SYNDICATE_ELITE_MOVETIME/10)] seconds."))
