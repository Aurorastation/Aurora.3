...
/obj/structure/AIcore/deactivated/attackby(var/obj/item/W, var/mob/user)
	...
	if(istype(W, /obj/item/aicard))
		...
		if(anchored)
			user.visible_message("<b>[user]</b> starts to unbolt \the [src] from the plating...")
			if(!do_after(user,40/W.toolspeed))
				user.visible_message("<b>[user]</b> decides not to unbolt \the [src].")
				return
			user.visible_message("<b>[user]</b> finishes unfastening \the [src]!")
			anchored = 0
			return
		else
			user.visible_message("<b>[user]</b> starts to bolt \the [src] to the plating...")
			if(!do_after(user,40/W.toolspeed))
				user.visible_message("<b>[user]</b> decides not to bolt \the [src].")
				return
			user.visible_message("<b>[user]</b> finishes fastening down \the [src]!")
			anchored = 1
			return
	else
		return ..()
