...
/obj/structure/AIcore/pra/attackby(obj/item/W as obj, mob/user as mob)
	if(W.iswrench())
		if(anchored)
			user.visible_message("<span class='notice'>\The [user] starts to unbolt \the [src] from the plating...</span>")
			if(!do_after(user,40/W.toolspeed))
...
/obj/structure/AIcore/pra/complete/attackby(obj/item/W as obj, mob/user as mob)

	if(istype(W, /obj/item/aicard/pra))
		var/obj/item/aicard/pra/card = W
		var/mob/living/silicon/ai/transfer = locate() in card
		if(transfer)
			load_ai(transfer,card,user)
		else
...
/obj/structure/AIcore/pra/complete/proc/load_ai(var/mob/living/silicon/ai/pra/transfer, var/obj/item/aicard/card, var/mob/user)
