...
/obj/machinery/computer/arcade/orion_trail/proc/win()
	...
	if(emagged)
		...
		log_game(text="[key_name(usr)] made it to Orion on an emagged machine and got an explosive toy ship.",ckey=key_name(usr))
	else
		prizevend()
	event = null
	src.updateUsrDialog()
...
/obj/item/weapon/orion_ship/attack_self(mob/user)
	...
	log_game(text="[key_name(usr)] primed an explosive Orion ship for detonation.",ckey=key_name(usr))
