...
//Orion Trail Events
#define ORION_TRAIL_RAIDERS				"Vaurca Raiders"
		...
		if(ORION_VIEW_MAIN)
			...
			switch(event)
				...
				if(ORION_TRAIL_RAIDERS)
					event_desc   = "You arm yourselves as you prepare to fight off the Vaurca menace!"
		...
		if(ORION_TRAIL_RAIDERS)
			if(prob(17 * settlers.len))
				...
				if(prob(10*settlers.len))
					remove_settler(null, "was kidnapped by the Vaurca!")
				change_resource(null,-1)
