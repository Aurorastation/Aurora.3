	...
	if(istype(W, /obj/item/device/debugger))
		if(!shut_up)
			...
			if(do_after(user, 100 / W.toolspeed, act_target = src))
				to_chat(user, "<b>[W]</b> reads, \"Solution found. Fix applied.\".")
				shut_up = TRUE
		if(shoot_inventory)
			...
			to_chat(user, SPAN_WARNING("\The [W] reads, \"Software error detected. Rectifying.\"."))
			if(do_after(user, 100 / W.toolspeed, act_target = src))
				to_chat(user, "<b>[W]</b> reads, \"Solution found. Fix applied. Have a NanoTrasen day!\".")
				shoot_inventory = FALSE
		else
			to_chat(user, "<b>[W]</b> reads, \"All systems nominal.\".")
