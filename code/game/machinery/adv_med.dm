...
/obj/machinery/bodyscanner/attackby(obj/item/grab/G, mob/user)
	...
	if (do_mob(user, G.affecting, 30 - 30 * skill_time_reduction("medical", 0.1, user), needhand = 0))
...
/obj/machinery/bodyscanner/MouseDrop_T(atom/movable/O as mob|obj, mob/living/user as mob)
	...
	if (do_mob(user, L, 30 - 30 * skill_time_reduction("medical", 0.1, user), needhand = 0))
		if (bucklestatus == 2)
