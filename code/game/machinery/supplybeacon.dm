...
/obj/item/supply_beacon/attack_self(var/mob/user)
	user.visible_message("<b>[user]</b> begins setting up \the [src].")
	if(!do_after(user, deploy_time))
		return
	var/obj/S = new deploy_path(get_turf(user))
	user.visible_message("<b>[user]</b> deploys \the [S].")
	user.unEquip(src)
	qdel(src)
...
/obj/machinery/power/supply_beacon/attackby(var/obj/item/W, var/mob/user)
	if(!use_power && W.iswrench())
		if(!anchored && !connect_to_network())
			...
			return
		anchored = !anchored
		user.visible_message("<b>[user]</b> [anchored ? "secures" : "unsecures"] \the [src].")
