/proc/get_footstep(var/footstep_type, var/mob/caller)
	...
	if(!.)
		...
		. = pick(FS.footstep_types)

/turf/simulated/proc/get_footstep_type(var/mob/caller)
...
/turf/simulated/floor/get_footstep_type(var/mob/caller)
	. = ..()
	if(!.)
...
/mob/living/carbon/human/proc/handle_footsteps()
	...
	if(istype(T))
		var/footsound = T.get_footstep_type(src)
		if(footsound)
