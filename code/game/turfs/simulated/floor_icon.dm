/turf/simulated/floor/update_icon(var/update_neighbors)
		...
		for(var/step_dir in GLOB.cardinal)
			...
			if (!is_linked)
				...
				if(flooring.has_damage_state && !isnull(broken) && (flooring.flags & TURF_CAN_BREAK))
					...
					add_overlay(get_flooring_overlay("[flooring.icon]_[flooring.icon_base]-edge-[step_dir]", "[flooring.icon_base]_edges", step_dir, TRUE))
	...
	if(decals && decals.len && flooring.can_paint)
