...
/proc/randmutg(var/mob/living/M)
	...
	if(ishuman(M))
		...
		if(H.species && H.species.flags & NO_SCAN)
			return
	M.dna.check_integrity()
	var/block = pick(XRAYBLOCK,FIREBLOCK,NOBREATHBLOCK,REMOTEVIEWBLOCK,REGENERATEBLOCK,INCREASERUNBLOCK,REMOTETALKBLOCK,MORPHBLOCK,BLENDBLOCK,NOPRINTSBLOCK,SHOCKIMMUNITYBLOCK,SMALLSIZEBLOCK)
	M.dna.SetSEState(block, 1)
