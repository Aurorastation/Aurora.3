...
/obj/machinery/computer/scan_consolenew/Topic(href, href_list)
	...
	if (href_list["pulseRadiation"])
		...
		src.connected.occupant.apply_damage(((src.radiation_intensity*3)+src.radiation_duration*3), IRRADIATE, damage_flags = DAMAGE_DISPERSED)
	...
	if (href_list["pulseUIRadiation"])
		...
		if (prob((80 + (src.radiation_duration / 2))))
			...
			src.connected.occupant.apply_damage((src.radiation_intensity+src.radiation_duration), IRRADIATE, damage_flags = DAMAGE_DISPERSED)
		else
			if	(prob(20+src.radiation_intensity))
				...
				randmuti(src.connected.occupant)
				src.connected.occupant.UpdateAppearance()
			src.connected.occupant.apply_damage(((src.radiation_intensity*2)+src.radiation_duration), IRRADIATE, damage_flags = DAMAGE_DISPERSED)
		src.connected.locked = lock_state
		return 1 // return 1 forces an update to all Nano uis attached to src
	...
	if(href_list["bufferOption"])
		...
		if (bufferOption == "transfer")
			...
			if ((buf.types & DNA2_BUF_UI))
				...
				domutcheck(src.connected.occupant,src.connected)
			src.connected.occupant.apply_damage(rand(20,50), damage_flags = DAMAGE_DISPERSED)
			return 1
