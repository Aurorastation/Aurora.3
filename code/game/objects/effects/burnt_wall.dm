...
/obj/effect/overlay/burnt_wall/attackby(obj/item/I, mob/user)
	if(I.iswelder())
		...
		if(WT.remove_fuel(0,user))
			user.visible_message("<b>[user] starts slicing \the [src] apart.", SPAN_NOTICE("You start slicing \the [src] apart."))
			playsound(src, 'sound/items/Welder.ogg', 100, 1)
			var/slice_time = reinf_material ? 100 : 30
			if(do_after(user, slice_time, TRUE))
				user.visible_message("<b>[user] slices \the [src] apart.", SPAN_NOTICE("You slice \the [src] apart."))
