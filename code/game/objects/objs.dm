 ...
 * When a different type then the original type is returned, the original type still is responsible to return the appropiate values for the new type during persistent_objects_get_content().
