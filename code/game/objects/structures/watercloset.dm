	...
	if (istype(RG) && RG.is_open_container())
		...
		switch(atype)
			if ("Fill")
				...
				user.visible_message(SPAN_NOTICE("[user] fills \the [RG] using \the [src]."),
			...
			if ("Empty")
				...
				user.visible_message(SPAN_NOTICE("[user] empties [empty_amount]u of \the [RG] into \the [src]."),
									 SPAN_NOTICE("You empty [empty_amount]u of \the [RG] into \the [src]."))
