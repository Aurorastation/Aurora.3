...
/obj/structure/girder/attackby(obj/item/W as obj, mob/user as mob)
	if(W.iswrench() && state == 0)
		...
		if(do_after(user, 30/W.toolspeed - 30/W.toolspeed * skill_time_reduction("construction", 0.1, user)))
			if(!src) return
		...
		if(WT.active)
			to_chat(user, "<span class='notice'>Now slicing apart the girder...</span>")
			if(do_after(user, 30/W.toolspeed * skill_time_reduction("construction", 0.1, user)))
				if(!src) return
				to_chat(user, "<span class='notice'>You slice apart the girder!</span>")
				dismantle()
		else
		...
		if(WT.active)
			to_chat(user, "<span class='notice'>Now slicing apart the girder...</span>")
			if(do_after(user, 60/W.toolspeed * skill_time_reduction("construction", 0.1, user)))
				if(!src) return
				to_chat(user, "<span class='notice'>You slice apart the girder!</span>")
				dismantle()
		else
		...
		to_chat(user, "<span class='notice'>Now slicing apart the girder...</span>")
		if(do_after(user, 150 - 150 * skill_time_reduction("construction", 0.1, user)))
			if(!src)
				return
			to_chat(user, "<span class='notice'>You slice apart the girder!</span>")
			dismantle()
...
/obj/structure/girder/proc/construct_wall(obj/item/stack/material/S, mob/user)
	...
	if(!do_after(user, 40 - 40 * skill_time_reduction("construction", 0.1, user)) || !S.use(2))
...
/obj/structure/girder/cult/attackby(obj/item/W as obj, mob/user as mob)
	if(W.iswrench())
		...
		if(WT.active)
			to_chat(user, "<span class='notice'>Now slicing apart the girder...</span>")
			if(do_after(user, 60/W.toolspeed - 60/W.toolspeed * skill_time_reduction("construction", 0.1, user)))
				to_chat(user, "<span class='notice'>You slice apart the girder!</span>")
			dismantle()
		else
			to_chat(user, "<span class='notice'>You need to activate the weapon to do that!</span>")
