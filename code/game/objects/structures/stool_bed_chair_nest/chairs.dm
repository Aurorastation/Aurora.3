...
/obj/structure/bed/stool/chair/update_icon()
	...
	// Padding overlay.
	if(padding_material)
		generate_overlay_cache(padding_material, CACHE_TYPE_PADDING_OVER, ABOVE_HUMAN_LAYER + 0.01, TRUE) // Generate padding overlay for backrest

	if(buckled)
		generate_overlay_cache(material, CACHE_TYPE_ARMREST, ABOVE_HUMAN_LAYER) // Generate armrests
		if(padding_material)
			generate_overlay_cache(padding_material, CACHE_TYPE_PADDING_ARMREST, ABOVE_HUMAN_LAYER + 0.01, TRUE) // Generate padding overlay for armrest
