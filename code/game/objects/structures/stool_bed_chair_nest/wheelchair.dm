...
/obj/structure/bed/chair/wheelchair/Bump(atom/A)
	...
	if(propelled || (pulling && (pulling.a_intent == I_HURT)))
		...
		if(pulling)
			...
			msg_admin_attack(text="[pulling.name] ([pulling.ckey]) has thrusted [occupant.name]'s ([occupant.ckey]) [name] into \a [A] (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[pulling.x];Y=[pulling.y];Z=[pulling.z]'>JMP</a>)",ckey=key_name(pulling),ckey_target=key_name(occupant))
		else
