...
/obj/item/bedsheet/proc/toggle_fold(var/mob/living/user) // Fold sheets to make them more portable through secret janitor-fu.
	...
	if (do_after(user, 25, src))
		...
		var/folds = fold
		user.visible_message("<b>[user]</b> [folds ? "unfolds" : "folds"] \the [src].",
...
/obj/item/bedsheet/proc/toggle_roll(var/mob/living/user) // Tuck yourself in just by clicking. Also automatically rests you (if you're under it)
	...
	if (do_after(user, 6, src))
		...
		playsound(get_turf(loc), "rustle", 15, 1, -5)
		var/rolls = roll
		user.visible_message("<b>[user]</b> [rolls ? "unrolls" : "rolls"] \the [src].",
...
/obj/item/bedsheet/attackby(obj/item/I, mob/user)
	if(I.isscrewdriver())
		user.visible_message("<b>[user]</b> begins poking eyeholes in \the [src] with \the [I].",
		...
		user.visible_message("<b>[user]</b> begins cutting up [src] with [I].",
