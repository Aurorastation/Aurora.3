...
/obj/structure/machinery/crusher_base/update_icon()
	...
	if(powered(AREA_USAGE_EQUIP))
		if(blocked == 1)
			...
			emissive_overlay = emissive_appearance(icon, "[asmtype]-overlay-red", src)
		else if(action != "idle")
			image_overlay = image(icon, "[asmtype]-overlay-orange")
			emissive_overlay = emissive_appearance(icon, "[asmtype]-overlay-orange", src)
		else
			image_overlay = image(icon, "[asmtype]-overlay-green")
			emissive_overlay = emissive_appearance(icon, "[asmtype]-overlay-green", src)
