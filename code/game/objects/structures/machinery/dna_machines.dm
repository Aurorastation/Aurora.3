...
/obj/structure/machinery/dna_scannernew/relaymove(mob/living/user, direction)
	. = ..()
...
/obj/structure/machinery/dna_scannernew/verb/eject()
	set src in oview(1)
	set category = "Object"
...
/obj/structure/machinery/dna_scannernew/proc/eject_occupant()
	src.go_out()
...
/obj/structure/machinery/dna_scannernew/verb/move_inside()
	set src in oview(1)
	set category = "Object"
...
/obj/structure/machinery/dna_scannernew/attackby(obj/item/attacking_item, mob/user)
...
/obj/structure/machinery/dna_scannernew/mouse_drop_receive(atom/dropped, mob/user, params)
	if(!istype(user))
		return
...
/obj/structure/machinery/dna_scannernew/proc/put_in(var/mob/M)
	...
	if(locate(/obj/structure/machinery/computer/cloning, get_step(src, NORTH)) \
		|| locate(/obj/structure/machinery/computer/cloning, get_step(src, SOUTH)) \
		|| locate(/obj/structure/machinery/computer/cloning, get_step(src, EAST)) \
		|| locate(/obj/structure/machinery/computer/cloning, get_step(src, WEST)))
...
/obj/structure/machinery/dna_scannernew/proc/go_out()
...
/obj/structure/machinery/dna_scannernew/ex_act(severity)
	switch(severity)
		if(1.0)
...
/obj/structure/machinery/computer/scan_consolenew
	...
	var/obj/structure/machinery/dna_scannernew/connected = null
...
/obj/structure/machinery/computer/scan_consolenew/Initialize()
	..()
	for(var/i=0;i<3;i++)
		buffers[i+1]=new /datum/dna2/record
	return INITIALIZE_HINT_LATELOAD

/obj/structure/machinery/computer/scan_consolenew/LateInitialize()
	. = ..()
...
/obj/structure/machinery/computer/scan_consolenew/Destroy()
	connected = null
	disk = null
...
/obj/structure/machinery/computer/scan_consolenew/ex_act(severity)
	switch(severity)
		if(1.0)
			//SN src = null
			qdel(src)
...
/obj/structure/machinery/computer/scan_consolenew/proc/all_dna_blocks(var/list/buffer)
	var/list/arr = list()
	for(var/i = 1, i <= buffer.len, i++)
		arr += "[i]:[num2hex(buffer[i], 3)]"
	return arr

/obj/structure/machinery/computer/scan_consolenew/proc/setInjectorBlock(var/obj/item/dnainjector/I, var/blk, var/datum/dna2/record/buffer)
...
/*
/obj/structure/machinery/computer/scan_consolenew/process() //not really used right now
...
/obj/structure/machinery/computer/scan_consolenew/attack_ai(user as mob)
...
/obj/structure/machinery/computer/scan_consolenew/attack_hand(user as mob)
	if(!..())
		// DNA Modifier is currently deprecated (old NanoUI code)
		to_chat(user, SPAN_WARNING("The firmware on this thing is so hopelessly out of date as to render it unusable. Dang."))
		return
