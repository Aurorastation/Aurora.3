...
/obj/structure/machinery/door/airlock/proc/set_airlock_overlays(state, force_compile)
	...
	if(arePowerSystemsOn())
		switch(state)
			if(AIRLOCK_CLOSED)
				if(lights && locked)
					lights_overlay = overlay_image(bolts_file, plane = ABOVE_LIGHTING_PLANE, plane_context = src)
					set_light(1, 2, COLOR_RED_LIGHT)

			if(AIRLOCK_DENY)
				if(lights)
					lights_overlay = overlay_image(deny_file, plane = ABOVE_LIGHTING_PLANE, plane_context = src)
					set_light(1, 2, COLOR_RED_LIGHT)

			if(AIRLOCK_EMAG)
				sparks_overlay = overlay_image(emag_file, plane = ABOVE_LIGHTING_PLANE, plane_context = src)

			if(AIRLOCK_CLOSING)
				if(lights)
					lights_overlay = overlay_image(lights_file, plane = ABOVE_LIGHTING_PLANE, plane_context = src)
		...
		if(stat & BROKEN)
			damage_overlay = overlay_image(sparks_broken_file, plane = ABOVE_LIGHTING_PLANE, plane_context = src)
		else if (health < maxhealth * 3/4 && !(stat & NOPOWER))
			damage_overlay = overlay_image(sparks_damaged_file, plane = ABOVE_LIGHTING_PLANE, plane_context = src)
