...
/obj/structure/machinery/mass_driver/Initialize()
	. = ..()
	if(_wifi_id)
		wifi_receiver = new(_wifi_id, src)

/obj/structure/machinery/mass_driver/Destroy()
	qdel(wifi_receiver)
	wifi_receiver = null
	return ..()
...
/obj/structure/machinery/mass_driver/attackby(obj/item/attacking_item, mob/user)
...
/obj/item/mass_driver_diy/attack_self(mob/user)
	...
	if(do_after(user, 8 SECONDS, src, DO_REPAIR_CONSTRUCT))
		...
		var/obj/structure/machinery/mass_driver/mining/R = new /obj/structure/machinery/mass_driver/mining(user.loc)
		R.id = master_id
		R.power = 2.0
...
/obj/item/mass_driver_button/afterattack(var/atom/A, var/mob/user, var/adjacent, var/clickparams)
	...
	var/stuff_on_wall = 0
	if (locate(/obj/structure/machinery/button) in W)
		stuff_on_wall = 1

	//crude, but will cover most cases. We could do stuff like check pixel_x/y but it's not really worth it.
	for (var/dir in GLOB.cardinals)
		var/turf/T = get_step(W, dir)
		if (locate(/obj/structure/machinery/button) in T)
			stuff_on_wall = 1
			break
	...
	var/obj/structure/machinery/button/mass_driver/B = new /obj/structure/machinery/button/remote/driver(user.loc)
...
/obj/structure/machinery/mass_driver/mining
