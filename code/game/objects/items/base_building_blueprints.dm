...
/obj/item/base_building_blueprints/mechanics_hints()
	. += ..()
...
/obj/item/base_building_blueprints/Initialize()
	. = ..()
	return INITIALIZE_HINT_LATELOAD

/obj/item/base_building_blueprints/LateInitialize()
	invalid_z_levels = SSmapping.levels_by_trait(ZTRAIT_STATION)
	AddComponent(/datum/component/eye/base_planner)

/obj/item/base_building_blueprints/AltClick(mob/user)
	if(folded)
		...
		folded = FALSE
		anchored = TRUE
		icon_state = "base_planner_unfolded"
	else
		folded = TRUE
		anchored = FALSE
		icon_state = "base_planner"

/obj/item/base_building_blueprints/attack_hand(mob/user)
	if(folded)
		return ..()
