...
/obj/item/stack/nanopaste/attack(mob/living/M as mob, mob/user as mob, var/target_zone)
	...
	if (istype(M,/mob/living/silicon/robot))	//Repairing cyborgs
		...
		if (R.getBruteLoss() || R.getFireLoss() )
			...
			if(do_mob(user, M, 7 - 7 * skill_time_reduction("devices", 0.1, user)))
	...
	if (istype(M,/mob/living/carbon/human))		//Repairing robolimbs
		...
		if (S && (S.status & ORGAN_ASSISTED))
			if(S.get_damage())
				...
				if(do_mob(user, M, 7 - 7 * skill_time_reduction("medical", 0.1, user)))
...
/obj/item/stack/nanopaste/surge/attack(mob/living/carbon/human/M as mob, mob/user as mob, var/target_zone)
	...
	if (isipc(M))
		...
		if(isnull(s))
			...
			if (!do_mob(user, M, 2 - 2 * skill_time_reduction("devices", 0.1, user)))
				return 0
