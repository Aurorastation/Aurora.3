...
/obj/item/device/flash/attack(mob/living/M, mob/living/user, var/target_zone)
	...
	user.attack_log += text("\[[time_stamp()]\] <span class='warning'>Used [src] to flash [M.name] ([M.ckey])</span>")
	msg_admin_attack("[user.name] ([user.ckey]) Used [src] to flash [M.name] ([M.ckey]) (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[user.x];Y=[user.y];Z=[user.z]'>JMP</a>)",ckey=key_name(user),ckey_target=key_name(M))
