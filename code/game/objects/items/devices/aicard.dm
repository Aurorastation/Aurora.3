...
/obj/item/aicard/pra/update_icon()
	if(carded_ai)
		if(carded_ai.stat)
			icon_state = "aicard-404"
		else
...
/obj/item/aicard/pra/grab_ai(var/mob/living/silicon/ai/ai, var/mob/living/user)
	if(!istype(ai, /mob/living/silicon/ai/pra))
		return
	...
	if(istype(ai.loc, /turf/))
		new /obj/structure/AIcore/pra/complete(get_turf(ai))
