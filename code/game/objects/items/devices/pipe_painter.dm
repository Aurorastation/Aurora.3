...
/obj/item/device/pipe_painter/afterattack(var/atom/A, var/mob/user, proximity)
	...
	if(!in_range(user, A))
		return
	else if(is_type_in_list(A, list(/obj/machinery/atmospherics/pipe/tank, /obj/machinery/atmospherics/pipe/vent, /obj/machinery/atmospherics/pipe/simple/heat_exchanging)))
		return
