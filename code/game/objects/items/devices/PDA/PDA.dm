		...
		if(FC.updated > feed["updated"])
			...
			if(!FC.censored)
				...
				for(var/datum/feed_message/FM in FC.messages)
					index++
					if(FM.img)
						send_rsc(usr, FM.img, "pda_news_tmp_photo_[feed["channel"]]_[index].png")
...
/obj/item/device/pda/proc/new_info(var/beep_silent, var/message_tone, var/reception_message)
	if (!beep_silent)
		...
		for (var/mob/O in hearers(2, loc))
			O.show_message(text("[hicon(src)] *[message_tone]*"))
...
/obj/item/device/pda/proc/new_news(var/message)
	new_info(news_silent, newstone, news_silent ? "" : "[hicon(src)] <b>[message]</b>")
...
/obj/item/device/pda/proc/new_message(var/sending_unit, var/sender, var/sender_job, var/message)
	var/reception_message = "[hicon(src)] <b>Message from [sender] ([sender_job]), </b>\"[message]\" (<a href='byond://?src=\ref[src];choice=Message;skiprefresh=1;target=\ref[sending_unit]'>Reply</a>)"
...
/obj/item/device/pda/ai/new_message(var/atom/movable/sending_unit, var/sender, var/sender_job, var/message)
	...
	var/reception_message = "[hicon(src)] <b>Message from [sender] ([sender_job]), </b>\"[message]\" (<a href='byond://?src=\ref[src];choice=Message;skiprefresh=1;target=\ref[sending_unit]'>Reply</a>) [track]"
