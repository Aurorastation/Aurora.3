...
/obj/item/device/flashlight/attack(mob/living/M as mob, mob/living/user as mob)
	...
	if(on && user.zone_sel.selecting == BP_EYES)
		...
		if(istype(H))
			...
			user.visible_message("<b>[user]</b> directs [src] to [M]'s eyes.", SPAN_NOTICE("You direct [src] to [M]'s eyes."))
			...
			if (H != user)	//can't look into your own eyes buster
				...
				if(XRAY in M.mutations)
					to_chat(user, "<b>[M]</b>'s pupils give an eerie glow!")
				if(vision.damage)
					to_chat(user, SPAN_WARNING("There's visible damage to [M]'s [vision.name]!"))
				else if(M.eye_blurry)
					to_chat(user, "<b>[M]</b>'s pupils react slower than normally.")
				...
				if(H.reagents.has_any_reagent(pinpoint) || ingested.has_any_reagent(pinpoint))
					to_chat(user, "<b>[M]</b>'s pupils are already pinpoint and cannot narrow any more.")
					...
					to_chat(user, "<b>[M]</b>'s pupils narrow slightly, but are still very dilated.")
				else
					to_chat(user, "<b>[M]</b>'s pupils narrow.")
