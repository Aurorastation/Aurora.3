...
/obj/item/device/radio/headset/wrist/verb/change_layer()
	...
	if(mob_wear_layer == ABOVE_SUIT_LAYER)
		mob_wear_layer = ABOVE_UNIFORM_LAYER
	else
		mob_wear_layer = ABOVE_SUIT_LAYER
	to_chat(usr, SPAN_NOTICE("\The [src] will now layer [mob_wear_layer == ABOVE_SUIT_LAYER ? "over" : "under"] your outerwear."))
	if (ishuman(src.loc))
		var/mob/living/carbon/human/H = src.loc
		if(H.wrists == src)
			H.update_inv_wrists()
...
/obj/item/device/radio/headset/wrist/clip/get_wrist_examine_text(mob/living/carbon/human/user)
	if(!istype(user))
		return ..()
	return "clipped to [user.get_pronoun("his")] [(mob_wear_layer == ABOVE_SUIT_LAYER) && user.wear_suit ? user.wear_suit.name : user.w_uniform ? user.w_uniform.name : "chest"]"
