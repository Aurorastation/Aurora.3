...
/obj/item/toy/crossbow
	...
	afterattack(atom/target as mob|obj|turf|area, mob/user as mob, flag)
		...
		if (locate (/obj/structure/table, src.loc))
			...
			for(var/i=0, i<6, i++)
				if (D)
					...
					for(var/mob/living/M in D.loc)
						...
						if(M == user) continue
						D.visible_message(SPAN_WARNING("\The [M] is hit by the foam dart!"))
			...
			user.Weaken(5)
			user.visible_message(SPAN_WARNING("\The [user] realizes they are out of ammo and starts scrounging for some!"))
...
// ******* Check
		...
		if (src.bullets > 0 && M.lying)
			...
			for(var/mob/O in viewers(M, null))
				if(O.client)
					O.show_message(SPAN_DANGER("\The [user] casually lines up a shot with [M]'s head and pulls the trigger!"), 1, SPAN_WARNING("You hear the sound of foam bouncing."), 2)
