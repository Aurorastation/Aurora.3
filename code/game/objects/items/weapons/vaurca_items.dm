...
/obj/item/vaurca/box/attack_self(mob/user as mob)
	...
	if(isvaurca(user))
		...
		to_chat(user, SPAN_NOTICE("You are surprised to recognize the markings of the Apex, the Masters! You know this thing... (You must stand still to complete the puzzle box.)"))
		if(do_after(user, 100))
			to_chat(user, SPAN_NOTICE("After a few seconds of remembering, you input the solution to the riddle - a lovely riddle indeed - and open the box to reveal an ancient thing."))
			var/obj/item/archaeological_find/X = new /obj/item/archaeological_find
			user.remove_from_mob(src)
			user.put_in_hands(X)
			qdel(src)
		...
		if(do_after(user, 60))
			to_chat(user, SPAN_NOTICE("After a few more seconds, you hesitantly turn the first piece of the puzzle box."))
			if(do_after(user,30))
				to_chat(user, SPAN_NOTICE("After nothing bad happens, you dive into the puzzle with a feeling of confidence!"))
				if(do_after(user,200))
					to_chat(user, SPAN_NOTICE("Twenty seconds pass, and suddenly you're feeling a lot less confident. You struggle on..."))
					if(do_after(user,100))
...
/obj/item/gun/launcher/crossbow/vaurca/proc/load(obj/item/W, mob/user)
	...
	user.visible_message("<b>[user]</b> inserts \a [W] into [src].", SPAN_NOTICE("You insert \a [W] into [src]."))
...
/obj/item/gun/launcher/crossbow/vaurca/proc/unload(mob/user)
	if(belt.len)
		...
		belt.len--
		user.put_in_hands(R)
		user.visible_message("<b>[user]</b> removes \a [R] from [src].", SPAN_NOTICE("You remove \a [R] from [src]."))
	else
