...
/obj/item/dnainjector/attack(mob/living/target_mob, mob/living/user, target_zone)
	...
	target_mob.log_message("Has been injected with [name] by [user.name] ([user.ckey])", LOG_VICTIM, log_globally = FALSE)
	user.log_message("Used the [name] to inject [target_mob.name] ([target_mob.ckey])", LOG_ATTACK, log_globally = FALSE)
