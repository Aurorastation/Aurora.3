...
/obj/item/material/twohanded/attack_self(mob/user as mob)
	...
	if(wielded) //Trying to unwield it
		...
		if(user.get_inactive_hand())
			...
			return
		wield()
		to_chat(user, SPAN_NOTICE("You grab the [base_name] with both hands."))
		if (src.wieldsound)
...
/obj/item/material/twohanded/chainsaw/AltClick(mob/user as mob)
	...
	if(powered)
		...
		user.visible_message(\
			SPAN_NOTICE("[user] pulls the cord on the [src], but nothing happens."),\
			SPAN_NOTICE("You pull the cord on the [src], but nothing happens."),\
		...
		for(var/i in 1 to max)
			user.visible_message(\
				SPAN_NOTICE("[user] pulls the cord on the [src]..."),\
				SPAN_NOTICE("You pull the cord on the [src]..."),\
