...
/obj/item/large_trap_foundation/attackby(obj/item/attacking_item, mob/user)
	if(istype(attacking_item, /obj/item/stack/rods))
		...
		if(O.get_amount() >= 12)
			user.visible_message("<b>[user]</b> starts adding metal bars to \the [src]...", SPAN_NOTICE("You start adding metal bars to \the [src]..."))
			...
			user.visible_message("<b>[user]</b> finishes constructing \the [new_trap].", SPAN_NOTICE("You finish constructing \the [new_trap]."))
