...
/obj/item/trap/animal/large/attackby(obj/item/W, mob/user)
	if(W.iswrench())
		...
		if(do_after(user, 30/W.toolspeed - 30/W.toolspeed * skill_time_reduction("construction", 0.1, user), act_target = src))
			anchored = !anchored
			user.visible_message("<span class='notice'>[user] [anchored ? "" : "un" ]secures \the [src]!</span>",
								"<span class='notice'>You [anchored ? "" : "un" ]secure \the [src]!</span>")
...
/obj/item/large_trap_foundation/attackby(obj/item/W, mob/user)
	if(istype(W, /obj/item/stack/rods))
		...
		if(O.get_amount() >= 12)
			...
			if (!do_after(user, 2 SECONDS - 2 SECONDS * skill_time_reduction("construction", 0.15, user), act_target = src))
				return
