...
/obj/item/weapon/nullrod/attack(mob/M as mob, mob/living/user as mob) //Paste from old-code to decult with a null rod.
	...
	if (M.stat !=2 && ishuman(M) && user.a_intent != I_HURT)
		...
		msg_admin_attack("[user.name] ([user.ckey]) attempted to deconvert [M.name] ([M.ckey]) with [src.name] (INTENT: [uppertext(user.a_intent)]) (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[user.x];Y=[user.y];Z=[user.z]'>JMP</a>)")
