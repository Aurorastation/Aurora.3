...
/obj/item/rfd/construction/proc/alter_turf(var/turf/T,var/mob/user,var/deconstruct)
	...
	if(build_delay && !do_after(user, build_delay - build_delay * skill_time_reduction("construction", 0.1, user)))
		working = 0
		return 0
...
/obj/item/rfd/transformer/afterattack(atom/A, mob/user as mob, proximity)
	...
	if(do_after(user, 30 SECONDS - 30 SECONDS * skill_time_reduction("construction", 0.05, user), act_target = src))
