...
/obj/item/weapon/melee/baton/attack(mob/M, mob/user, var/hit_zone)
	...
	msg_admin_attack(text="[key_name(user)] stunned [key_name(L)] with the [src] (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[user.x];Y=[user.y];Z=[user.z]'>JMP</a>)",ckey=key_name(user),ckey_target=key_name(L))
