...
/obj/item/book/manual/fusion_reactor/Initialize()
	...
	dat = {"<html>
				...
				<br>
				<li>The INDRA Mk VIII Fusion Reactor is designed to operate at reactor temperatures up to ten million K, and generate up to 12MW of power.</li>
				<br>
				<h2>HARDWARE CONFIGURATION</h2>
				<li>In the event that any machinery comprising the INDRA Mk VIII Fusion Reactor is rendered inoperable, circuit boards for each part can be fabricated and installed in new machine frames alongside requisite components (detailed with each board). On construction, these replacement parts can then be linked to the primary reactor network using a multitool.</li>
				<h2>STARTUP PROCEDURE</h2>
				<ol>
				...
				<br>
				<li>Ensure that you have performed all steps in EMERGENCY STABILIZATION PROCEDURE. In the event that the reactor is rapidly approaching 100% instability in spite of all preventative measures, a rupture event is highly likely. In this event:</li>
				<ol>
