...
/obj/item/taperoll/attack_self(mob/user as mob)
	if(icon_state == "[icon_base]_start")
		start = get_turf(src)
		usr << span("You place the first end of the [src].", "notice")
		icon_state = "[icon_base]_stop"
	else
		icon_state = "[icon_base]_start"
		end = get_turf(src)
		if(start.y != end.y && start.x != end.x || start.z != end.z)
			usr << span("[src] can only be laid horizontally or vertically.", "notice")
			return
		...
		if (!can_place)
			usr << span("You can't run \the [src] through that!", "notice")
			return
		...
		while (cur!=end)
			...
			cur = get_step_towards(cur,end)
		usr << span("You finish placing the [src].", "notice")	//Git Test
...
/obj/item/taperoll/afterattack(var/atom/A, mob/user as mob, proximity)
	...
	if (istype(A, /obj/machinery/door/airlock))
		...
		P.layer = 3.2
		user << span("You finish placing the [src].", "notice")
...
/obj/item/tape/attack_hand(mob/user as mob)
	if (user.a_intent == I_HELP && src.allowed(user))
		user.show_viewers(span("[user] lifts [src], allowing passage.", "notice"))
		crumple()
		lifted = 1
		spawn(200)
			lifted = 0
...
/obj/item/tape/proc/breaktape(obj/item/weapon/W as obj, mob/user as mob)
	if(user.a_intent == I_HELP && ((!can_puncture(W) && src.allowed(user))))
		...
		return
	user.show_viewers(span("[user] breaks the [src]!", "notice"))
