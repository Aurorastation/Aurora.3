...
/obj/item/razor/attack(mob/M, mob/user, var/target_zone)
	...
	if(target_zone == BP_HEAD)
		...
		if(H == user) //shaving yourself
			user.visible_message(SPAN_WARNING("\The [user] starts to shave \his facial hair with \the [src]."), \
									 SPAN_NOTICE("You take a moment to shave your facial hair with \the [src]."))
			if(do_mob(user, user, 20))
				user.visible_message(SPAN_WARNING("\The [user] shaves \his facial hair clean with \the [src]."), \
										 SPAN_NOTICE("You finish shaving with \the [src]."))
				shave(H, target_zone)
