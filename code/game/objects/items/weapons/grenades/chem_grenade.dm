...
//				detonator.loc=src.loc
		...
		if(stage > 1 && !active && clown_check(user))
			...
			msg_admin_attack(text="[user.name] ([user.ckey]) primed \a [src]. (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[user.x];Y=[user.y];Z=[user.z]'>JMP</a>)",ckey=key_name(user))
