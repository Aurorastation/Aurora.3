...
/obj/item/implant/integrated_circuit/attackby(var/obj/item/O, var/mob/user)
	if(istype(O, /obj/item/crowbar) || istype(O, /obj/item/device/integrated_electronics) || istype(O, /obj/item/integrated_circuit) || istype(O, /obj/item/screwdriver) || istype(O, /obj/item/cell/device) )
		IC.attackby(O, user)
	else
		..()
