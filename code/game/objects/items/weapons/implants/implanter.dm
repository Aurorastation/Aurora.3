...
/obj/item/implanter/ipc_tag/attack(mob/living/target_mob, mob/living/user, target_zone)
	...
	target_mob.log_message("Implanted with [name] ([ipc_tag.name]) by [user.name] ([user.ckey])", LOG_VICTIM, log_globally = FALSE)
	user.log_message("Used the [name] ([ipc_tag.name]) to implant [target_mob.name] ([target_mob.ckey])", LOG_ATTACK, log_globally = FALSE)
