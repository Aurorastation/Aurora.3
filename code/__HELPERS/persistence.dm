 ...
 */
#define SAVE_IF_DIFFERENT(content, var_to_save) if(var_to_save != initial(var_to_save)) {content["[var_to_save]"] = var_to_save}
 ...
 * Input 2 can be any variable that is not a datum or reference.
 */
#define SET_IF_EXISTS(content, var_to_set) if(content["[var_to_set]"] != null) {var_to_set = content["[var_to_set]"]}
