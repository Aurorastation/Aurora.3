...
/client/proc/browse_files(root_type=BROWSE_ROOT_ALL_LOGS, max_iterations=10, list/valid_extensions=list("txt", "log", "htm", "html", "json", "gz"))
	var/root = "data/logs/"
	switch(root_type)
		if(BROWSE_ROOT_ALL_LOGS)
			root = "data/logs/"
		if(BROWSE_ROOT_CURRENT_LOGS)
			root = "data/logs/[GLOB.round_id]/"
	var/path = root
	...
	for(var/i in 1 to max_iterations)
		...
		switch(choice)
			...
			if("Download Folder")
				...
				var/confirmation = tgui_alert(src, "Are you SURE you want to download all the files in this folder? (This will open [length(comp_flist)] prompt[length(comp_flist) == 1 ? "" : "s"])", "Confirmation", list("Yes", "No"), autofocus = TRUE)
				if(confirmation != "Yes")
					continue
				for(var/file as anything in comp_flist)
					DIRECT_OUTPUT(src, ftp(path + file))
				return
		path += choice
...
/client/proc/file_spam_check()
	...
	if(time_to_wait > 0)
		to_chat(src, SPAN_WARNING("Error: file_spam_check(): Spam. Please wait [DisplayTimeText(time_to_wait)]."))
		return TRUE
	var/delay = FTPDELAY
	if(holder)
