...
// when we render multiz, we do it by placing all atoms on lower levels on
// lower planes. This is done with stacks of plane masters. These macros exist
// to facilitate working with this system, and other associated small bits.
...
/// And the base case is important. Non-multiz should not get hit too badly by this code.
...
/// Takes a z reference that we are unsure of and sanity checks it.
/// Returns either its offset, or 0 if it is not a valid ref.
/// Will return the reference's PLANE'S offset if we cannot get anything out of the z level. We do our best.
...
/// Takes a z value and returns its offset with a list lookup.
/// Will runtime during parts of init. Be careful.
#define GET_Z_PLANE_OFFSET(z) (SSmapping.z_level_to_plane_offset[z])

/// Takes a plane to offset, and the multiplier to use, and offsets it.
/// Respects a blacklist used to remove redundant plane masters, such as HUD objects.
...
/// Implicit plane set. We take the turf from the object we are changing the plane of, and use its z as a spokesperson for our plane value.
...
// We do also try and guess at what the thing's z level is, even if it is not a z.
// The plane is cached to allow fancy expressions to be evaluated once.
