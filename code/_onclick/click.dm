...
/mob/proc/ClickOn(atom/A, params)
	...
	// An open, adjacent storage HUD allows interaction with its direct contents.
	var/open_storage_access = FALSE
	if(s_active && (A == s_active || A.loc == s_active))
		if(!s_active.Adjacent(src))
			s_active.close(src)
			return
		open_storage_access = TRUE
	if(open_storage_access || (!isturf(A) && A == loc) || (sdepth != -1 && sdepth <= 1))
		if(W)
