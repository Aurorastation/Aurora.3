...
/obj/screen/movable/MouseDrop(over_object, src_location, over_location, src_control, over_control, params)
	...
	screen_loc_X[1] = encode_screen_X(text2num(screen_loc_X[1]), usr.client)
	//Split Y+Pixel_Y up into list(Y, Pixel_Y)
	var/list/screen_loc_Y = text2list(screen_loc_params[2],":")
	screen_loc_Y[1] = encode_screen_X(text2num(screen_loc_X[1]), usr.client)
...
/obj/screen/movable/proc/encode_screen_X(X, client/C)
	if(X > C.view+1)
		. = "EAST-[C.view*2 + 1-X]"
	else if(X < C.view+1)
		. = "WEST+[X-1]"
...
/obj/screen/movable/proc/decode_screen_X(X, client/C)
...
/obj/screen/movable/proc/encode_screen_Y(Y, client/C)
	if(Y > C.view+1)
		. = "NORTH-[C.view*2 + 1-Y]"
	else if(Y < C.view+1)
		. = "SOUTH+[Y-1]"
...
/obj/screen/movable/proc/decode_screen_Y(Y, client/C)
	if(findtext(Y,"NORTH-"))
