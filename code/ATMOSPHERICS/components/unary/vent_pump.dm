...
/obj/machinery/atmospherics/unary/vent_pump/attackby(obj/item/W, mob/user)
	if(W.iswelder())
		...
		if (!WT.welding)
			...
			if(do_after(user, 30/W.toolspeed - 30/W.toolspeed * (skill_time_reduction("atmos", 0.05, user) * skill_time_reduction("construction", 0.1, user))))
				if(!src || !WT.isOn()) 
					return
				welded = !welded
				update_icon()
...
/obj/machinery/atmospherics/unary/vent_pump/attackby(var/obj/item/W as obj, var/mob/user as mob)
	...
	if (do_after(user, 40/W.toolspeed - 40/W.toolspeed * (skill_time_reduction("atmos", 0.05, user) * skill_time_reduction("construction", 0.1, user))))
