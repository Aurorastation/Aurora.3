...
/turf/simulated/glass/proc/dismantle(var/newloc = src)
	if(reinf)
		new /obj/item/stack/material/glass/reinforced(newloc, 4)
	else
		new /obj/item/stack/material/glass(newloc, 4)
	ChangeToOpenturf()

/turf/simulated/glass/proc/take_damage(var/force)
	playsound(src, 'sound/effects/glass_hit.ogg', 100, 1)
	health -= force
	return check_health()
...
/turf/simulated/glass/attackby(obj/item/I, mob/user)
	...
	if(I.iswelder())
		...
		take_damage(I.force * (secured ? 1 : 2)) // the screws make it absorb a bit of force
		if(!take_damage(I.force * (secured ? 1 : 2))) // take double damage if unsecured due to being loose; call parent if not broken yet
			. = ..()
