...
/obj/item/ladder_mobile/proc/handle_action(atom/A, mob/user)
	if (!do_after(user, 30 - 30 * (skill_time_reduction("prospecting", 0.1, user) + skill_time_reduction("construction", 0.1, user)), act_target = user))
...
/obj/structure/ladder/mobile/verb/fold()
	...
	if (!do_after(H, 30 - 30 * (skill_time_reduction("prospecting", 0.1, H) + skill_time_reduction("construction", 0.1, H)), act_target = src))
