...
/atom/proc/add_fibers(mob/living/carbon/human/M)
	...
	var/fibertext
	var/spawn_multipliertiplier = istype(src,/obj/item)?1.2:1
	var/suit_coverage = 0
	if(M.wear_suit)
		fibertext = "Material from \a [initial(M.wear_suit.name)]."
		if(prob(10*spawn_multipliertiplier) && !(fibertext in suit_fibers))
	...
	if(M.w_uniform && (M.w_uniform.body_parts_covered & ~suit_coverage))
		...
		if(prob(15*spawn_multipliertiplier) && !(fibertext in suit_fibers))
			suit_fibers += fibertext

	if(M.gloves && (M.gloves.body_parts_covered & ~suit_coverage))
		fibertext = "Material from a pair of [initial(M.gloves.name)]."
		if(prob(20*spawn_multipliertiplier) && !(fibertext in suit_fibers))
			suit_fibers += fibertext
