/*
	...
	This Maze Generator was made by AffectedArc07, and was ported to Aurorastation from Paradise by me
	- Geeves
...
/obj/effect/mazegen/generator/proc/generate_path()
	...
	for(var/target_x in 0 to (width - 1)) // -1 so it spawns on the right tile
		for(var/target_y in 0 to (height - 1))
			...
			// Windows time
			var/obj/structure/window/reinforced/crescent/WN = new(T)
			WN.dir = NORTH
			var/obj/structure/window/reinforced/crescent/WS = new(T)
			WS.dir = SOUTH
			var/obj/structure/window/reinforced/crescent/WE = new(T)
			WE.dir = EAST
			var/obj/structure/window/reinforced/crescent/WW = new(T)
			WW.dir = WEST
			CHECK_TICK
		CHECK_TICK
	...
	while(length(working_stack))
		...
		if(length(unvisited_neighbours))
			...
			for(var/obj/structure/window/reinforced/crescent/W in T)
				if(W.dir == text2num(D))
					qdel(W)
