...
/obj/item/clothing/under/proc/set_sensors(mob/usr as mob)
	...
	if (src.loc == usr)
		...
		switch(sensor_mode)
			if(0)
				usr.visible_message(SPAN_WARNING("[usr] disables [src.loc]'s remote sensing equipment."), range = 1)
			if(1)
				usr.visible_message("[usr] turns [src.loc]'s remote sensors to binary.", range = 1)
			if(2)
				usr.visible_message("[usr] sets [src.loc]'s remote sensors to track vitals.", range = 1)
			if(3)
				usr.visible_message("[usr] sets [src.loc]'s remote sensors to maximum.", range = 1)
