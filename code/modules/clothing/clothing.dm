...
/obj/item/clothing/mask/proc/adjust_mask(mob/user, var/self = TRUE)
	...
	if(hanging)
		...
		if(self)
			user.visible_message(SPAN_NOTICE("[user] pulls \the [src] down to hang around \his neck."), SPAN_NOTICE("You pull \the [src] down to hang around your neck."))
	else
		...
		if(self)
			user.visible_message(SPAN_NOTICE("[user] pulls \the [src] up to cover \his face."), SPAN_NOTICE("You pull \the [src] up to cover your face."))
