	...
	if(isipc(A))
		...
		if(!isnull(s))
			if(s.surge_left >= 0.5)
				...
				s.surge_left -= 0.5
				if(s.surge_left)
					H.visible_message(SPAN_WARNING("[H] was not affected by EMP pulse."), SPAN_WARNING("Warning: EMP detected, integrated surge prevention module activated. There are [s.surge_left] preventions left."))
				else
					s.broken = 1
		...
		H.Weaken(5)
		to_chat(H, SPAN_DANGER("ERROR: detected low setting EMP, acutators experience temporary power loss. Attempting to restore power."))
	else if (isrobot(A))
		...
		if(C && C.installed)
			if(C.surge_left >= 0.5)
				playsound(src.loc, 'sound/magic/LightningShock.ogg', 25, 1)
				C.surge_left -= 0.5
				R.visible_message(SPAN_WARNING("[R] was not affected by EMP pulse."), SPAN_WARNING("Warning: Power surge detected, source - EMP. Surge prevention module re-routed surge to prevent damage to vital electronics."))
				if(C.surge_left)
