			...
			if(!isrobot(target))
				...
				if(def_zone == "torso")
					if(blocked < 100 && !(blocked < 20))
						L.emote("yawns")
					if(blocked < 20)
						if(L.reagents)	L.reagents.add_reagent(/datum/reagent/stoxin, 10)
				if(def_zone == BP_HEAD && blocked < 100)
					if(L.reagents)	L.reagents.add_reagent(/datum/reagent/stoxin, 15)
				if(def_zone != "torso" && def_zone != BP_HEAD)
					if(blocked < 100 && !(blocked < 20))
						L.emote("yawns")
					if(blocked < 20)
						if(L.reagents)	L.reagents.add_reagent(/datum/reagent/stoxin, 5)
