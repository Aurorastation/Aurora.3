...
/obj/item/weapon/gun/proc/special_check(var/mob/user)
	...
	if((CLUMSY in M.mutations) && prob(40)) //Clumsy handling
		...
		if(P)
			if(process_projectile(P, user, user, pick(TARGET_L_FOOT, TARGET_R_FOOT)))
...
/obj/item/weapon/gun/attack(atom/A, mob/living/user, def_zone)
	if (A == user && user.zone_sel.selecting == TARGET_MOUTH && !mouthshoot)
		handle_suicide(user)
...
/obj/item/weapon/gun/proc/handle_suicide(mob/living/user)
	...
	if (istype(in_chamber))
		...
		if (in_chamber.damage == 0)
			...
			user.apply_damage(in_chamber.damage*2.5, in_chamber.damage_type, TARGET_HEAD, used_weapon = "Point blank shot in the mouth with \a [in_chamber]", sharp=1)
			user.death()
		qdel(in_chamber)
		mouthshoot = 0
		return
