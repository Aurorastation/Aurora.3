...
/obj/item/weapon/gun/energy/laser/prototype/proc/updatetype()
	...
	if(modifier.type == MOD_SILENCE)
		silenced = 1
	else if(modifier.type == MOD_NUCLEAR_CHARGE)
		self_recharge = 1
	projectile_type.damage = capacitor.damage + modifier.damage
	projectile_type.armor_penetration = capacitor.armor_penetration + modifier.armor_penetration
