...
/obj/item/gun/energy/wand/teleport/special_check(var/mob/living/user) //todo: think of something else for this
	...
	if(!user.is_wizard())
		if(istype(user, /mob/living/carbon/human))
			var/mob/living/carbon/human/H = user
			var/obj/item/organ/O = H.internal_organs_by_name[pick(H.species.vision_organ || BP_EYES,BP_APPENDIX,BP_KIDNEYS,BP_RKIDNEYS,BP_LIVER, BP_HEART, BP_LUNGS, BP_BRAIN)]
			if(O == null)
