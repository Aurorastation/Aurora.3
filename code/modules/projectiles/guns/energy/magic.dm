...
/obj/item/weapon/gun/energy/staff/chaos/special_check(var/mob/living/user)
	...
	if(!user.is_wizard())
		if(istype(user, /mob/living/carbon/human))
			...
			if(P)
				if(process_projectile(P, user, user, pick(TARGET_HEAD, TARGET_CHEST)))
...
/obj/item/weapon/gun/energy/wand/teleport/special_check(var/mob/living/user) //todo: think of something else for this
	...
	if(!user.is_wizard())
		if(istype(user, /mob/living/carbon/human))
			var/mob/living/carbon/human/H = user
			var/obj/item/organ/O = H.internal_organs_by_name[pick(H.species.vision_organ || TARGET_EYES,"appendix","kidneys","liver", "heart", "lungs", "brain")]
			if(O == null)
...
/obj/item/weapon/gun/energy/wand/force/special_check(var/mob/living/user)
	...
	if(!user.is_wizard())
		if(istype(user, /mob/living/carbon/human))
			...
			H.apply_damage(35, BRUTE, TARGET_HEAD, 0, sharp=0, edge=0)
