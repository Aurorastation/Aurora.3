...
/obj/item/gun/projectile/proc/process_chambered()
	...
	switch(handle_casings)
		...
		if(EJECT_CASINGS) //eject casing onto ground.
			...
			playsound(chambered, "casing_drop", 50, 1)
	...
	if(istype(A, /obj/item/ammo_magazine))
		...
		switch(AM.mag_type)
			if(MAGAZINE)
				...
				playsound(src.loc, AM.insert_sound, 50, 1)
			if(SPEEDLOADER)
				...
				for(var/obj/item/ammo_casing/C in AM.stored_ammo)
					...
					if(C.caliber == caliber)
						...
						count++
				if(count)
					user.visible_message("[user] reloads [src].", "<span class='notice'>You load [count] round\s into [src] using the [AM].</span>")
					playsound(src.loc, AM.insert_sound, 50, 1)
		AM.update_icon()
		...
		playsound(src.loc, C.reload_sound, 50, 1)
	update_maptext()
	update_icon()
...
/obj/item/gun/projectile/proc/unload_ammo(mob/user, var/allow_dump = 1)
	if(ammo_magazine)
		...
		playsound(src.loc, ammo_magazine.eject_sound, 50, 1)
...
/obj/item/gun/projectile/attack_self(mob/user)
	if(jam_num)
		...
		if(!jam_num)
			...
			playsound(src.loc, 'sound/weapons/unjam.ogg', 100, 1)
...
/obj/item/gun/projectile/afterattack(atom/A, mob/living/user)
	...
	if(auto_eject && ammo_magazine && ammo_magazine.stored_ammo && !ammo_magazine.stored_ammo.len)
		...
		playsound(user, ammo_magazine.eject_sound, 40, 1)
		ammo_magazine.update_icon()
		ammo_magazine = null
