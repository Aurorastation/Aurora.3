...
/obj/item/gun/launcher/crossbow/attack_self(mob/living/user as mob)
	if(tension)
		if(bolt)
			user.visible_message("<b>[user]</b> relaxes the tension on [src]'s string and removes [bolt].","You relax the tension on [src]'s string and remove [bolt].")
...
/obj/item/gun/launcher/crossbow/proc/draw(var/mob/user as mob)
	...
	while(bolt && tension && loc == current_user)
		if(!do_after(user, draw_time)) //crossbow strings don't just magically pull back on their own.
			user.visible_message("<b>[usr]</b> stops drawing and relaxes the string of [src].",SPAN_WARNING("You stop drawing back and relax the string of [src]."))
			tension = 0
			update_icon()
			return
		...
		user.visible_message("<b>[usr]</b> draws back the string of [src]!",SPAN_NOTICE("You continue drawing back the string of [src]!"))
...
/obj/item/gun/launcher/crossbow/attackby(obj/item/W as obj, mob/user as mob)
	if(!bolt)
		if (istype(W,/obj/item/arrow))
			...
			bolt = W
			user.visible_message("<b>[user]</b> slides [bolt] into [src].","You slide [bolt] into [src].")
			update_icon()
			return
			...
			if (R.use(1))
				...
				bolt.forceMove(src)
				update_icon()
				user.visible_message("<b>[user]</b> jams [bolt] into [src].","You jam [bolt] into [src].")
				superheat_rod(user)
			return
...
/obj/item/gun/launcher/crossbow/proc/superheat_rod(var/mob/user)
	...
	to_chat(user, SPAN_WARNING("[bolt] plinks and crackles as it begins to glow red-hot."))
	bolt.throwforce = 15
...
/obj/item/gun/launcher/crossbow/RFD/attackby(obj/item/W as obj, mob/user as mob)
	...
	if(istype(W, /obj/item/arrow/RFD))
		...
		qdel(A)
		playsound(src.loc, 'sound/machines/click.ogg', 50, 1)
		to_chat(user, SPAN_NOTICE("Flashforged bolt reclaimed. The RXD now holds [stored_matter]/[max_stored_matter] matter-units."))
		update_icon()
		return
