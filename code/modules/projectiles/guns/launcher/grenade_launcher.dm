...
/obj/item/gun/launcher/grenade/proc/load(obj/item/grenade/G, mob/user)
	...
	user.visible_message("<b>[user]</b> inserts \a [G] into [src].", SPAN_NOTICE("You insert \a [G] into [src]."))
	update_maptext()
...
/obj/item/gun/launcher/grenade/proc/unload(mob/user)
	if(grenades.len)
		...
		grenades.len--
		user.put_in_hands(G)
		user.visible_message("<b>[user]</b> removes \a [G] from [src].", SPAN_NOTICE("You remove \a [G] from [src]."))
	else
...
/obj/item/gun/launcher/grenade/underslung/unload(mob/user)
	if(chambered)
		...
		user.visible_message("<b>[user]</b> removes \a [chambered] from [src].", SPAN_NOTICE("You remove \a [chambered] from [src]."))
		chambered = null
	else
		to_chat(user, SPAN_WARNING("[src] is empty."))
