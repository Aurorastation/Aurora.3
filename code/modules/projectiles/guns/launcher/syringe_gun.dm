...
/obj/item/gun/launcher/syringe/attack_self(mob/living/user as mob)
	if(next)
		user.visible_message("<b>[user]</b> unlatches and carefully relaxes the bolt on [src].", SPAN_WARNING("You unlatch and carefully relax the bolt on [src], unloading the spring."))
		playsound(src.loc, 'sound/weapons/blade_close.ogg', 50, 1)
		next = null
	else if(darts.len)
		playsound(src.loc, 'sound/weapons/blade_open.ogg', 50, 1)
		user.visible_message("<b>[user]</b> draws back the bolt on [src], clicking it into place.", SPAN_WARNING("You draw back the bolt on [src], loading the spring!"))
		next = darts[1]
