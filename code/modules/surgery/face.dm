...
/singleton/surgery_step/generic/cut_face/can_use(mob/living/user, mob/living/carbon/teshari/target, target_zone, obj/item/tool)
	return ..() && target_zone == BP_MOUTH && target.op_stage.face == FACE_NORMAL

/singleton/surgery_step/generic/cut_face/begin_step(mob/user, mob/living/carbon/teshari/target, target_zone, obj/item/tool)
	user.visible_message("[user] starts to cut open [target]'s face and neck with \the [tool].", \
		"You start to cut open [target]'s face and neck with \the [tool].")
	..()

/singleton/surgery_step/generic/cut_face/end_step(mob/living/user, mob/living/carbon/teshari/target, target_zone, obj/item/tool)
...
/singleton/surgery_step/generic/cut_face/fail_step(mob/living/user, mob/living/carbon/teshari/target, target_zone, obj/item/tool)
...
/singleton/surgery_step/robotics/face/synthskin/can_use(mob/living/user, mob/living/carbon/teshari/target, target_zone, obj/item/tool)
	return ..() && target.op_stage.face == FACE_NORMAL && target.get_species() == SPECIES_IPC_SHELL

/singleton/surgery_step/robotics/face/synthskin/begin_step(mob/user, mob/living/carbon/teshari/target, target_zone, obj/item/tool)
...
/singleton/surgery_step/robotics/face/synthskin/end_step(mob/living/user, mob/living/carbon/teshari/target, target_zone, obj/item/tool)
...
/singleton/surgery_step/robotics/face/synthskin/fail_step(mob/living/user, mob/living/carbon/teshari/target, target_zone, obj/item/tool)
	user.visible_message(SPAN_WARNING("[user]'s hand slips, slicing [target]'s throat wth \the [tool]!") , \
		SPAN_WARNING("Your hand slips, slicing [target]'s throat wth \the [tool]!") )
	target.apply_damage(40, DAMAGE_BRUTE, target_zone, 0, tool, damage_flags = tool.damage_flags())