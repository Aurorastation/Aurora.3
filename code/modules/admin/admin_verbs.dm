...
/client/proc/check_antagonists()
	...
	if(holder)
		...
		log_admin(text="[key_name(usr)] checked antagonists.",ckey=key_name(usr))	//for tsar~
...
/client/proc/stealth()
	...
	if(holder)
		...
		log_admin(text="[key_name(usr)] has turned stealth mode [holder.fakekey ? "ON" : "OFF"]", admin_key=key_name(usr),ckey=holder.fakekey)
...
/client/proc/give_disease(mob/T as mob in mob_list) // -- Giacom
	...
	log_admin(text="[key_name(usr)] gave [key_name(T)] the disease [D].",admin_key=key_name(usr),ckey=key_name(T))
...
/client/proc/give_disease2(mob/T as mob in mob_list) // -- Giacom
	...
	log_admin(text="[key_name(usr)] gave [key_name(T)] a [greater] disease2 with infection chance [D.infectionchance].",admin_key=key_name(usr),ckey=key_name(T))
...
/client/proc/make_sound(var/obj/O in range(world.view)) // -- TLE
	...
	if(O)
		...
		log_admin(text="[key_name(usr)] made [O] at [O.x], [O.y], [O.z]. make a sound",admin_key=key_name(usr))
...
/client/proc/kill_air() // -- TLE
	...
	log_admin(text="[key_name(usr)] used 'kill air'.",admin_key=key_name(usr))
...
/client/proc/readmin_self()
	...
	if(deadmin_holder)
		...
		log_admin(text="[src] re-admined themself.",admin_key=key_name(src))
...
/client/proc/deadmin_self()
	...
	if(holder)
		if(alert("Confirm self-deadmin for the round? You can re-admin yourself at any time.",,"Yes","No") == "Yes")
			log_admin(text="[src] deadmined themself.",admin_key=key_name(src))
...
/client/proc/change_security_level()
	...
	if(alert("Switch from code [get_security_level()] to code [sec_level]?","Change security level?","Yes","No") == "Yes")
		...
		log_admin(text="[key_name(usr)] changed the security level to code [sec_level].",admin_key=key_name(usr))
...
/client/proc/man_up(mob/T as mob in mob_list)
	...
	log_admin(text="[key_name(usr)] told [key_name(T)] to man up and deal with it.", admin_key=key_name(usr), ckey=key_name(T))
...
/client/proc/global_man_up()
	...
	log_admin(text="[key_name(usr)] told everyone to man up and deal with it.",admin_key=key_name(usr))
...
/client/proc/give_spell(mob/T as mob in mob_list) // -- Urist
	...
	log_admin(text="[key_name(usr)] gave [key_name(T)] the spell [S].",admin_key=key_name(usr),ckey=key_name(T))
