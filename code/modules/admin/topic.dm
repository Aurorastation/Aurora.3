/datum/admins/Topic(href, href_list)
	...
	if(href_list["dbsearchckey"] || href_list["dbsearchadmin"])
		...
		switch(href_list["call_shuttle"])
			if("1")
				...
				if (emergency_shuttle.can_call())
					...
					message_admins(SPAN_NOTICE("[key_name_admin(usr)] called the Emergency Shuttle to the station"), 1)
			...
			if("2")
				...
				if (emergency_shuttle.can_call())
					...
					message_admins(SPAN_NOTICE("[key_name_admin(usr)] sent the Emergency Shuttle back"), 1)
		...
		if (emergency_shuttle.wait_for_launch)
			...
			message_admins(SPAN_NOTICE("[key_name_admin(usr)] edited the Emergency Shuttle's launch time to [new_time_left*10]"), 1)
			...
			message_admins(SPAN_NOTICE("[key_name_admin(usr)] edited the Emergency Shuttle's arrival time to [new_time_left*10]"), 1)
		else
	...
	// Are now moved to banjob.dm. For use with the newer system.
		...
		if (ismob(M))
			...
			if(!reason)
				to_chat(M, SPAN_DANGER("You have been kicked from the server"))
			else
				to_chat(M, SPAN_DANGER("You have been kicked from the server: [reason]"))
		...
		switch(alert("Temporary Ban?",,"Yes","No", "Cancel"))
			if("Yes")
				...
				to_chat(M, SPAN_DANGER("This is a temporary ban, it will be removed in [mins] minutes."))
