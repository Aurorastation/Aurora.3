/datum/admins/Topic(href, href_list)
	...
	if(usr.client != src.owner || !check_rights(0))
		log_admin(text="[key_name(usr)] tried to use the admin panel without authorization.",admin_key=key_name(usr))
	...
	if(href_list["dbsearchckey"] || href_list["dbsearchadmin"])
		...
		if(!check_rights(R_PERMISSIONS))
			...
			log_admin(text="[key_name(usr)] attempted to edit the admin permissions without sufficient rights.",admin_key=key_name(usr))
			return
		...
		if(task == "remove")
			if(alert("Are you sure you want to remove [adm_ckey]?","Message","Yes","Cancel") == "Yes")
				...
				log_admin(text="[key_name(usr)] removed [adm_ckey] from the admins list",admin_key=key_name(usr))
			...
			log_admin(text="[key_name(usr)] edited the admin rank of [adm_ckey] to [new_rank]",admin_key=key_name(usr))
			...
			log_admin(text="[key_name(usr)] toggled the [new_permission] permission of [adm_ckey]", admin_key=key_name(usr))
		...
		switch(href_list["call_shuttle"])
			if("1")
				...
				if (emergency_shuttle.can_call())
					...
					log_admin(text="[key_name(usr)] called the Emergency Shuttle",admin_key=key_name(usr))
			...
			if("2")
				...
				if (emergency_shuttle.can_call())
					...
					log_admin(text="[key_name(usr)] sent the Emergency Shuttle back",admin_key=key_name(usr))
		...
		if (emergency_shuttle.wait_for_launch)
			...
			log_admin(text="[key_name(usr)] edited the Emergency Shuttle's launch time to [new_time_left]",admin_key=key_name(usr))
			...
			log_admin(text="[key_name(usr)] edited the Emergency Shuttle's arrival time to [new_time_left]",admin_key=key_name(usr))
		...
		log_admin(text="[key_name(usr)] [ticker.delay_end ? "delayed the round end" : "has made the round end normally"].",admin_key=key_name(usr))
		...
		log_admin(text="[key_name(usr)] has used rudimentary transformation on [key_name(M)]. Transforming to [href_list["simplemake"]]; deletemob=[delmob]",admin_key=key_name(usr))
	...
	/////////////////////////////////////new ban stuff
		...
		log_admin(text="[key_name(usr)] edited [banned_key]'s ban. Reason: [reason] Duration: [duration]",admin_key=key_name(usr))
...
//Regular jobs
	...
	//JOBBAN'S INNARDS
			...
			switch(alert("Temporary Ban?",,"Yes","No", "Cancel"))
				if("Yes")
					...
					for(var/job in notbannedlist)
						...
						log_admin(text="[key_name(usr)] temp-jobbanned [key_name(M)] from [job] for [mins] minutes",admin_key=key_name(usr))
				...
				if("No")
					...
					if(reason)
						...
						for(var/job in notbannedlist)
							...
							log_admin(text="[key_name(usr)] perma-banned [key_name(M)] from [job]",admin_key=key_name(usr))
			...
			for(var/job in joblist)
				...
				switch(alert("Job: '[job]' Reason: '[reason]' Un-jobban?","Please Confirm","Yes","No"))
					if("Yes")
						...
						log_admin(text="[key_name(usr)] unbanned [key_name(M)] from [job]",admin_key=key_name(usr),ckey=key_name(M))
		...
		if (ismob(M))
			...
			log_admin(text="[key_name(usr)] booted [key_name(M)].",admin_key=key_name(usr),ckey=key_name(M))
		...
		if(t)
			if((alert("Do you want to unjobban [t]?","Unjobban confirmation", "Yes", "No") == "Yes") && t) //No more misclicks! Unless you do it twice.
				log_admin(text="[key_name(usr)] removed [t]",admin_key=key_name(usr))
		...
		switch(alert("Temporary Ban?",,"Yes","No", "Cancel"))
			if("Yes")
				...
				log_admin(text="[usr.client.ckey] has banned [M.ckey].\nReason: [reason]\nThis will be removed in [mins] minutes.",admin_key=key_name(usr),ckey=M.ckey)
			...
			if("No")
				...
				log_admin(text="[usr.client.ckey] has banned [M.ckey].\nReason: [reason]\nThis is a permanent ban.",admin_key=key_name(usr),ckey=M.ckey)
		...
		log_admin(text="[key_name(usr)] set the mode as [master_mode].",admin_key=key_name(usr))
		...
		log_admin(text="[key_name(usr)] set the forced secret mode as [secret_force_mode].",admin_key=key_name(usr))
		...
		log_admin(text="[key_name(usr)] attempting to monkeyize [key_name(H)]",admin_key=key_name(usr))
		...
		log_admin(text="[key_name(usr)] attempting to corgize [key_name(H)]",admin_key=key_name(usr),ckey=key_name(H))
		...
		log_admin(text="[key_name(usr)] forced [key_name(M)] to say: [speech]",admin_key=key_name(usr))
		...
		log_admin(text="[key_name(usr)] sent [key_name(M)] to the prison station.",,admin_key=key_name(usr),ckey=key_name(M))
		...
		log_admin(text="[key_name(usr)] has sent [key_name(M)] to the thunderdome. (Team 1)",admin_key=key_name(usr),ckey=key_name(M))
		...
		spawn(50)
			M << "\blue You have been sent to the Thunderdome."
		log_admin(text="[key_name(usr)] has sent [key_name(M)] to the thunderdome. (Team 2)",admin_key=key_name(usr),ckey=key_name(M))
		...
		spawn(50)
			M << "\blue You have been sent to the Thunderdome."
		log_admin(text="[key_name(usr)] has sent [key_name(M)] to the thunderdome. (Admin.)",admin_key=key_name(usr),ckey=key_name(M))
		...
		spawn(50)
			M << "\blue You have been sent to the Thunderdome."
		log_admin(text="[key_name(usr)] has sent [key_name(M)] to the thunderdome. (Observer.)",admin_key=key_name(usr),ckey=key_name(M))
		...
		if(config.allow_admin_rev)
			...
			log_admin(text="[key_name(usr)] healed / Revived [key_name(L)]",admin_key=key_name(usr),ckey=key_name(L))
		else
		...
		log_admin(text="[key_name(usr)] AIized [key_name(H)]",admin_key=key_name(usr),ckey=key_name(H))
		H.AIize()
		...
		if(!(istype(H.l_hand,/obj/item/weapon/reagent_containers/food/snacks/cookie)))
			...
			if(!(istype(H.r_hand,/obj/item/weapon/reagent_containers/food/snacks/cookie)))
				log_admin(text="[key_name(H)] has their hands full, so they did not receive their cookie, spawned by [key_name(src.owner)].",admin_key=key_name(src.owner),ckey=key_name(H))
			...
			H.update_inv_l_hand()
		log_admin(text="[key_name(H)] got their cookie, spawned by [key_name(src.owner)]",admin_key=key_name(src.owner),ckey=key_name(H))
		...
		log_admin(text="[key_name(M)] has been hit by Bluespace Artillery fired by [src.owner]",admin_key=src.owner,ckey=key_name(M))
		...
		if(L.can_centcom_reply())
			...
			log_admin(text="[src.owner] replied to [key_name(L)]'s Centcomm message with the message [input].",admin_key=src.owner,ckey=key_name(L))
		...
		log_admin(text="[src.owner] replied to [key_name(H)]'s illegal message with the message [input].", admin_key=src.owner, ckey=key_name(H))
		...
		*/
		...
		if(src.admincaster_feed_channel.channel_name == "" || src.admincaster_feed_channel.channel_name == "\[REDACTED\]" || check )
			...
			if(choice=="Confirm")
				...
				log_admin(text="[key_name_admin(usr)] created command feed channel: [src.admincaster_feed_channel.channel_name]!",admin_key=key_name(usr))
		...
		log_admin(text="[key_name_admin(usr)] submitted a feed story to channel: [src.admincaster_feed_channel.channel_name]!",admin_key=key_name(usr))
		src.access_news_network()
		...
		if(src.admincaster_feed_message.author == "" || src.admincaster_feed_message.body == "")
			...
			if(choice=="Confirm")
				...
				log_admin(text="[key_name_admin(usr)] issued a Station-wide Wanted Notification for [src.admincaster_feed_message.author]!",admin_key=key_name(usr))
		src.access_news_network()
