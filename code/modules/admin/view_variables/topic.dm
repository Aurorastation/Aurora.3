...
/client/proc/view_var_Topic(href, href_list, hsrc)
	...
	//~CARN: for renaming mobs (updates their name, real_name, mind.name, their ID/PDA and datacore records).
		...
		switch(action_type)
			if("Strict type")
				...
				if(!i)
					...
					return
				log_admin(text="[key_name(usr)] deleted all objects of type [O_type] ([i] objects deleted)", admin_key=key_name(usr))
			...
			if("Type and subtypes")
				...
				if(!i)
					usr << "No objects of this type exist"
					return
				log_admin(text="[key_name(usr)] deleted all objects of type or subtype of [O_type] ([i] objects deleted)", admin_key=key_name(usr))
		...
		log_admin(text="[key_name(usr)] resent the NanoUI resource files to [key_name(H)] ", admin_key=key_name(usr), ckey=key_name(H))
		...
		if(amount != 0)
			log_admin(text="[key_name(usr)] dealt [amount] amount of [Text] damage to [L]", admin_key=key_name(usr), ckey=key_name(L))
