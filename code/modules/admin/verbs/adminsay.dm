...
/client/proc/cmd_mod_say(msg as text)
	...
	for(var/s in staff)
		...
		if ((R_ADMIN|R_MOD) & C.holder.rights)
			to_chat(C, "<span class='mod_channel'>" + create_text_tag("mod", "MOD:", C) + " <span class='name'>[sender_name]</span>(<A HREF='?src=\ref[C.holder];adminplayerobservejump=\ref[mob]'>JMP</A>): <span class='message linkify'>[msg]</span></span>")
...
/client/proc/cmd_dev_say(msg as text)
	...
	if(check_rights(R_DEV,0))
		msg = "<span class='devsay'><span class='prefix'>DEV:</span> <EM>[key_name(usr, 0, 1, 0)]</EM>: <span class='message linkify'>[msg]</span></span>"
		for(var/s in staff)
			var/client/C = s
