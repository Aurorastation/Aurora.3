	...
	if(check_rights(R_ADMIN,0))
		for(var/s in staff)
			...
			if(R_ADMIN & C.holder.rights)
				to_chat(C, span("admin_channel", "" + create_text_tag("admin", "ADMIN:", C) + " <span class='name'>[key_name(usr, 1)]</span>([admin_jump_link(mob, src)]): <span class='message'>[msg]</span>"))
...
/client/proc/cmd_mod_say(msg as text)
	...
	for(var/s in staff)
		...
		if ((R_ADMIN|R_MOD) & C.holder.rights)
			to_chat(C, span("mod_channel", "" + create_text_tag("mod", "MOD:", C) + " <span class='name'>[sender_name]</span>(<A HREF='?src=\ref[C.holder];adminplayerobservejump=\ref[mob]'>JMP</A>): <span class='message'>[msg]</span>"))
...
/client/proc/cmd_dev_say(msg as text)
	...
	if(check_rights(R_DEV,0))
		msg = span("devsay", "<span class='prefix'>DEV:</span> <EM>[key_name(usr, 0, 1, 0)]</EM>: <span class='message'>[msg]</span>")
		for(var/s in staff)
			var/client/C = s
...
/client/proc/cmd_cciaa_say(msg as text)
	...
	if(check_rights((R_CCIAA|R_ADMIN),0))
		msg = span("cciaasay", "<span class='prefix'>CCIAAgent:</span> <EM>[key_name(usr, 0, 1, 0)]</EM>: <span class='message'>[msg]</span>")
		for(var/s in staff)
			var/client/C = s
