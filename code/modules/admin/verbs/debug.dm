/client/proc/Debug2()
	...
	if(Debug2)
		...
		log_admin(text="[key_name(src)] toggled debugging off.",admin_key=key_name(usr))
	else
		Debug2 = 1
		message_admins("[key_name(src)] toggled debugging on.")
		log_admin(text="[key_name(src)] toggled debugging on.",admin_key=key_name(usr))
...
/client/proc/cmd_admin_robotize(var/mob/M in mob_list)
	...
	if(istype(M, /mob/living/carbon/human))
		log_admin(text="[key_name(src)] has robotized [M.key].",admin_key=key_name(usr),ckey=key_name(M))
		spawn(10)
			M:Robotize()
...
/client/proc/cmd_admin_animalize(var/mob/M in mob_list)
	...
	log_admin(text="[key_name(src)] has animalized [M.key].",admin_key=key_name(usr),ckey=key_name(M))
	spawn(10)
		M.Animalize()
...
/client/proc/cmd_admin_alienize(var/mob/M in mob_list)
	...
	if(!ticker)
		...
		return
	if(ishuman(M))
		log_admin(text="[key_name(src)] has alienized [M.key].",admin_key=key_name(usr),ckey=key_name(M))
		spawn(10)
			M:Alienize()
			feedback_add_details("admin_verb","MKAL") //If you are copy-pasting this, ensure the 2nd parameter is unique to the new proc!
		log_admin(text="[key_name(usr)] made [key_name(M)] into an alien.",admin_key=key_name(usr),ckey=key_name(M))
...
/client/proc/cmd_admin_slimeize(var/mob/M in mob_list)
	...
	if(!ticker)
		...
		return
	if(ishuman(M))
		log_admin(text="[key_name(src)] has slimeized [M.key].",admin_key=key_name(usr))
		spawn(10)
			M:slimeize()
			feedback_add_details("admin_verb","MKMET") //If you are copy-pasting this, ensure the 2nd parameter is unique to the new proc!
		log_admin(text="[key_name(usr)] made [key_name(M)] into a slime.",admin_key=key_name(usr),ckey=key_name(M))
...
/client/proc/cmd_debug_del_all()
	...
	if(hsbitem)
		for(var/atom/O in world)
			if(istype(O, hsbitem))
				qdel(O)
		log_admin(text="[key_name(src)] has deleted all instances of [hsbitem].",admin_key=key_name(usr))
...
/client/proc/cmd_debug_make_powernets()
	...
	makepowernets()
	log_admin(text="[key_name(src)] has remade the powernet. makepowernets() called.",admin_key=key_name(usr))
...
/client/proc/cmd_debug_tog_aliens()
	...
	log_admin(text="[key_name(src)] has turned aliens [config.aliens_allowed ? "on" : "off"].",admin_key=key_name(usr))
...
/client/proc/cmd_admin_grantfullaccess(var/mob/M in mob_list)
	...
	log_admin(text="[key_name(src)] has granted [M.key] full access.",admin_key=key_name(usr),ckey=key_name(M))
...
/client/proc/cmd_assume_direct_control(var/mob/M in mob_list)
	...
	log_admin(text="[key_name(usr)] assumed direct control of [M].",admin_key=key_name(usr))
...
/client/proc/cmd_admin_dress()
	...
	log_admin(text="[key_name(usr)] changed the equipment of [key_name(M)] to [dresscode].",admin_key=key_name(usr),ckey=key_name(M))
...
/client/proc/cmd_admin_toggle_block(var/mob/M,var/block)
	...
	if(istype(M, /mob/living/carbon))
		...
		log_admin(text="[key_name(src)] has toggled [M.key]'s [blockname] block [state]!",admin_key=key_name(src),ckey=key_name(M))
	else
		alert("Invalid mob")
