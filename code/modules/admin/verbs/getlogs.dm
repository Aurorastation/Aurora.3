...
/client/proc/browseserverlogs(current=FALSE)
	...
	switch(tgui_alert(src, "View (in game), Open (in your system's text editor), or Download?", path, list("View", "Open", "Download")))
		if ("View")
			DIRECT_OUTPUT(src, browse("<pre style='word-wrap: break-word;'>[html_encode(file2text(file(path)))]</pre>", list2params(list("window" = "viewfile.[path]"))))
		if ("Open")
			DIRECT_OUTPUT(src, run(file(path)))
		if ("Download")
			DIRECT_OUTPUT(src, ftp(file(path)))
		else
			return
