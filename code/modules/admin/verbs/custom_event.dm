...
/client/proc/cmd_admin_change_custom_event()
	...
	if(!input || input == "")
		...
		log_admin(text="[usr.key] has cleared the custom event text.",admin_key=key_name(usr))
		message_admins("[key_name_admin(usr)] has cleared the custom event text.")
		return

	log_admin(text="[usr.key] has changed the custom event text.",admin_key=key_name(usr))
