/proc/possess(obj/O as obj in range(world.view))
	...
	if(T)
		log_admin(text="[key_name(usr)] has possessed [O] ([O.type]) at ([T.x], [T.y], [T.z])",admin_key=key_name(src))
		message_admins("[key_name(usr)] has possessed [O] ([O.type]) at ([T.x], [T.y], [T.z])", 1)
	else
		log_admin(text="[key_name(usr)] has possessed [O] ([O.type]) at an unknown location",admin_key=key_name(src))
