...
/client/proc/mod_list_add(var/list/L, atom/O, original_name, objectvar)
	...
	log_admin(text="[key_name(src)] modified [original_name]'s [objectvar]: ADDED=[var_value]",admin_key=key_name(src))
...
/client/proc/mod_list(var/list/L, atom/O, original_name, objectvar)
	...
	switch(class) //Spits a runtime error if you try to modify an entry in the contents list. Dunno how to fix it, yet.
		...
		if("DELETE FROM LIST")
			...
			log_admin(text="[key_name(src)] modified [original_name]'s [objectvar]: REMOVED=[variable]",admin_key=key_name(src))
	...
	log_admin(text="[key_name(src)] modified [original_name]'s [objectvar]: [original_var]=[new_var]",admin_key=key_name(src))
...
/client/proc/modify_variables(var/atom/O, var/param_var_name = null, var/autodetect_class = 0)
	...
	log_admin(text="[key_name(src)] modified [original_name]'s [variable] to [O.vars[variable]]",admin_key=key_name(src))
