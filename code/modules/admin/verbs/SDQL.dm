...
/client/proc/SDQL_query(query_text as message)
	...
	if(!check_rights(R_DEBUG))  //Shouldn't happen... but just to be safe.
		message_admins(SPAN_WARNING("ERROR: Non-admin [usr.key] attempted to execute a SDQL query!"))
...
/proc/SDQL_tokenize(query_text)
	...
	for(var/i = 1, i <= len, i++)
		...
		if(char in whitespace)
			...
			if(i > len)
				to_chat(usr, SPAN_DANGER("SDQL: You have an error in your SDQL syntax, unmatched \" in query: \"<font color=gray>[query_text]</font>\". Please check your syntax, and try again."))
				return null
