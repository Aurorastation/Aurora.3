...
/obj/item/organ/internal/heart/proc/handle_blood()
	...
	if(pulse != PULSE_NONE || BP_IS_ROBOTIC(src))
		...
		if(world.time >= next_blood_squirt && istype(owner.loc, /turf) && do_spray.len)
			owner.visible_message(SPAN_DANGER("Blood squirts from \the [owner]'s [pick(do_spray)]!"), SPAN_DANGER("<font size=3>Blood is squirting out of your [pick(do_spray)]!</font>"))
			owner.eye_blurry = 2
			owner.Stun(1)
