...
/obj/item/organ/internal/augment/timepiece/attack_self(var/mob/user)
	...
	to_chat(
		owner,
		SPAN_NOTICE(
			"Hello [user], it is currently: '[worldtime2text()]'. " \
				+ "Today's date is '[time2text(world.time, "Month DD")]. " \
				+ "[GLOB.game_year]'. Have a lovely day.")
			)
	if(evacuation_controller.get_status_panel_eta())
		to_chat(
			owner,
			SPAN_WARNING(
				"Notice: You have one (1) scheduled flight, ETA: [evacuation_controller.get_status_panel_eta()].")
			)
