...
/obj/item/organ/vaurca/preserve/attackby(obj/item/W as obj, mob/user as mob)
	...
	if ((istype(W, /obj/item/device/analyzer)) && get_dist(user, src) <= 1)
		user.visible_message("<span class='warning'>[user] has used [W] on [icon2html(icon, usr)] [src]</span>")

		var/pressure = air_contents.return_pressure()
		manipulated_by = user.real_name			//This person is aware of the contents of the tank.
		var/total_moles = air_contents.total_moles

		to_chat(user, "<span class='notice'>Results of analysis of [icon2html(icon, usr)]</span>")
		if (total_moles>0)
