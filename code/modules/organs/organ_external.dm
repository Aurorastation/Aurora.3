	...
	switch(disintegrate)
		if(DROPLIMB_EDGE)
			if(!clean)
				...
				owner.visible_message(
					...
					span("moderate", "<b><font size=2>Your [src.name] goes flying off!</font></b>"),\
		...
		if(DROPLIMB_BURN)
			...
			owner.visible_message(
				...
				span("moderate", "<b><font size=2>Your [src.name] flashes away into ashes!</font></b>"),\
		...
		if(DROPLIMB_BLUNT)
			...
			owner.visible_message(
				...
				span("moderate", "<b><font size=3>Your [src.name] explodes[gore]!</font></b>"),\
...
/obj/item/organ/external/proc/embed(var/obj/item/W, var/silent = 0, var/supplied_message)
	...
	if(!silent)
		if(supplied_message)
			...
			owner.visible_message(SPAN_DANGER("\The [W] sticks in [owner]'s wound!"), SPAN_DANGER("\The [W] sticks in your wound!"))
	implants += W
...
/obj/item/organ/external/proc/disfigure(var/type = "brute")
	...
	if(owner)
		if(type == "brute")
			...
			owner.visible_message(SPAN_WARNING("\The [owner]'s [name] melts away, turning into mangled mess!"),	\
