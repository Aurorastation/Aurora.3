...
/datum/category_item/player_setup_item/general/basic/content(var/mob/user)
	...
	if(istype(S, /datum/species/machine))
		dat += "<b>Custom Model:</b> <a href='byond://?src=[REF(src)];custom_model=1'>[pref.machine_custom_model ? "[pref.machine_custom_model]": ""]</a><br>"
...
/datum/category_item/player_setup_item/general/basic/OnTopic(var/href,var/list/href_list, var/mob/user)
	if(href_list["rename"])
		...
		var/custom_model = sanitize(tgui_input_text(user, "Enter a custom model that will display on examine. This is purely flavor.", "IPC Custom Model", max_length = 20))
		if(CanUseTopic(user))
