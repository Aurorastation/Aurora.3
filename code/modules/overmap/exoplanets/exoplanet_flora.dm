...
/obj/effect/overmap/visitable/sector/exoplanet/proc/adapt_seed(var/datum/seed/S)
	...
	SET_SEED_TRAIT_BOUNDED(S, TRAIT_LOWKPA_TOLERANCE, XGM_PRESSURE(exterior_atmosphere) + rand(-5,-50), 80, 0, null)
	SET_SEED_TRAIT_BOUNDED(S, TRAIT_HIGHKPA_TOLERANCE, XGM_PRESSURE(exterior_atmosphere) + rand(5,50), 500, 110, null)
	SET_SEED_TRAIT(S, TRAIT_SPREAD, 0)
	if(S.exude_gasses)
