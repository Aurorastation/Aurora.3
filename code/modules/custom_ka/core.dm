...
/obj/item/gun/custom_ka/Fire(atom/target, mob/living/user, clickparams, pointblank=0, reflex=0)
	...
	for(var/i in 1 to burst)
		...
		acc += burst_accuracy[min(i, burst_accuracy.len)] - burst_accuracy[min(i, burst_accuracy.len)] * (skill_time_reduction("weapons", 0.08, user) + skill_time_reduction("weapons", 0.05, user))
