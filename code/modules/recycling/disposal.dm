...
/obj/structure/machinery/disposal/attackby(obj/item/attacking_item, mob/user)
	...
	if(istype(G))
		if(ismob(G.affecting))
			...
			if(do_after(usr, 20))
				if (GM.client)
					GM.client.set_eye(src, EYE_PERSPECTIVE)
				GM.forceMove(src)
...
/obj/structure/machinery/disposal/mouse_drop_receive(atom/dropped, mob/user, params)
	...
	if(target == user && !user.stat && !user.weakened && !user.stunned && !user.paralysis)
		...
		return
	if (target.client)
		target.client.set_eye(src, EYE_PERSPECTIVE)
...
/obj/structure/machinery/disposal/proc/go_out(mob/user)
	...
	if (user.client)
		user.client.set_eye(user.client.mob, MOB_PERSPECTIVE)
...
/mob/pipe_eject(var/direction)
	if (src.client)
		src.client.set_eye(src, MOB_PERSPECTIVE)
