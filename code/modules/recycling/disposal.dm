...
/obj/machinery/disposal/attackby(var/obj/item/I, var/mob/user)
		...
		if(ismob(G.affecting))
			...
			if(do_after(usr, 20))
				...
				for (var/mob/C in viewers(src))
					C.show_message(SPAN_WARNING("[GM.name] has been placed in [src] by [user]."), 3)
				qdel(G)
	...
	to_chat(user, "You place \the [I] into [src].")
	for(var/mob/M in viewers(src))
		if(M == user)
			continue
		M.show_message("[user.name] places \the [I] into [src].", 3)
