	...
	if(istype(G))	// handle grabbed mob
		if(ismob(G.affecting))
			...
			if(do_after(usr, 20))
				...
				msg_admin_attack(text="[usr] ([usr.ckey]) placed [GM] ([GM.ckey]) in a disposals unit. (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[usr.x];Y=[usr.y];Z=[usr.z]'>JMP</a>)",ckey=key_name(usr),ckey_target=key_name(GM))
		return
	if(!dropsafety(I))
		return
...
/obj/machinery/disposal/MouseDrop_T(mob/target, mob/user)
	...
	if(target == user && !user.stat && !user.weakened && !user.stunned && !user.paralysis)	// if drop self, then climbed in
		...
		msg_admin_attack(text="[user] ([user.ckey]) placed [target] ([target.ckey]) in a disposals unit. (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[user.x];Y=[user.y];Z=[user.z]'>JMP</a>)",ckey=key_name(user),ckey_target=key_name(target))
	else
		return
	if (target.client)
