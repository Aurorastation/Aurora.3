/obj/structure/machinery/computer/fusion/fuel_control
...
/obj/structure/machinery/computer/fusion/fuel_control/terminal
...
/obj/structure/machinery/computer/fusion/fuel_control/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	. = ..()
	if(.)
		return

	var/datum/local_network/lan = get_local_network()
	var/list/fuel_injectors = lan.get_devices(/obj/structure/machinery/fusion_fuel_injector)
	if(!lan || !fuel_injectors)
		return

	switch(action)
		if("global_toggle")
			for(var/obj/structure/machinery/fusion_fuel_injector/F in fuel_injectors)
				if(F.injecting)
					F.StopInjecting()
				else
					F.BeginInjecting()
		...
		if("toggle_injecting")
			var/obj/structure/machinery/fusion_fuel_injector/I = locate(params["machine"])
...
/obj/structure/machinery/computer/fusion/fuel_control/ui_data(mob/user)
	var/list/data = ..()
	...
	if(lan)
		...
		for(var/i = 1 to LAZYLEN(fuel_injectors))
			...
			var/obj/structure/machinery/fusion_fuel_injector/I = fuel_injectors[i]
			injector["id"] = "#[i]"
			injector["ref"] = "[REF(I)]"
