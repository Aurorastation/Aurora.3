					...
					for(var/datum/wound/W in affecting.wounds)
						...
						if(W.current_stage <= W.max_bleeding_stage)
							...
							if(!do_mob(user, M, 200))
								...
								break
							user.visible_message("<b>[user]</b> barely manages to stitch \a [W.desc] on [M]'s [affecting.name].", \
...
/obj/item/stack/cable_coil/proc/repair_organ(var/mob/living/user, var/mob/living/carbon/human/target, var/obj/item/organ/external/affecting)
	if(!affecting.burn_dam)
		user.visible_message("<b>[user]</b> finishes mending the burnt wiring in [target]'s [affecting].")
		return
	...
	if(do_mob(user, target, 30))
		if(use(2))
			...
			user.visible_message("<b>[user]</b> [pick(repair_messages)] in [target]'s [affecting.name] with \the [src].")
