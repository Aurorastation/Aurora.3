...
/obj/machinery/power/apc/update_icon()
	...
	if(update & 2)
		...
		if(!(stat & (BROKEN|MAINT)) && update_state & UPDATE_ALLGOOD)
			...
			add_overlay(emissive_appearance(icon, "apc-cover-0"))
			add_overlay(emissive_appearance(icon, "apc-charge-0"))
			if(operating)
				...
				add_overlay(emissive_appearance(icon, "apcoequip-0"))
				add_overlay(emissive_appearance(icon, "apcolight-0"))
				add_overlay(emissive_appearance(icon, "apcoenv-0"))
