...
/obj/machinery/power/apc/attackby(obj/item/W, mob/user)
	...
	if (W.iscrowbar() && opened)
		...
		if(do_after(user, 20/W.toolspeed - 20/W.toolspeed * skill_time_reduction("electrical", 0.1, user)))
		...
		playsound(src.loc, 'sound/items/Deconstruct.ogg', 50, 1)
		if(do_after(user, 50/W.toolspeed - 50/W.toolspeed * skill_time_reduction("construction", 0.1, user)))
		...
		playsound(src.loc, 'sound/items/Deconstruct.ogg', 50, 1)
		if(do_after(user, 10/W.toolspeed - 10/W.toolspeed * skill_time_reduction("electrical", 0.05, user)))
		...
		if(emagged || hacker || infected)
			...
			if(do_after(user, 10/W.toolspeed SECONDS - 10/W.toolspeed SECONDS * skill_time_reduction("devices", 0.1, user), act_target = src))
				to_chat(user, "<span class='notice'>Problem diagnosed, searching for solution...</span>")
				if(do_after(user, 30/W.toolspeed SECONDS - 30/W.toolspeed SECONDS * skill_time_reduction("devices", 0.1, user), act_target = src))
					to_chat(user, "<span class='notice'>Solution found. Applying fixes...</span>")
					if(do_after(user, 60/W.toolspeed SECONDS - 60/W.toolspeed SECONDS * skill_time_reduction("devices", 0.1, user), act_target = src))
						if(prob(15))
		...
		if ((stat & BROKEN) \
			...
			if (WT.get_fuel() <1)
				...
				return
			playsound(src.loc, 'sound/items/Welder.ogg', 50, 1)
			if(do_after(user, 10/W.toolspeed - 10/W.toolspeed * skill_time_reduction("construction", 0.1, user)))
...
/obj/machinery/power/apc/emag_act(var/remaining_charges, var/mob/user)
	if (!(emagged || hacker))		// trying to unlock with an emag card
		if(opened)
			...
			flick("apc-spark", src)
			if (do_after(user, 6 - 6 * skill_time_reduction("devices", 0.1, user)))
				if(prob(50))
					emagged = 1
					locked = 0
