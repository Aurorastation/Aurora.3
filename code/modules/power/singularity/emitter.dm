	...
	if(W.iswrench())
		...
		switch(state)
			...
			if(EMITTER_BOLTED)
				state = EMITTER_LOOSE
				playsound(get_turf(src), W.usesound, 75, TRUE)
				user.visible_message("<b>[user]</b> unsecures \the [src]'s reinforcing bolts from the floor.", \
		...
		switch(state)
			...
			if(EMITTER_BOLTED)
				if(WT.remove_fuel(0, user))
					...
					user.visible_message("<b>[user]</b> starts to weld \the [src] to the floor.", \
			...
			if(EMITTER_WELDED)
				if(WT.remove_fuel(0, user))
					playsound(get_turf(src), 'sound/items/welder_pry.ogg', 50, TRUE)
					user.visible_message("<b>[user]</b> starts to cut \the [src] free from the floor.", \
