	...
	switch(construction_state)//TODO:Might be more interesting to have it need several parts rather than a single list of steps
		if(0)
			if(O.iswrench())
				...
				anchored = TRUE
				user.visible_message("<b>[user]</b> secures \the [src] to the floor.", \
		...
		if(1)
			if(O.iswrench())
				playsound(get_turf(src), O.usesound, 75, TRUE)
				anchored = FALSE
				user.visible_message("<b>[user]</b> detaches \the [src] from the floor.", \
					SPAN_NOTICE("You remove the external bolts."))
				temp_state--
			else if(O.iscoil())
				var/obj/item/stack/cable_coil/C = O
				if(C.use(1))
					user.visible_message("<b>[user]</b> adds wires to \the [src].", \
						SPAN_NOTICE("You add some wires."))
					temp_state++
		if(2)
			if(O.iswirecutter())//TODO:Shock user if its on?
				user.visible_message("<b>[user]</b> removes some wires from \the [src].", \
					SPAN_NOTICE("You remove some wires."))
				temp_state--
			else if(O.isscrewdriver())
				user.visible_message("<b>[user]</b> closes \the [src]'s access panel.", \
					SPAN_NOTICE("You close the access panel."))
				temp_state++
		if(3)
			if(O.isscrewdriver())
				user.visible_message("<b>[user]</b> opens \the [src]'s access panel.", \
