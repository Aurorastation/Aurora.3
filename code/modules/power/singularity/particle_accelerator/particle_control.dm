...
/obj/machinery/particle_accelerator/control_box/proc/remove_strength(var/s)
	if(assembled)
		...
		if(strength < 0)
			...
			investigate_log("decreased to <span class='good'>[strength]</span> by [usr.key]","singulo")
		strength_change()
...
/obj/machinery/particle_accelerator/control_box/proc/toggle_power(mob/user)
	...
	investigate_log("turned [active?"<span class='warning'>ON</span>":"<span class='good'>OFF</span>"] by [user ? user.key : "outside forces"]","singulo")
