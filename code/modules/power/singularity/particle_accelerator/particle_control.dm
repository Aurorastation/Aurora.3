...
/obj/machinery/particle_accelerator/control_box/proc/add_strength(var/s)
	if(assembled)
		...
		if(strength > strength_upper_limit)
			...
			log_game(text="PA Control Computer increased to [strength] by [usr.ckey]([usr]) in ([x],[y],[z])",ckey=key_name(usr))
...
/obj/machinery/particle_accelerator/control_box/proc/remove_strength(var/s)
	if(assembled)
		...
		if(strength < 0)
			...
			log_game(text="PA Control Computer decreased to [strength] by [usr.ckey]([usr]) in ([x],[y],[z])",ckey=key_name(usr))
...
/obj/machinery/particle_accelerator/control_box/proc/toggle_power()
	...
	log_game(text="PA Control Computer turned [active ?"ON":"OFF"] by [usr.ckey]([usr]) in ([x],[y],[z])",ckey=key_name(usr))
	if(src.active)
		update_use_power(2)
		for(var/obj/structure/particle_accelerator/part in connected_parts)
