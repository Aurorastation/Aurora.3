...
/obj/item/device/am_shielding_container/attackby(var/obj/item/I, var/mob/user)
	if(I.ismultitool() && isturf(loc))
		if(locate(/obj/machinery/am_shielding/) in loc)
			to_chat(user, "<span class='warning'>[hicon(src)]There is already an antimatter reactor section there.</span>")
			return
		...
		if(AMC && AMC.anchored)
			...
			to_chat(user, "<span class='warning'>[hicon(src)]Couldn't connect to an Antimatter Control Unit.</span>")
			return
