...
/obj/machinery/power/am_control_unit/attackby(obj/item/W, mob/user)
	...
	if(W.iswrench())
		if(!anchored)
			...
			playsound(get_turf(src), W.usesound, 75, 1)
			user.visible_message("[user.name] unsecures [src].", \
				"You remove the anchor bolts.", \
				"You hear a ratchet")
			src.anchored = 0
			disconnect_from_network()
		else
			to_chat(user, "<span class='warning'>Once bolted and linked to a shielding unit it [src] is unable to be moved!</span>")
		return
	...
	if(istype(W, /obj/item/am_containment))
		...
		user.visible_message("[user.name] loads an [W.name] into [src].", \
				"You load an [W.name].", \
				"You hear a thunk.")
		return
