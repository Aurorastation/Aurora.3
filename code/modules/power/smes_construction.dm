...
/obj/machinery/power/smes/buildable/attackby(var/obj/item/W as obj, var/mob/user as mob)
	...
	if (..())
		...
		if(W.iscrowbar())
			...
			if (do_after(usr, 100 * cur_coils - 100 * cur_coils * skill_time_reduction("electrical", 0.1, user))) // More coils = takes longer to disassemble. It's complex so largest one with 5 coils will take 50s
