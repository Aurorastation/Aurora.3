	...
	if(target_mob.can_inject(user, 1))
		if(reagents.total_volume)
			if(target_mob.reagents)
				...
				target_mob.log_message("Has been stabbed with [name] by [user.name] ([user.ckey])", LOG_VICTIM, log_globally = FALSE)
				user.log_message("Used the [name] to stab [target_mob.name] ([target_mob.ckey])", LOG_ATTACK, log_globally = FALSE)
