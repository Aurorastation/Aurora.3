...
/datum/event/economic_event/announce()
	...
	if(!body)
		switch(event_type)
			...
			if(ALIEN_RAIDERS)
				if(prob(20))
					body = "The Tiger Co-operative have raided [affected_dest.name] today, no doubt on orders from their enigmatic masters. Stealing wildlife, farm animals, medical research materials and kidnapping civilians. [SSmapping.current_map.company_name] authorities are standing by to counter attempts at bio-terrorism."
				else
