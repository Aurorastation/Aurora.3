...
/datum/trade_destination/anansi/get_custom_eventstring(var/event_type)
	if(event_type == RESEARCH_BREAKTHROUGH)
		...
		[pick("mind-machine interfacing","neuroscience","nano-augmentation","genetics")]. [SSmapping.current_map.company_name] is expected to announce a co-exploitation deal within the fortnight."
	return null
...
/datum/trade_destination/redolant/get_custom_eventstring(var/event_type)
	if(event_type == RESEARCH_BREAKTHROUGH)
		...
		[pick("phoron research","high energy flux capacitance","super-compressed materials","theoretical particle physics")]. [SSmapping.current_map.company_name] is expected to announce a co-exploitation deal within the fortnight."
	return null
