	...
	if(!body)
		...
		switch(event_type)
			if(RESEARCH_BREAKTHROUGH)
				...
				[SSmapping.current_map.company_name] declined to comment as to whether this could impinge on profits."
			...
			if(RESIGNATION)
				body = "[SSmapping.current_map.company_name] regretfully announces the resignation of [pick("Sector Admiral","Division Admiral","Ship Admiral","Vice Admiral")] [random_name(pick(MALE,FEMALE))]."
				if(prob(25))
