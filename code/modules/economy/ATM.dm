...
/obj/machinery/atm/emag_act(var/remaining_charges, var/mob/user)
	...
	to_chat(user, "<span class='warning'>[hicon(src)] The [src] beeps: \"[response]\"</span>")
	return 1
...
/obj/machinery/atm/attackby(obj/item/I as obj, mob/user as mob)
	if(istype(I, /obj/item/weapon/card))
		if(emagged)
			...
			to_chat(user, "<span class='warning'>[hicon(src)] CARD READER ERROR. This system has been compromised!</span>")
			return
...
/obj/machinery/atm/attack_hand(mob/user as mob)
	if(istype(user, /mob/living/silicon))
		to_chat(user, "<span class='warning'>[hicon(src)] Artificial unit recognized. Artificial units do not currently receive monetary compensation, as per system banking regulation #1005.</span>")
		return
		...
		switch(href_list["choice"])
				...
				if(authenticated_account)
					...
					if(transfer_amount <= 0)
						...
						if(SSeconomy.charge_to_account(target_account_number, authenticated_account.owner_name, transfer_purpose, machine_id, transfer_amount))
							to_chat(usr, "[hicon(src)]<span class='info'>Funds transfer successful.</span>")
							...
							to_chat(usr, "[hicon(src)]<span class='warning'>Funds transfer failed.</span>")

					else
						to_chat(usr, "[hicon(src)]<span class='warning'>You don't have enough funds to do that!</span>")
			if("view_screen")
			...
			if("attempt_auth")
				...
				if (!potential_account)
					to_chat(usr, "<span class='warning'>[hicon(src)] Account number not found.</span>")
				...
				if (!authenticated_account)
					number_incorrect_tries++
					to_chat(usr, "<span class='warning'>[hicon(src)] Incorrect pin/account combination entered, [(max_pin_attempts+1) - number_incorrect_tries] attempts remaining.</span>")
					...
					to_chat(usr, "<span class='notice'> [hicon(src)] Access granted. Welcome user '[authenticated_account.owner_name].'</span>")
			...
			if("insert_card")
				if(!held_card)
					...
					if(emagged)
						to_chat(usr, "<span class='warning'>[hicon(src)] The ATM card reader rejected your ID because this machine has been sabotaged!</span>")
					else
	...
	if(!authenticated_account)
		if(human_user.wear_id)
			...
			if(I)
				...
				if(authenticated_account)
					to_chat(human_user, "<span class='notice'>[hicon(src)] Access granted. Welcome user '[authenticated_account.owner_name].'</span>")
