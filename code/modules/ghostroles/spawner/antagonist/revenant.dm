...
/datum/ghostspawner/revenant/proc/check_rift()
	...
	if(revenants.kill_count > kills_needed)
		...
		if(rift_turf)
			...
			if(!first_rift_done)
				command_announcement.Announce("Aurora, we're detecting energy signatures eerily similar to a bluespace rift breach inside your hull. [SScargo.shuttle ? "We are sending you a bluespace neutralizer via the cargo shuttle. " : ""]Locate the rift and shut it down.", "Bluespace Breach Alert")
