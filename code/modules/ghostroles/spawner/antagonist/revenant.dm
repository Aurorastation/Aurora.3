...
/datum/ghostspawner/revenant/proc/check_rift()
	...
	if(GLOB.revenants.kill_count > kills_needed)
		...
		if(rift_turf)
			...
			if(!first_rift_done)
				command_announcement.Announce("[SSmapping.current_map.station_name], we're detecting energy signatures eerily similar to a bluespace rift breach inside your hull. [SScargo.shuttle ? "We're sending you a bluespace neutralizer via the cargo elevator. If you need more, your research department should be able to print neutralizers as well if they've been increasing their bluespace research levels. " : ""]Locate the rift and shut it down.", "Bluespace Breach Alert")
