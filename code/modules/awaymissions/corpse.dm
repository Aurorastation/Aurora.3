...
/obj/effect/landmark/corpse
	...
	/// Species of the corpse.
	/// Should either be a species string, and then it is that specific string
	/// Or a list of species strings, where it randomly picks one species from that list
	var/species = SPECIES_HUMAN

	// Vars below are used for legacy corpses.
	// Should use the outfit var instead, as it can be shared by ghostrole spawners, and generally is more sane.
...
/obj/effect/landmark/corpse/proc/createCorpse() //Creates a mob and checks for gear in each slot before attempting to equip it.
	...
	if(outfit)
		// use the outfit to dress and equip the corpse
		if(islist(outfit))
			outfit = pick(outfit)
		M.preEquipOutfit(outfit, FALSE)
		M.equipOutfit(outfit, FALSE)
	else
		// otherwise, use the old vars
		if(src.corpseuniform)
			var/obj/item/clothing/under/cuniform = new corpseuniform(M)
			cuniform.sensor_mode = 0
			M.equip_to_slot_or_del(cuniform, slot_w_uniform_str)
		if(src.corpsesuit)
			M.equip_to_slot_or_del(new src.corpsesuit(M), slot_wear_suit_str)
		if(src.corpseshoes)
			M.equip_to_slot_or_del(new src.corpseshoes(M), slot_shoes_str)
		if(src.corpsegloves)
			M.equip_to_slot_or_del(new src.corpsegloves(M), slot_gloves_str)
		if(src.corpseradio)
			M.equip_to_slot_or_del(new src.corpseradio(M), slot_l_ear_str)
		if(src.corpseglasses)
			M.equip_to_slot_or_del(new src.corpseglasses(M), slot_glasses_str)
		if(src.corpsemask)
			M.equip_to_slot_or_del(new src.corpsemask(M), slot_wear_mask_str)
		if(src.corpsehelmet)
			M.equip_to_slot_or_del(new src.corpsehelmet(M), slot_head_str)
		if(src.corpsebelt)
			M.equip_to_slot_or_del(new src.corpsebelt(M), slot_belt_str)
		if(src.corpsepocket1)
			M.equip_to_slot_or_del(new src.corpsepocket1(M), slot_r_store_str)
		if(src.corpsepocket2)
			M.equip_to_slot_or_del(new src.corpsepocket2(M), slot_l_store_str)
		if(src.corpseback)
			M.equip_to_slot_or_del(new src.corpseback(M), slot_back_str)
		if(src.corpseid == 1)
			...
			M.set_id_info(W)
			M.equip_to_slot_or_del(W, slot_wear_id_str)
