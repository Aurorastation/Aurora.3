...
/datum/citizenship/izweski/get_objectives(mission_level, var/mob/living/carbon/human/H)
	...
	switch(mission_level)
		if(REPRESENTATIVE_MISSION_HIGH)
			if(isvaurca(H))
				rep_objectives = pick("Obtain [rand(2,3)] sheets of solid phoron below market value, buying directly from the source.",
								...
								"Promote the advantages of K'lax engineering to the [SSmapping.current_map.boss_name] in order to invite future investment in the Hegemony.")
			else
		...
		if(REPRESENTATIVE_MISSION_MEDIUM)
			if(isvaurca(H))
				rep_objectives = pick("Collect evidence of the [SSmapping.current_map.boss_name] being unfair or bigoted to Vaurca or Unathi Employees, to be used as leverage in future labor negotiations",
								"Upsell K'laxan Vaurca to different command staff. Have one complete a Bound Vaurca requisition form.",
								"Promote K'lax and Unathi culture to the crew of the [SSmapping.current_map.station_name]. Encourage tourism to Izweski space.")
			else
				rep_objectives = pick("Speak out against any violation of the Honor Code to or by Unathi on the [SSmapping.current_map.station_short]",
								"Proselytize the Sk'akh or Tha'kh religions to the crew.",
								"Discourage the Unathi crew of the [SSmapping.current_map.station_name] from the Aut'akh or Si'akh heresies.")
		else
			if(isvaurca(H))
