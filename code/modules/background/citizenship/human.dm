		...
		if(REPRESENTATIVE_MISSION_HIGH)
			if(isvaurca(H))
				rep_objectives = pick("Compile and report and audit [rand(1,3)] suspicious indivduals who might be spies or otherwise act hostile against the Republic.",
								"Collect evidence of the [SSmapping.current_map.boss_name] being unfair or bigoted to Vaurca employees, to be used as leverage in future hive labor negotiations.",
								"Convince the command of the [SSmapping.current_map.station_name] of the utility of Bound labor over similar alternatives such as cyborgs or owned synthetics.")
			else
				rep_objectives = pick("Compile and report and audit [rand(1,3)] suspicious indivduals who might be spies or otherwise act hostile against the Republic.",
		...
		if(REPRESENTATIVE_MISSION_MEDIUM)
			...
			if(isvaurca(H))
				rep_objectives = pick("Run a questionanaire on Tau Ceti citizens' views on Vaurca citizenship.",
								...
								"Protect and promote the public image of the Zo'ra Hive to all [SSmapping.current_map.boss_name] employees.")
			else
...
/datum/citizenship/sol_alliance/get_objectives(mission_level, var/mob/living/carbon/human/H)
	...
	switch(mission_level)
		if(REPRESENTATIVE_MISSION_HIGH)
			rep_objectives = pick("Collect evidence of the [SSmapping.current_map.boss_name] being unfair or oppressive against Solarian employees, to be used as leverage in future diplomatic talks.",
							"Convince [rand(1,3)] Solarian employees to apply for the Solarian Armed Forces.")

		if(REPRESENTATIVE_MISSION_MEDIUM)
			rep_objectives = pick("Have [rand(2,5)] amount of Sol citizens write down their grievances with the company, and present the report to [SSmapping.current_map.station_short] command.",
							"Convince [rand(3,6)] qualified specialists among crew to enter Sol Alliance space, and issue them a visa recommendation.")
		else
			rep_objectives = pick("Collect [rand(3,7)] pictures of secure [SSmapping.current_map.station_short] areas.",
	...
	switch(mission_level)
		...
		if(REPRESENTATIVE_MISSION_MEDIUM)
			rep_objectives = pick("Promote and distribute the copies of Dominian Code of Honor to [rand(3,6)] crewmembers.",
							"Convince a Dominian citizen to return to the Empire with valuable information on the [SSmapping.current_map.boss_name] to present.")
		else
