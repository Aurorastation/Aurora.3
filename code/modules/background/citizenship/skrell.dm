...
/datum/citizenship/nralakk/get_objectives(mission_level, var/mob/living/carbon/human/H)
	...
	switch(mission_level)
		if(REPRESENTATIVE_MISSION_HIGH)
			if(isvaurca(H))
				rep_objectives = pick("Collect evidence of the [SSmapping.current_map.boss_name] being unfair or bigoted to Federation employees, to be used as leverage in future hive labor negotiations",
								"Acquire information on dissidents towards the Federation, forwarding it to the embassy",
								"Convince the command of the [SSmapping.current_map.boss_name] of the advantages that Bound Vaurcae hold over synthetics.")
			else
		...
		if(REPRESENTATIVE_MISSION_MEDIUM)
			if(isvaurca(H))
				rep_objectives = pick("Legally curtail the advancements and liberal thinking towards synthetics.",
								"Remind C'thur Vaurcae aboard the [SSmapping.current_map.station_name] that they are representative of their hive-cell, and encourage them to increase their index rating",
				...
				rep_objectives = pick("Ensure the interests of Federation citizens are upheld by the vessel. This includes C'thur and Diona of Federation origin",
								...
								"The [SSmapping.current_map.station_name] hosts some of the brightest minds in the galaxy; winning them over towards the Federation is a major victory",
			...
			if(isvaurca(H))
				...
				rep_objectives = pick("Consider assisting crew within the capacity of your role, an altruistic image is good PR towards the Federation",
								"Some Skrell are not part of the Federation; attempt to convince them to become a citizen.",
								"Promote Nralakk tourism among the non-citizen employees of the [SSmapping.current_map.boss_name] in order to build positive opinion.")
