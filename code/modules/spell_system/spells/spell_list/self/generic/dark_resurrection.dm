...
/spell/targeted/dark_resurrection/cast(mob/target,var/mob/living/carbon/human/user as mob)
	..()
	if(user.stat != DEAD)
		to_chat(user, SPAN_NOTICE("You're not dead yet!"))
		return 0
	...
	if(P)
		...
		to_chat(user, SPAN_NOTICE("Your dead body returns to your phylactery, slowly rebuilding itself."))
		if(prob(25))
		...
		to_chat(user, SPAN_DANGER("Your phylactery was destroyed, your existence will face oblivion now."))
		user.visible_message(SPAN_CULT("As [user]'s body turns to dust, a twisted wail can be heard!"))
...
/spell/targeted/dark_resurrection/proc/post_dark_resurrection(var/mob/living/carbon/human/user as mob)
	user.revive()
	to_chat(user, SPAN_DANGER("You have returned to life!"))
	user.visible_message(SPAN_CULT("[user] rises up from the dead!"))
