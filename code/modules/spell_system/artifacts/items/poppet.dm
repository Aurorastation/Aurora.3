...
/obj/item/poppet/examine(mob/user)
	..(user)
	if(countenance)
		to_chat(user, SPAN_NOTICE("It is modeled after a [countenance]."))
...
/obj/item/poppet/attackby(obj/item/W as obj, mob/user as mob)
	...
	if(H && cooldown < world.time)
		...
		if(istype(W, /obj/item/device/flashlight))
			to_chat(H, SPAN_WARNING("You direct \the [W] towards \the [src]'s eyes!"))
			playsound(get_turf(H), 'sound/items/flashlight.ogg', 50, 1, -1)
			flick("flash", H.flash)
			H.eye_blurry = 5

		if(W.iscoil())
			to_chat(H, SPAN_WARNING("You strangle \the [src] with \the [W]!"))
			H.silent += 10
		...
		if(W.edge)
			to_chat(H, SPAN_WARNING("You stab \the [src] with \the [W]!"))
