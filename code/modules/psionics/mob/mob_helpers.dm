...
/mob/living/carbon/is_psi_blocked()
	if(!psi && !has_psi_aug())
		if(isSynthetic())
			return SPAN_ALIEN("There is no mind here for you to dip your mentality into.")
		if (isvaurca(src))
			return SPAN_CULT("You feel your thoughts pass right through a mind empty of psychic energy, unable to get a grasp on anything.")
		if (is_diona())
