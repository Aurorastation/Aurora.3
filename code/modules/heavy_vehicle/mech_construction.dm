...
/mob/living/heavy_vehicle/proc/install_system(var/obj/item/system, var/system_hardpoint, var/mob/user)
	...
	if(user)
		...
		if(delay > 0)
			...
			if(!do_after(user, delay - delay * skill_time_reduction("devices", 0.1, user), src) || user.get_active_hand() != system)
				return FALSE
...
/mob/living/heavy_vehicle/proc/remove_system(var/system_hardpoint, var/mob/user, var/force)
	...
	if(user)
		...
		if(delay > 0)
			...
			if(!do_after(user, delay - delay * skill_time_reduction("devices", 0.1, user), src) || hardpoints[system_hardpoint] != system)
				return FALSE
