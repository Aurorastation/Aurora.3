...
// A lot of this is copied from jukeboxes.
/obj/item/mecha_equipment/subwoofers
	name = "subwoofers"
	desc = "An exosuit-mounted speaker setup."
	icon_state = "mech_speakers"
	restricted_hardpoints = list(HARDPOINT_HEAD)
	mech_layer = MECH_GEAR_LAYER
	/// The jukebox obj that this equipment accesses.
	var/obj/machinery/media/jukebox/mounted/jukebox
	origin_tech = list(TECH_MATERIAL = 1, TECH_ENGINEERING = 1)
	module_hints = list(
		"<b>Alt Click(Icon):</b> Open the music player interface.",
	)

/obj/item/mecha_equipment/subwoofers/Initialize()
	. = ..()
	jukebox = new /obj/machinery/media/jukebox/mounted(src)
	jukebox.parent = src

/obj/item/mecha_equipment/subwoofers/attack_self(var/mob/user)
	. = ..()
	if(.)
...
/obj/item/mecha_equipment/subwoofers/Destroy()
	QDEL_NULL(jukebox)
	return ..()
