	...
	if(owner && LAZYISIN(owner.pilots, user))
		if(!owner.hatch_closed && length(carrying))
			...
			if(user.put_in_active_hand(chosen_obj))
				owner.visible_message("<b>[user]</b> carefully grabs \the [chosen_obj] from \the [src].")
				carrying -= chosen_obj
	. = ..()
	...
	if(.)
		...
		if(istype(target, /obj))
			...
			if(user.a_intent == I_HURT)
				...
				owner.visible_message("<b>[owner]</b> pushes [target] out of the way.")
...
/obj/item/mecha_equipment/drill/afterattack(var/atom/target, var/mob/living/user, var/inrange, var/params)
	...
	if(.)
		...
		if(istype(target,/obj/item/material/drill_head))
			...
			if(drill_head)
				owner.visible_message(SPAN_NOTICE("\The [owner] detaches the [drill_head] mounted on [src]."))
				drill_head.forceMove(owner.loc)
			DH.forceMove(src)
			drill_head = DH
			owner.visible_message(SPAN_NOTICE("\The [owner] mounts the [drill_head] on [src]."))
			return
