/mob/living/heavy_vehicle/proc/can_move(var/mob/user, delay_modifier)
	...
	next_mecha_move = world.time + max(1, delay_modifier + (incorporeal_move ? legs.move_delay / 2 : legs.move_delay) + (legs.damaged_delay * (legs.total_damage / legs.max_damage) * legs.damaged_delay_slope))
	return TRUE
...
/mob/living/heavy_vehicle/proc/can_turn(var/mob/user, delay_modifier)
	...
	next_mecha_turn = world.time + max(1, delay_modifier + legs.turn_delay + (legs.damaged_delay * (legs.total_damage / legs.max_damage) * legs.damaged_delay_slope))
	return TRUE
...
/mob/living/heavy_vehicle/proc/can_strafe(var/mob/user, delay_modifier)
	...
	next_mecha_move = world.time + max(1, delay_modifier + ((legs.move_delay + (legs.damaged_delay * (legs.total_damage / legs.max_damage) * legs.damaged_delay_slope)) * legs.strafe_delay_modifier))
	return TRUE
