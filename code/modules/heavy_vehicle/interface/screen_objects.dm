...
/obj/screen/mecha/toggle/sensor/toggled()
	if(!owner.head)
		to_chat(usr, SPAN_WARNING("I/O Error: Sensor systems not found."))
		return
	if(!owner.head.vision_flags)
		to_chat(usr, SPAN_WARNING("\The [owner.head] does not have any special sensor configurations."))
		return
	toggled = !toggled
	...
	to_chat(usr, SPAN_NOTICE("[capitalize_first_letters(owner.head.name)] Advanced Sensor mode is [owner.head.active_sensors ? "now" : "no longer" ] active."))
