...
/obj/structure/heavy_vehicle_frame/update_icon()
	...
	add_overlay(get_mech_icon(list(body), MECH_BASE_LAYER))
	if(body)
		density = TRUE
		add_overlay(get_mech_image("[body.icon_state]_cockpit", body.on_mech_icon, MECH_BASE_LAYER))
	else
		density = FALSE
	if(head)
		add_overlay(get_mech_image("[head.icon_state]", head.on_mech_icon, head.color, MECH_HEAD_LAYER))
		add_overlay(get_mech_image("[head.icon_state]_eyes", head.on_mech_icon, null, EYEGLOW_LAYER))
	if(arms)
		add_overlay(get_mech_image(arms.icon_state, arms.on_mech_icon, arms.color, MECH_ARM_LAYER))
	if(legs)
		add_overlay(get_mech_image(legs.icon_state, legs.on_mech_icon, legs.color, MECH_LEG_LAYER))
	if(density != opacity)
		set_opacity(density)
