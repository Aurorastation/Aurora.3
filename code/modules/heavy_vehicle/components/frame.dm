...
/obj/structure/heavy_vehicle_frame/update_icon()
	...
	if(body)
		density = TRUE
		AddOverlays(get_mech_image("[body.icon_state]_cockpit", body.on_mech_icon, MECH_BASE_LAYER))
	else
		density = FALSE
	if(head)
	...
	if(legs)
		AddOverlays(get_mech_image(legs.icon_state, legs.on_mech_icon, legs.color, MECH_LEG_LAYER))
	if(density != opacity)
		set_opacity(density)
