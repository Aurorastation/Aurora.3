...
/obj/item/plate/examine(mob/user, distance)
    . = ..()
    if(holding)
        to_chat(user, "It looks like there is \a [holding.name] on \the [name].")
    if(grease)
        to_chat(user, SPAN_WARNING("\The [name] looks a little unclean."))    

/obj/item/plate/attackby(obj/item/I, mob/user)
    if ((istype(I, /obj/item/reagent_containers/food/snacks) || istype(I, /obj/item/trash)) && !holding)
        holding = I
        user.prepare_for_slotmove(I)
        I.forceMove(src)
        to_chat(user, SPAN_NOTICE("You place \the [holding.name] on \the [name]"))
        update_icon()
        return
    if (istype(I, /obj/item/reagent_containers/food/snacks) || istype(I, /obj/item/trash))
        to_chat(user, SPAN_WARNING("\The [name] already has something on it!"))
        return
...
/obj/item/plate/on_rag_wipe(obj/item/reagent_containers/glass/rag/R)
    . = ..()
    if(grease)
        grease = FALSE
    return
...
/obj/item/plate/attack(mob/living/M, mob/living/user, target_zone)
...
//Is this technically a plate? No. Was it created to do a similar function? Yes
