...
/obj/machinery/rnd/experimentor/proc/SetTypeReactions()
	...
	for(var/I in typesof(/obj/item))
		...
		if(ispath(I, /obj/item/weapon/stock_parts))
		...
		if(ispath(I, /obj/item/weapon/rcd) || ispath(I, /obj/item/weapon/grenade) || ispath(I, /obj/item/weapon/aicard) || ispath(I, /obj/item/weapon/storage/backpack/holding) || ispath(I, /obj/item/slime_extract) || ispath(I, /obj/item/device/onetankbomb) || ispath(I, /obj/item/device/transfer_valve))
			var/obj/item/tempCheck = I
...
/obj/machinery/rnd/experimentor/proc/experiment(exp,obj/item/exp_on)
	...
	if(exp == SCANTYPE_IRRADIATE)
		...
		if(prob(EFFECT_PROB_LOW) && criticalReaction)
			...
			/*if(istype(loaded_item, /var/obj/item/weapon/grenade/chem_grenade))
	...
	if(exp == SCANTYPE_GAS)
		...
		if(prob(EFFECT_PROB_LOW) && criticalReaction)
			...
			new /obj/item/stack/material/phoron(get_turf(pick(oview(1,src))))
		else if(prob(EFFECT_PROB_VERYLOW-badThingCoeff))
	...
	if(exp == SCANTYPE_HEAT)
		...
		if(prob(EFFECT_PROB_LOW) && criticalReaction)
			...
			chosenchem = pick("phoron","capsaicin","ethanol")
			...
			if(MT)
				...
				investigate_log("Experimentor has launched a <font color='red'>fireball</font> at [M]!", "experimentor") // meme
	...
	if(exp == SCANTYPE_OBLITERATE)
		...
		if(prob(EFFECT_PROB_LOW) && criticalReaction)
			...
			new /obj/item/stack/material/plasteel(get_turf(pick(oview(1,src))))
		else if(prob(EFFECT_PROB_VERYLOW-badThingCoeff))
	...
	if(prob(EFFECT_PROB_VERYLOW-badThingCoeff) && loaded_item)
		...
		if(globalMalf < 15)
			...
			ejectItem()
		if(globalMalf > 16 && globalMalf < 35) // meme
			...
			ejectItem(TRUE)
		if(globalMalf > 36 && globalMalf < 50) // meme
		...
		if(globalMalf > 51 && globalMalf < 75) // meme
...
/obj/item/relic/proc/corgicannon(mob/user) // meme
	warn_admins(user, "Corgi Cannon", 0)

/obj/item/relic/proc/clean(mob/user) // check
	playsound(src, "sparks", rand(25,50), 1)
	var/obj/item/weapon/grenade/chem_grenade/cleaner/CL = new
	CL.prime()
	warn_admins(user, "Smoke", 0)

/obj/item/relic/proc/flash(mob/user) //redo
	playsound(src, "sparks", rand(25,50), 1)
	//var/obj/item/weapon/grenade/flashbang/CB = new/var/obj/item/weapon/grenade/flashbang(user.loc)
	//CB.prime()
	warn_admins(user, "Flash")

/obj/item/relic/proc/petSpray(mob/user) // meme
