			...
			if(w.get_fuel() >= 4 && !src.method)
				if(inside)
					...
					for(var/mob/M in viewers(world.view, user))
						M.show_message(span("info", "[src] burns away revealing [inside]."),1)
				else
					for(var/mob/M in viewers(world.view, user))
						M.show_message(span("info", "[src] burns away into nothing."),1)
				qdel(src)
				w.remove_fuel(4)
			else
				for(var/mob/M in viewers(world.view, user))
					M.show_message(span("info", "A few sparks fly off [src], but nothing else happens."),1)
				w.remove_fuel(1)
			return
