	...
	if(activated)
		if(inserted_battery && inserted_battery.battery_effect && (inserted_battery.stored_charge > 0) )
			...
			if(world.time - last_activation > interval)
				if(inserted_battery.battery_effect.effect == EFFECT_TOUCH)
					if(interval > 0)
						...
						if(holder)
							to_chat(holder, "the [hicon(src)] [src] held by [holder] shudders in your grasp.")
						else
							src.loc.visible_message("the [hicon(src)] [src] shudders.")
			...
			if(inserted_battery.stored_charge <= 0)
				src.loc.visible_message("<span class='notice'>[hicon(src)] [src] buzzes.</span>", "<span class='notice'>[hicon(src)] You hear something buzz.</span>")
				shutdown_emission()
			else if(world.time > time_end)
				src.loc.visible_message("<span class='notice'>[hicon(src)] [src] chimes.</span>", "<span class='notice'>[hicon(src)] You hear something chime.</span>")
				shutdown_emission()
		else
			src.visible_message("<span class='notice'>[hicon(src)] [src] buzzes.</span>", "<span class='notice'>[hicon(src)] You hear something buzz.</span>")
			shutdown_emission()
...
/obj/item/weapon/anodevice/Topic(href, href_list)
	...
	if(href_list["startup"])
		if(inserted_battery && inserted_battery.battery_effect && (inserted_battery.stored_charge > 0) )
			activated = 1
			src.visible_message("<span class='notice'>[hicon(src)] [src] whirrs.</span>", "[hicon(src)]<span class='notice'>You hear something whirr.</span>")
			if(!inserted_battery.battery_effect.activated)
				inserted_battery.battery_effect.ToggleActivate(1)
