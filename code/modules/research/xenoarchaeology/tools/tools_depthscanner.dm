	...
	if(istype(A,/turf/simulated/mineral))
		...
		if((M.finds && M.finds.len) || M.artifact_find)
			...
			for(var/mob/L in range(src, 1))
				to_chat(L, "<span class='notice'>[icon2html(src, usr)] [src] pings.</span>")
		...
		if(B.artifact_find)
			...
			for(var/mob/L in range(src, 1))
				to_chat(L, "<span class='notice'>[icon2html(src, usr)] [src] pings [pick("madly","wildly","excitedly","crazily")]!</span>")
