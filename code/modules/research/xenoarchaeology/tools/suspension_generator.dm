	...
	if(href_list["select_field"])
		...
		if (istype(I, /obj/item/card))
			...
			if(attempt_unlock(I, usr))
				to_chat(usr, span("info", "You insert [I], the console flashes \'<i>Access granted.</i>\'"))
			else
				to_chat(usr, SPAN_WARNING("You insert [I], the console flashes \'<i>Access denied.</i>\'"))
	...
	if (W.isscrewdriver())
		if(!open)
			if(screwed)
				...
				screwed = 1
			to_chat(user, span("info", "You [screwed ? "screw" : "unscrew"] the battery panel."))
	else if (W.iscrowbar())
		if(!locked)
			if(!screwed)
				if(!suspension_field)
					if(open)
						...
						open = 1
					to_chat(user, span("info", "You crowbar the battery panel [open ? "open" : "in place"]."))
...
/obj/machinery/suspension_gen/proc/activate()
	...
	switch(field_type)
		if("carbon")
			...
			for(var/mob/living/carbon/C in T)
				C.weakened += 5
				C.visible_message(SPAN_NOTICE("\icon[C] [C] begins to float in the air!"), SPAN_NOTICE("You feel tingly and light, but it is difficult to move."))
		if("nitrogen")
			success = 1
			//
		if("mercury")
		...
		if("iron")
			...
			for(var/mob/living/silicon/R in T)
				R.weakened += 5
				R.visible_message(SPAN_NOTICE("\icon[R] [R] begins to float in the air!"), SPAN_NOTICE("You feel tingly and light, but it is difficult to move."))
			//
	...
	if(collected)
		suspension_field.icon_state = "energynet"
		suspension_field.add_overlay("shield2")
		src.visible_message(SPAN_NOTICE("\icon[suspension_field] [suspension_field] gently absconds [collected > 1 ? "something" : "several things"]."))
	else
