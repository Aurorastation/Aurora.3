...
/obj/machinery/auto_cloner/machinery_process()
	if(powered(power_channel))
		if(!previous_power_state)
			...
			icon_state = "cellold1"
			src.visible_message(SPAN_NOTICE("\icon[src] [src] suddenly comes to life!"))

		//slowly grow a mob
		if(prob(5))
			src.visible_message(SPAN_NOTICE("\icon[src] [src] [pick("gloops","glugs","whirrs","whooshes","hisses","purrs","hums","gushes")]."))
		...
		if(previous_power_state)
			...
			icon_state = "cellold0"
			src.visible_message(SPAN_NOTICE("\icon[src] [src] suddenly shuts down."))
