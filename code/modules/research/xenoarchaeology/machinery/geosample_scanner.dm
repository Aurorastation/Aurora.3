	...
	if(scanning)
		if(!scanned_item || scanned_item.loc != src)
			...
			if(scanner_seal_integrity <= 0 || (scanner_temperature >= 1273 && !rad_shield))
				stop_scanning()
				visible_message("<span class='notice'>\icon[src] buzzes unhappily. It has failed mid-scan!</span>")

			if(prob(5))
				visible_message("<span class='notice'>\icon[src] [pick("whirrs","chuffs","clicks")][pick(" excitedly"," energetically"," busily")].</span>")
	else
		//gradually cool down over time
		if(scanner_temperature > 0)
			scanner_temperature = max(scanner_temperature - 5 - 10 * rand(), 0)
		if(prob(0.75))
			visible_message("<span class='notice'>\icon[src] [pick("plinks","hisses")][pick(" quietly"," softly"," sadly"," plaintively")].</span>")
...
/obj/machinery/radiocarbon_spectrometer/proc/complete_scan()
	visible_message("<span class='notice'>\icon[src] makes an insistent chime.</span>")
