	...
	if(scanning)
		if(!scanned_item || scanned_item.loc != src)
			...
			if(scanner_seal_integrity <= 0 || (scanner_temperature >= 1273 && !rad_shield))
				stop_scanning()
				visible_message("<span class='notice'>[icon2html(src, usr)] buzzes unhappily. It has failed mid-scan!</span>", range = 2)

			if(prob(5))
				visible_message("<span class='notice'>[icon2html(src, usr)] [pick("whirrs","chuffs","clicks")][pick(" excitedly"," energetically"," busily")].</span>", range = 2)
	else
		//gradually cool down over time
		if(scanner_temperature > 0)
			scanner_temperature = max(scanner_temperature - 5 - 10 * rand(), 0)
		if(prob(0.75))
			visible_message("<span class='notice'>[icon2html(src, usr)] [pick("plinks","hisses")][pick(" quietly"," softly"," sadly"," plaintively")].</span>", range = 2)
...
/obj/machinery/radiocarbon_spectrometer/proc/complete_scan()
	visible_message("<span class='notice'>[icon2html(src, usr)] makes an insistent chime.</span>", range = 2)
