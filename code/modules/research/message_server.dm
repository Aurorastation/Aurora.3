			...
			switch(priority)
				if(2)
					if(!Console.silent)
						...
						Console.audible_message("\icon[Console] *The Requests Console beeps: 'PRIORITY Alert in [sender]'",,5)
					...
					if(!Console.silent)
						playsound(Console.loc, 'sound/machines/twobeep.ogg', 50, 1)
						Console.audible_message("\icon[Console] *The Requests Console beeps: 'Message from [sender]'",,4)
