...
/singleton/state/weather/sandstorm/handle_exposure_effects(mob/living/M, obj/abstract/weather_system/weather)
	if((istype(M, /mob/living/simple_animal) && M.faction == "Moghes") || M.get_species(FALSE) == SPECIES_MONKEY_UNATHI) //Not realistic that they're just immune, but we don't have AI for simplemobs seeking shelter from storms.
		return
