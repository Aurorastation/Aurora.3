...
/obj/item/integrated_circuit/converter/string_formatter
	name = "string formatter"
	desc = "Formats up to eight values into a template string."
	extended_desc = "Uses {1} through {8} as replacement tokens. For example, 'Pressure: {1} kPa' will replace {1} with value 1."
	icon_state = "string"
	complexity = 4
	inputs = list(
	...
	outputs = list(
		"formatted string" = IC_PINTYPE_STRING
	)
	activators = list(
		"format" = IC_PINTYPE_PULSE_IN,
		"on formatted" = IC_PINTYPE_PULSE_OUT
	)
	spawn_flags = IC_SPAWN_DEFAULT|IC_SPAWN_RESEARCH

/obj/item/integrated_circuit/converter/string_formatter/do_work()
	var/template = get_pin_data(IC_INPUT, 1)
	...
	for(var/i = 1 to 8)
		...
		if(isnull(value))
			value = ""
		output = replacetext(output, "{[i]}", "[value]")
...
/obj/item/integrated_circuit/converter/string_splitter
	name = "string splitter"
	desc = "Splits a string into a list using a separator."
	extended_desc = "Useful for parsing text pad commands or breaking apart formatted data."
	icon_state = "string-list"
	complexity = 4
	inputs = list(
	...
	outputs = list(
		"split list" = IC_PINTYPE_LIST
	)
	activators = list(
		"split" = IC_PINTYPE_PULSE_IN,
		"on split" = IC_PINTYPE_PULSE_OUT
	)
	spawn_flags = IC_SPAWN_DEFAULT|IC_SPAWN_RESEARCH

/obj/item/integrated_circuit/converter/string_splitter/do_work()
	var/text = get_pin_data(IC_INPUT, 1)
	...
	if(!istext(separator) || !length(separator))
		separator = " "

	set_pin_data(IC_OUTPUT, 1, splittext(text, separator))
	push_data()
	activate_pin(2)


/obj/item/integrated_circuit/converter/command_parser
	name = "command parser"
	desc = "Parses a command string into a command and arguments."
	extended_desc = "Splits text by spaces. The first word is the command. The next three words are exposed directly, and all remaining words are returned as an argument list."
	icon_state = "string-list"
	complexity = 6
	inputs = list(
	...
	outputs = list(
		...
		"argument 3" = IC_PINTYPE_STRING,
		"argument list" = IC_PINTYPE_LIST
	)
	activators = list(
		"parse" = IC_PINTYPE_PULSE_IN,
		"on parsed" = IC_PINTYPE_PULSE_OUT,
		"not parsed" = IC_PINTYPE_PULSE_OUT
	)
	spawn_flags = IC_SPAWN_DEFAULT|IC_SPAWN_RESEARCH

/obj/item/integrated_circuit/converter/command_parser/do_work()
	var/text = get_pin_data(IC_INPUT, 1)
	...
	set_pin_data(IC_OUTPUT, 1, uppertext(clean_parts[1]))
	set_pin_data(IC_OUTPUT, 2, length(clean_parts) >= 2 ? clean_parts[2] : null)
	set_pin_data(IC_OUTPUT, 3, length(clean_parts) >= 3 ? clean_parts[3] : null)
	set_pin_data(IC_OUTPUT, 4, length(clean_parts) >= 4 ? clean_parts[4] : null)
	set_pin_data(IC_OUTPUT, 5, arguments)
