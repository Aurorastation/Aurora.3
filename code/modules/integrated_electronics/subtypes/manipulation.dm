...
/obj/item/integrated_circuit/manipulation/plant_module/do_work()
	...
	// TODO: Test if this is correct for vines!
	if(istype(OM, /obj/effect/plant) && check_target(OM) && get_pin_data(IC_INPUT, 2) == 2)
		qdel(OM)
		push_data()
		activate_pin(2)
		return
		...
		if(1)
			if(can_pull(to_pull))
				if(check_target(to_pull, exclude_contents = TRUE))
					...
					moved_event.register(to_pull, src, .proc/check_pull) //Whenever the target moves, make sure we can still pull it!
					destroyed_event.register(to_pull, src, .proc/stop_pulling) //Stop pulling if it gets destroyed
					moved_event.register(acting_object, src, .proc/pull) //Make sure we actually pull it.
			push_data()
		if(3)
			if(pulling)
				stop_pulling()
...
/obj/item/integrated_circuit/manipulation/claw/proc/stop_pulling()
	...
	moved_event.unregister(pulling, src)
	moved_event.unregister(AM, src)
	AM.visible_message("\The [AM] stops pulling \the [pulling]")
	destroyed_event.unregister(pulling, src)
	pulling = null
