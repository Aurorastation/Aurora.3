...
/obj/item/integrated_circuit/manipulation/locomotion/builtin
	name = "integrated locomotion circuit"
	desc = "A built-in chip that allows a mobile assembly to move in a given direction. Drone-optimized for responsive movement."
	size = -1
	complexity = 0
	removable = FALSE
	spawn_flags = null
	...
	if(T && assembly)
		...
		if(assembly.loc == T)
			...
			if(isnum(wanted_dir.data))
				...
				// Drones move faster for better responsiveness; other assemblies move at standard pace
				var/move_delay = istype(assembly, /obj/item/electronic_assembly/drone) ? 1 SECOND : 2 SECONDS
...
/obj/item/integrated_circuit/manipulation/shocker
	...
	Severity determines the power draw and usage of each shock. It accepts values between 0 and 30. It has a 2 second cooldown."
	w_class = WEIGHT_CLASS_TINY
	complexity = 24
	inputs = list(
		"target" = IC_PINTYPE_REF,
...
/obj/item/integrated_circuit/manipulation/shocker/on_data_written()
	...
	power_draw_per_use = clamp(s, 0, 30) * 200
...
/obj/item/integrated_circuit/manipulation/shocker/do_work()
	...
	if(shocktime + (2 SECONDS) > world.time)
...
/obj/item/integrated_circuit/manipulation/plant_module/do_work()
	...
	var/obj/structure/machinery/portable_atmospherics/hydroponics/TR = OM
	if(!istype(TR))
		push_data()
		activate_pin(2)
		return
