...
/obj/item/integrated_circuit/reagent/injector/proc/inject()
	...
	// Account for insert_slots
	AM = check_for_insert(AM)
...
/obj/item/integrated_circuit/reagent/pump/do_work()
	...
	// Account for insert_slots
	source = check_for_insert(source)
	target = check_for_insert(target)
...
/obj/item/integrated_circuit/reagent/filter/do_work()
	...
	// Account for possible insert_slots
	source = check_for_insert(source)
	target = check_for_insert(target)
...
/obj/item/integrated_circuit/reagent/temp/process()
	if(!active)
		return PROCESS_KILL

	last_temperature = reagents.temperature

	if(mode == IC_HEATER_MODE_HEAT && reagents.temperature < target_temp)
		reagents.temperature = min(reagents.temperature + heating_power, max_temp)
	else if(mode == IC_HEATER_MODE_COOL && reagents.temperature > target_temp)
		reagents.temperature = max(reagents.temperature - heating_power, min_temp)

	if(reagents.temperature != last_temperature)
		// Lost power
		if(!check_power())
			power_fail()
			return ..()
