...
/obj/item/integrated_circuit/output/screen/medium/do_work()
	...
	for(var/mob/M in nearby_things)
		...
		to_chat(M, "<span class='notice'>[hicon(O)] [stuff_to_display]</span>")
...
/obj/item/integrated_circuit/output/screen/large/do_work()
	...
	O.visible_message("<span class='notice'>[hicon(O)] [stuff_to_display]</span>")
...
/obj/item/integrated_circuit/output/text_to_speech/do_work()
	...
	if(!isnull(text))
		var/obj/O = assembly ? loc : assembly
		audible_message("[hicon(O)] \The [O.name] states, \"[text]\"")
