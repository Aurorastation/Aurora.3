	...
	if(decal)
		...
		if(!mecha_icon_cache[decal_key])
			...
			if(!mecha_icon_cache[template_key])
				...
				mecha_icon_cache[template_key] = template

			var/icon/decal_icon = icon('icons/mecha/mecha_decals.dmi',decal)
			decal_icon.Blend(mecha_icon_cache[template_key], ICON_MULTIPLY)
