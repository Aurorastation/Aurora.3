...
/obj/structure/table/attackby(obj/item/W, mob/user)
	...
	if(carpeted && W.iscrowbar())
		user.visible_message("<b>[user]</b> removes the carpet from \the [src].",
	...
	if(!carpeted && material && istype(W, /obj/item/stack/tile/carpet))
		...
		if(C.use(1))
			user.visible_message("<b>[user]</b> adds \the [C] to \the [src].",
	...
	if(health < maxhealth && W.iswelder())
		...
		if(F.welding)
			...
			if(!do_after(user, 20/W.toolspeed) || !F.remove_fuel(1, user))
				return
			user.visible_message("<b>[user]</b> repairs some damage to \the [src].",
...
/obj/structure/table/proc/common_material_add(obj/item/stack/material/S, mob/user, verb) // Verb is actually verb without 'e' or 'ing', which is added. Works for 'plate'/'plating' and 'reinforce'/'reinforcing'.
	...
	if(!do_after(user, 20) || !S.use(1))
		manipulating = 0
		return null
	user.visible_message("<b>[user]</b> [verb]es \the [src] with [M.display_name].", SPAN_NOTICE("You finish [verb]ing \the [src]."))
	manipulating = 0
	return M
...
/obj/structure/table/proc/common_material_remove(mob/user, material/M, delay, what, type_holding, sound)
	...
	if(manipulating) return M
	manipulating = 1
	user.visible_message("<b>[user]</b> begins removing the [type_holding] holding \the [src]'s [M.display_name] [what] in place.",
	...
	if(!do_after(user, 40))
		manipulating = 0
		return M
	user.visible_message("<b>[user]</b> removes the [M.display_name] [what] from \the [src].",
...
/obj/structure/table/proc/dismantle(obj/item/wrench/W, mob/user)
	...
	if(!do_after(user, 20/W.toolspeed))
		manipulating = 0
		return
	user.visible_message("<b>[user]</b> dismantles \the [src].",
