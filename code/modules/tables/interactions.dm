		...
		if (istype(G.affecting, /mob/living))
			...
			if (G.state < GRAB_AGGRESSIVE)
				if(user.a_intent == I_HURT)
					var/blocked = M.run_armor_check(TARGET_HEAD, "melee")
					if (prob(30 * BLOCKED_MULT(blocked)))
						M.Weaken(5)
					M.apply_damage(8, BRUTE, TARGET_HEAD, blocked)
					...
					for(var/obj/item/weapon/material/shard/S in L)
						if(prob(50))
							...
							M.apply_damage(10, BRUTE, TARGET_HEAD, blocked)
							M.standard_weapon_hit_effects(S, G.assailant, 10, blocked, TARGET_HEAD)
				else
