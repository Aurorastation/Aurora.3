...
/obj/machinery/keycard_auth/proc/broadcast_request()
	...
	if(confirmed)
		confirmed = 0
		trigger_event(event)
		log_game(text="[key_name(event_triggered_by)] triggered and [key_name(event_confirmed_by)] confirmed event [event]",ckey=key_name(event_triggered_by),ckey_target=key_name(event_confirmed_by))
