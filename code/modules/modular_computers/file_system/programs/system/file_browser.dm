...
/datum/computer_file/program/filemanager/Topic(href, href_list)
	...
	if(href_list["PRG_edit"])
		...
		if(!F || !istype(F))
			return 1
		if(F.do_not_edit && (alert("WARNING: This file is not compatible with editor. Editing it may result in permanently corrupted formatting or damaged data consistency. Edit anyway?", "Incompatible File", "Edit", "Cancel") == "Cancel"))
			return 1
