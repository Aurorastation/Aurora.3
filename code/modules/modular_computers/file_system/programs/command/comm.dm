...
/datum/computer_file/program/comm
	...
	extended_desc = "Used to command and control the ship. Can relay long-range communications."
		...
		if("announce")
			if(is_authenticated(user) && !issilicon(usr) && ntn_comm)
				...
				if(announcement_cooldown)
					...
					return
				var/input = input(usr, "Please write a message to announce to the ship crew.", "Priority Announcement") as null|message
