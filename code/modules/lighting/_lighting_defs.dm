...
// Simplified version of LUM_FALLOFF that only applies height falloff.
#define HEIGHT_FALLOFF(HEIGHT) (1 - CLAMP01(sqrt(HEIGHT) / max(1, light_range)))
...
#define APPLY_CORNER_SIMPLE(C,hdiff) \
	. = HEIGHT_FALLOFF(hdiff) * light_power; \
...
// Similar to above, but does not calculate lighting falloff. Used by sunlight.
#define APPLY_CORNER_BY_HEIGHT_SIMPLE \
	if (C.z != Sz) {                                      \
		corner_height = CALCULATE_CORNER_HEIGHT(C.z, Sz); \
	}                                                     \
	else {                                                \
	...
	APPLY_CORNER_SIMPLE(C, corner_height);
