...
/proc/fire_flicker_middleman(datum/light_middleman/middleman)
	...
	for(var/i in 1 to 17)
		...
		var/random_down = light_middleman_flicker_jitter(0.075 SECONDS)
		var/random_bottom = light_middleman_flicker_jitter(0.05 SECONDS)
		var/random_up = light_middleman_flicker_jitter(0.075 SECONDS)
		...
		var/random_top = light_middleman_flicker_jitter(0.225 SECONDS)
...
/proc/light_middleman_flicker_jitter(max_offset)
	var/scaled_offset = round(max_offset * 1000)
	return rand(-scaled_offset, scaled_offset) / 1000
