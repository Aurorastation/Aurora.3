...
/datum/lighting_corner/proc/display(max_lum)
	...
	var/mutable_appearance/display = mutable_appearance('icons/effects/alphacolors.dmi', "white", LIGHT_DEBUG_LAYER, draw_to, BALLOON_CHAT_PLANE)
	if(x > draw_to.x)
		display.pixel_w = 16
	else
		display.pixel_w = -16
