...
/datum/parsed_map/proc/_tgm_load(x_offset, y_offset, z_offset, crop_map, no_changeturf, x_lower, x_upper, y_lower, y_upper, z_lower, z_upper, place_on_top, new_z)
	...
	if(bad_paths.len)
#ifdef TESTING
		CRASH("Non-existent paths in map file [original_path]! [english_list(bad_paths)]")
#else
		stack_trace("Non-existent paths in map file [original_path]! [english_list(bad_paths)]")
#endif
