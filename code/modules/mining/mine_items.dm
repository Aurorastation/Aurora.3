...
/obj/item/extraction_pack/afterattack(atom/movable/A, mob/living/carbon/human/user)
	...
	if(!istype(A))
		...
		if(do_after(user, 50 - 50 * skill_time_reduction("prospecting", 0.1, user)))
...
/obj/item/warp_core/attack_self(mob/user)
	...
	if(do_after(user, 15 - 15 * (skill_time_reduction("devices", 0.1, user) + skill_time_reduction("prospecting", 0.05, user))))
...
/obj/structure/sculpting_block/attackby(obj/item/C as obj, mob/user as mob)
	...
	if (istype(C, /obj/item/autochisel))
		if(!sculpted)
			...
			last_struck = 1
			if(do_after(user, 20 - 20 * skill_time_reduction("prospecting", 0.05, user)))
				last_struck = 0
				if(times_carved <= 9)
					times_carved += 1
					if(times_carved < 1)
