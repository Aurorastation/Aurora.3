		...
		if(use_cell_power())
			if(!supported && !panel_open)
				...
				if(!supported) //if you can resolve it manually in three seconds then power to you good-sir.
					visible_message(SPAN_NOTICE("[icon2html(src, usr)] [src.name] beeps, \"Unbraced drill error automatically corrected. Please brace your drill.\""))
				else
					visible_message(SPAN_NOTICE("[icon2html(src, usr)] [src.name] beeps, \"Unbraced drill error manually resolved. Operations may resume normally.\""))
			if(supported && panel_open)
				if(cell)
					...
					if(panel_open)
						...
						visible_message(SPAN_NOTICE("[icon2html(src, usr)] [src.name] beeps, \"Unsealed cell fitting error manually resolved. Operations may resume normally.\""))
		else
			to_chat(user, SPAN_NOTICE("\The [src] is unpowered."))
	update_icon()
...
/obj/machinery/mining/drill/proc/system_error(var/error)
	if(error)
		visible_message(SPAN_WARNING("[icon2html(src, usr)] <b>[capitalize_first_letters(src.name)]</b> flashes a system warning: \"[error]\"."))
		current_error = error
