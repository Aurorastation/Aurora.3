...
/turf/simulated/mineral/attackby(obj/item/W as obj, mob/user as mob)
	...
	if (istype(W, /obj/item/device/measuring_tape))
		...
		if(do_after(user, 25 - 25 * skill_time_reduction("prospecting", 0.1, user)))
	...
	if (istype(W, /obj/item/pickaxe) && W.simulated)	// Pickaxe offhand is not simulated.
		...
		if(do_after(user, P.digspeed - P.digspeed * skill_time_reduction("prospecting", 0.1, user)))
			if (!istype(src, /turf/simulated/mineral))
				return
	...
	if (istype(W, /obj/item/autochisel))
		...
		if(!do_after(user, 80/W.toolspeed - 80/W.toolspeed * skill_time_reduction("prospecting", 0.1, user)))
			return
...
/turf/unsimulated/floor/asteroid/attackby(obj/item/W as obj, mob/user as mob)
	...
	if(is_type_in_typecache(W, usable_tools))
		...
		if(dug)
			...
			digging = 1
			if(!do_after(user, 60/W.toolspeed - 60/W.toolspeed * skill_time_reduction("prospecting", 0.1, user)))
		...
		digging = 1
		if(!do_after(user, 40 - 40 * skill_time_reduction("prospecting", 0.1, user)))
			if (istype(src, /turf/unsimulated/floor/asteroid))
				digging = 0
			return
