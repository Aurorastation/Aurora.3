...
/client/proc/delete_random_map()
	...
	if(istype(M))
		...
		log_admin(text="[key_name(usr)] has deleted [M.name].",admin_key=key_name(usr))
		qdel(M)
...
/client/proc/create_random_map()
	...
	if(M)
		...
		log_admin(text="[key_name(usr)] has created [M.name].",admin_key=key_name(usr))
...
/client/proc/apply_random_map()
	...
	if(istype(M))
		...
		log_admin(text="[key_name(usr)] has applied [M.name] at x[tx],y[ty],z[tz].",admin_key=key_name(usr))
...
/client/proc/overlay_random_map()
	...
	if(istype(base_map) && istype(overlay_map))
		...
		log_admin(text="[key_name(usr)] has applied [overlay_map.name] to [base_map.name] at x[tx],y[ty].",admin_key=key_name(usr))
