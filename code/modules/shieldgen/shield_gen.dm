...
/obj/machinery/shield_gen/proc/get_shielded_turfs()
	...
	if(parent_matrix.multi_z)
		...
		if(above)
			for(var/turf/turf_z as anything in above)
				LAZYADD(connected_levels, turf_z)
		if(below)
			for(var/turf/turf_z as anything in below)
				LAZYADD(connected_levels, turf_z)
	for(var/turf/turf_z in connected_levels)
		for (var/turf_tt in RANGE_TURFS(parent_matrix.field_radius, turf_z))
			turf_t = turf_tt
			// If we are directional, ignore turfs in the wrong direction
			if(directional && !(get_cardinal_dir(parent_matrix, turf_t) == dir))
				continue
			if(parent_matrix.has_modulator(MODEFLAG_HULL))
				// Ignore station areas if on hull mode.
				if ((is_station_area(turf_t.loc) || is_shuttle_area(turf_t.loc)))
					continue
				if (istype(turf_t, /turf/space) || isopenturf(turf_t) || istype(turf_t, /turf/simulated/floor/reinforced))
					for (var/turf_uu in RANGE_TURFS(1, turf_t))
						...
						if (is_station_area(turf_u.loc) || istype(turf_u, /turf/simulated/mineral/surface))
							...
							break
				continue
			if(get_dist(parent_matrix, turf_t) == parent_matrix.field_radius)
				LAZYADD(out, turf_t)
				continue
