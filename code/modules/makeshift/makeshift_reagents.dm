...
/obj/structure/chemkit/attackby(obj/item/W, mob/user)
	...
	if(istype(W) && W.force >= 5 && !has_edge(W) && LAZYLEN(contents - analyzer))
		...
		if(!do_after(user, 15 SECONDS - 15 SECONDS * skill_time_reduction("chemistry", 0.15, user)))
			return
		smash(smashed, user)
		return
	if(has_edge(W) && LAZYLEN(contents - analyzer))
		...
		if(!chopped.reagents)
			return
		if(!do_after(user, 5 SECONDS - 5 SECONDS * skill_time_reduction("chemistry", 0.1, user)))
			return
		chop(chopped, user)
		return
