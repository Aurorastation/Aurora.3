...
/obj/structure/chemkit/verb/phase_filter()
	...
	if(!usr.canmove || usr.stat || usr.restrained())
		return 0
	phase_filter = input("Which phase do you want to filter?", "Phase Filter", null) as null|anything in list("solid", "liquid", "gas")
...
/obj/structure/chemkit/attackby(obj/item/weapon/W, mob/user)
	...
	if(istype(W) && W.force >= 5 && !has_edge(W))
	...
	if(!analyzer && istype(W, /obj/item/device/analyzer))
		...
		W.forceMove(src)
		return
	if(!istype(W, /obj/item/weapon/reagent_containers/glass) && (W.w_class <= 3.0) && (W.reagents || sheet_reagents[W.type]))
		user.drop_from_inventory(W)
		W.forceMove(src)
		src.contents += W
		return
