	...
	if(N)
		...
		for(var/V in N.normal_members)
			...
			if(istype(V, /obj/machinery/atmospherics/unary/vent_pump) && vent.loc && !vent.is_welded())
				...
				R.my_atom = vent
				R.add_reagent(/datum/reagent/blood, reagent_amount, list("blood_colour" = blood_color))
	...
	if(target_move)
		if(is_type_in_list(target_move, ventcrawl_machinery) && target_move.can_crawl_through())
			if(target_move:is_welded())
				user.audible_message(SPAN_WARNING("You hear something banging on [target_move]!"), SPAN_NOTICE("You can't escape from a welded vent."))
			else
				user.remove_ventcrawl()
				user.forceMove(target_move.loc) //handles entering and so on
				user.sight &= ~(SEE_TURFS|BLIND)
				user.audible_message(SPAN_WARNING("You hear something squeezing through the ducts."), "You climb out the ventilation system.")
		...
		if((direction & initialize_directions) || is_type_in_list(src, ventcrawl_machinery) && src.can_crawl_through()) //if we move in a way the pipe can connect, but doesn't - or we're in a vent
			...
			user.sight &= ~(SEE_TURFS|BLIND)
			user.audible_message(SPAN_WARNING("You hear something squeezing through the pipes."), "You climb out of the ventilation system.")
