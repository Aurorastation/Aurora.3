	...
	switch (status)
		if ("departing")
			for (var/obj/item/device/assembly/mousetrap/S in location.loc)
				if (prob(25))
					visible_message(SPAN_DANGER("[src] gets caught in the mousetrap while trying to crawl into the vent!"), SPAN_DANGER("You get caught in the mousetrap while trying to crawl into the vent!"))
		...
		if(!lying)
			...
			if(vent_found)
				if(vent_found.network && (vent_found.network.normal_members.len || vent_found.network.line_members.len))
					...
					visible_message(SPAN_WARNING("[src] begins to climb into the ventilation system!"),SPAN_NOTICE("You begin climbing into the ventilation system..."))
