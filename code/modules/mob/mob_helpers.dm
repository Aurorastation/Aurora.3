...
/proc/broadcast_hud_message(var/message, var/broadcast_source, var/list/targets, var/icon)
	...
	for(var/mob/M in targets)
		...
		if(targetturf && (targetturf.z == sourceturf.z))
			M.show_message("<span class='info'>[icon2html(icon, usr)] [message]</span>", 1)
...
/mob/proc/get_accent_icon(var/datum/language/speaking = null, var/force_accent)
	...
	if(used_accent && speaking?.allow_accents)
		...
		return icon2html(icon('./icons/accent_tags.dmi', final_icon), src, realsize=TRUE, class="text_tag")