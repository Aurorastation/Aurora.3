...
/obj/item/grab/proc/inspect_organ(mob/living/carbon/human/H, mob/user, var/target_zone)
	...
	if(!do_mob(user,H, 10 - 10 * (skill_time_reduction("anatomy", 0.1, user) + skill_time_reduction("medical", 0.05, user))))
	...
	if(!do_mob(user, H, 20 - 20 * (skill_time_reduction("anatomy", 0.1, user) + skill_time_reduction("medical", 0.05, user))))
...
/obj/item/grab/proc/pin_down(mob/target, mob/attacker)
	...
	if(do_after(attacker, 20 - 20 * skill_time_reduction("combat", 0.1, attacker)) && target)
