...
/mob/proc/update_Login_details()
	...
	log_access(text="Login: [key_name(src)] from [lastKnownIP ? lastKnownIP : "localhost"]-[computer_id] || BYOND v[client.byond_version]",ckey=key_name(src))
			...
			if( M.key && (M.key != key) )
				...
				if(matches)
					if(M.client)
						...
						log_access(text="Notice: [key_name(src)] has the same [matches] as [key_name(M)].",ckey=key_name(src))
					else
						...
						log_access(text="Notice: [key_name(src)] has the same [matches] as [key_name(M)] (no longer logged in).",ckey=key_name(src))
