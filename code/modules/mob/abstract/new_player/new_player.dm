...
/mob/abstract/new_player/proc/AttemptLateSpawn(rank,var/spawning_at)
	...
	if(!IsJobAvailable(rank))
		...
		return 0
	if(!(spawning_at in SSmapping.current_map.allowed_spawns))
		to_chat(usr, SPAN_NOTICE("Spawn location [spawning_at] invalid for [SSmapping.current_map]. Defaulting to [SSmapping.current_map.default_spawn]."))
		spawning_at = SSmapping.current_map.default_spawn
...
/mob/abstract/new_player/proc/AnnounceCyborg(var/mob/living/character, var/rank, var/join_message)
	if (SSticker.current_state == GAME_STATE_PLAYING)
		...
		GLOB.global_announcer.autosay("A new[rank ? " [rank]" : " visitor" ] [join_message ? join_message : "has arrived on the [SSmapping.current_map.station_type]"].", "Arrivals Announcer")
