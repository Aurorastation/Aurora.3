...
/mob/abstract/new_player/Topic(href, href_list[])
	...
	if(href_list["late_join"])
		...
		if(!check_rights(R_ADMIN, 0))
			...
			if(!(S.spawn_flags & CAN_JOIN))
				to_chat(usr, "<span class='danger'>Your current species, [client.prefs.species], is not available for play on the [current_map.station_type].</span>")
				return 0
	...
	if(href_list["SelectedJob"])
		...
		if(!config.enter_allowed)
			...
			to_chat(usr, "<span class='danger'>The [current_map.station_type] is currently exploding. Joining would go poorly.</span>")
			return
...
/mob/abstract/new_player/proc/AnnounceCyborg(var/mob/living/character, var/rank, var/join_message)
	if (SSticker.current_state == GAME_STATE_PLAYING)
		...
		global_announcer.autosay("A new[rank ? " [rank]" : " visitor" ] [join_message ? join_message : "has arrived on the [current_map.station_type]"].", "Arrivals Announcement Computer")
