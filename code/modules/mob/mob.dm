	...
	if(.)
		...
		if(client.holder)
			if(statpanel("Status"))
				...
				stat("World Size:", "X: [world.maxx], Y: [world.maxy], Z: [world.maxz]")
				if (client.selected_template)
					stat("Currently selected template:", client.selected_template)
					stat("Load Mode:", loadmode2text(client.template_load_mode))
					if (client.template_load_mode != LOADMODE_NEWZ)
						stat("Target:", client.template_target || "(None)")
						stat("Preview:", LAZYLEN(client.template_markers) ? "Visible" : "Hidden")
