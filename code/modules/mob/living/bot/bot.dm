	...
	if(O.GetID())
		...
		if(health < maxHealth)
			if(open)
				...
				user.visible_message("<b>[user]</b> repairs [src].", SPAN_NOTICE("You repair [src]."))
			else
		...
		user.visible_message("<b>[user]</b> pries \the [pAI.pai] out of \the [src].", SPAN_NOTICE("You pry \the [pAI.pai] out of \the [src]."))
		pAI = null
		...
		pAI = P
		user.visible_message("<b>[user]</b> places \the [pAI.pai] into \the [src].", SPAN_NOTICE("You place \the [O] into \the [src]."))
		old_name = src.name
		name = pAI.pai.name
	else
		..()
