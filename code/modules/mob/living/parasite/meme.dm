...
/mob/living/parasite/meme/verb/Thought(mob/M as mob in oview())
	...
	if(message)
		log_say(text="MemeThought: [key_name(src)]->[M.key] : [message]",ckey=key_name(src))
		M << "[message]"
...
/mob/living/parasite/meme/verb/SubtleJump(mob/living/carbon/human/target as mob in oview())
	...
	log_admin(text="[src.key] has jumped to [target]",ckey=key_name(src))
...
/mob/living/parasite/meme/verb/Attune()
	...
	log_admin(text="[src.key] has attuned [host]",ckey=key_name(src))
