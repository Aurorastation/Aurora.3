...
/mob/living/silicon/robot/proc/show_cell_power()
	if(cell)
		...
		stat(null, "Cell Rating: [round(cell.maxcharge)]") // Round just in case we somehow get crazy values
...
/mob/living/silicon/robot/attackby(obj/item/W, mob/user)
	...
	if(user.a_intent == I_HELP)
		if(W.iswelder())
			...
			if(opened)
				if(cell)
					user.visible_message("<b>[user]</b> begins clasping shut \the [src]'s maintenance hatch.", SPAN_NOTICE("You begin closing up \the [src]'s maintenance hatch."))
					...
					if(!mmi)
						...
						return
					user.visible_message("<b>[user]</b> begins ripping \the [mmi] from \the [src].", SPAN_NOTICE("You jam the crowbar into the robot and begin levering out \the [mmi]."))
					if(do_after(user, 50 / W.toolspeed, act_target = src))
				...
				if(locked)
					...
					user.visible_message("<b>[user]</b> begins prying open \the [src]'s maintenance hatch.", SPAN_NOTICE("You start opening \the [src]'s maintenance hatch."))
					if(do_after(user, 50 / W.toolspeed, act_target = src))
						if(!Adjacent(user))
			...
			user.visible_message("<b>[user]</b> [wires_exposed ? "exposes" : "covers"] \the [src]'s wires.", SPAN_NOTICE("You [wires_exposed ? "expose" : "cover"] \the [src]'s wires."))
			handle_panel_overlay()
