...
/mob/living/silicon/robot/drone/construction/process_level_restrictions()
	...
	if(!self_destructing)
		to_chat(src, SPAN_DANGER("WARNING: Removal from [SSmapping.current_map.company_name] property detected. Anti-Theft mode activated."))
...
/mob/living/silicon/robot/drone/construction/matriarch/Initialize()
	...
	matrix_tag = SSmapping.current_map.station_short
...
/mob/living/silicon/robot/drone/init()
	...
	flavor_text = replacetext(desc_flavor, "%MAPNAME%", SSmapping.current_map.company_name)
...
/mob/living/silicon/robot/drone/process_level_restrictions()
	...
	if((!T || !(A in GLOB.the_station_areas)) && src.stat != DEAD)
		if(!self_destructing)
			to_chat(src, SPAN_WARNING("WARNING: Removal from [SSmapping.current_map.company_name] property detected. Anti-Theft mode activated."))
			start_self_destruct(TRUE)
		return TRUE
