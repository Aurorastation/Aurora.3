...
/mob/living/silicon/robot/drone/do_possession(var/mob/dead/observer/possessor)
	...
	log_admin(text="[key_name(possessor)] took control of \the [src].",ckey=key_name(possessor))
...
/mob/living/silicon/robot/drone/emag_act(var/remaining_charges, var/mob/user)
	...
	log_game(text="[key_name(user)] emagged drone [key_name(src)].  Laws overridden.",ckey=key_name(user),ckey_target=key_name(src))
