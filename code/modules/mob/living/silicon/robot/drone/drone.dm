	...
	if(user.a_intent == "help" && istype(W, /obj/item/clothing/head))
		...
		if(stat == DEAD)
			...
			if(!allowed(usr))
				...
				return
			user.visible_message("<b>[user]</b> swipes \his ID card through \the [src], attempting to reboot it.", SPAN_NOTICE("You swipe your ID card through \the [src], attempting to reboot it."))
			request_player()
			return
		else
