...
/obj/machinery/computer/drone_control/attack_hand(var/mob/user as mob)
	...
	for(var/mob/living/silicon/robot/drone/D in silicon_mob_list)
		if(isStationLevel(src.z) && isNotStationLevel(D.z)) //If the console is on a station level, then list all drones on the station levels
			...
			continue
		dat += "<BR>[D.real_name] ([D.stat == 2 ? "<span class='warning'>INACTIVE</span>" : "<span class='good'>ACTIVE</span>"])"
	...
	dat += "[dronefab ? "<A href='?src=\ref[src];toggle_fab=1'>[(dronefab.produce_drones && !(dronefab.stat & NOPOWER)) ? "ACTIVE" : "INACTIVE"]</A>" : "<span class='warning'><b>FABRICATOR NOT DETECTED.</b></span> (<A href='?src=\ref[src];search_fab=1'>search</a>)"]"
