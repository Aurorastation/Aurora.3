...
/datum/component/robotics_analyzer/proc/robot_scan(mob/living/silicon/robot/robot)
	...
	status_results += "Damage: <font color='#FFA500'>[get_robot_severity(robot.getFireLoss())] electronics</font>, [SPAN_WARNING("[get_robot_severity(robot.getBruteLoss())] physical damage")]"
	...
	if(length(damaged))
		for(var/datum/robot_component/component in damaged)
			...
			var/damage = "<font color='#FFA500'>[component.electronics_damage] electronics</font>, [SPAN_WARNING("[component.brute_damage] physical")]; [component_state]"
...
/datum/component/robotics_analyzer/proc/prosthetics_scan(mob/living/carbon/human/human)
	...
	for(var/obj/item/organ/external/external_organ in human.organs)
		if(!(external_organ.status & (ORGAN_ROBOT | ORGAN_ASSISTED)))
			continue
		var/damage = "[SPAN_WARNING("[get_robot_severity(LIMB_GET_BRUTE_DAMAGE(external_organ))] wiring")], <font color='#FFA500'>[get_robot_severity(LIMB_GET_BURN_DAMAGE(external_organ))] electronics</font>"
