...
/mob/living/silicon/pai/attack_hand(var/mob/living/carbon/human/M)
	..()
	switch(M.a_intent)
		if(I_HELP)
			M.visible_message(span("notice","[M] [response_help] \the [src]"))
		if(I_DISARM)
			M.visible_message(span("warning", "[M.name] boops [src] on the head."))
...
/mob/living/silicon/pai/proc/greet()
	if(!greeted)
		// Basic intro text.
		to_chat(src, span("danger", "<font size=3>You are a Personal AI!</font>"))
		to_chat(src, span("notice", "You are a small artificial intelligence contained inside a portable tablet, and you are bound to a master. Your primary directive is to serve them and follow their instructions, follow this prime directive above all others. Check your Software interface to spend ram on programs that can help, and unfold your chassis to take a holographic form and move around the world."))
		greeted = TRUE
...
/mob/living/silicon/pai/verb/reset_record_view()
	...
	active = null
	security_cannotfind = FALSE
	medical_cannotfind = FALSE
	SSnanoui.update_uis(src)
	to_chat(usr, span("notice", "You reset your record-viewing software."))
...
/mob/living/silicon/pai/verb/fold_out()
	...
	if(world.time <= last_special)
		to_chat(src, span("warning", "Your actuators are still warming up."))
		return
	...
	if(istype(card.loc,/obj/item/rig_module))
		to_chat(src, span("warning", "There is no room to unfold inside this rig module. You're good and stuck."))
		return FALSE
		...
		if(ishuman(holder))
			...
			for(var/obj/item/organ/external/affecting in H.organs)
				if(card in affecting.implants)
					...
					H.visible_message(span("danger", "\The [src] explodes out of \the [H]'s [affecting.name] in shower of gore!"))
					break
...
/mob/living/silicon/pai/AltClick(var/mob/user)
...
//I'm not sure how much of this is necessary, but I would rather avoid issues.
/mob/living/silicon/pai/proc/close_up(var/retry = FALSE)
	if(src.loc == card)
		return
	...
	// Something went wrong when we folded up, try again
	if(src.loc != card)
		src.forceMove(card)
		card.forceMove(get_turf(card))
