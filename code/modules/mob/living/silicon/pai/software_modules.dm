	...
	if(href_list["getdna"])
		...
		while(!istype(M, /mob/living))
			if(!M || !M.loc || count > 6)
				...
				to_chat(src, span("notice", "You are not being carried by anyone!"))
				return FALSE
			M = M.loc
			count++
		...
		if(answer == "Yes")
			...
			if(dna.unique_enzymes == P.master_dna)
				to_chat(P, span("good", "DNA is a match to stored Master DNA."))
			else
...
/datum/pai_software/messenger/on_ui_interact(mob/living/silicon/pai/user, datum/nanoui/ui=null, force_open=1)
	...
	if(!user.pda.toff)
		for(var/obj/item/device/pda/P in sortAtom(PDAs))
			if(!P.owner || P.toff || P == user.pda || P.hidden)
				continue
			var/pda[0]
			pda["name"] = "[P]"
			pda["owner"] = "[P.owner]"
