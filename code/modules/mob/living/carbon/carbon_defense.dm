...
/mob/living/carbon/hitby(atom/movable/AM as mob|obj,var/speed = THROWFORCE_SPEED_DIVISOR)
	...
	if(H && show_ssd && !client && !teleop)
		if(H.bg)
			visible_message(span("danger", "[src] is hit by [AM] waking [gender_datums[src.gender].him] up!"))
			...
			visible_message(span("danger","[src] is hit by [AM], but [gender_datums[src.gender].he] [gender_datums[src.gender].does] not respond... Maybe [gender_datums[src.gender].he] [gender_datums[src.gender].has] S.S.D?"))
...
/mob/living/carbon/bullet_act(var/obj/item/projectile/P, var/def_zone)
	...
	if(H && show_ssd && !client && !teleop)
		if(H.bg)
			visible_message("<span class='danger'>[P] hits [src], waking [gender_datums[src.gender].him] up!</span>")
			if(H.health / H.maxHealth < 0.5)
				H.bg.awaken_impl(TRUE)
				sleeping = 0
				willfully_sleeping = 0
			...
			visible_message("<span class='danger'>[P] hits [src], but [gender_datums[src.gender].he] [gender_datums[src.gender].does] not respond... Maybe [gender_datums[src.gender].he] [gender_datums[src.gender].has] S.S.D?</span>")
	else if(client && willfully_sleeping)
		visible_message("<span class='danger'>[P] hits [src], waking [gender_datums[src.gender].him] up!</span>")
		sleeping = 0
		willfully_sleeping = 0
...
/mob/living/carbon/standard_weapon_hit_effects(obj/item/I, mob/living/user, var/effective_force, var/blocked, var/hit_zone)
	...
	if(H && show_ssd && !client && !teleop)
		if(H.bg)
			...
			user.visible_message("<span class='danger'>[user] attacks [src] with [I], waking [gender_datums[src.gender].him] up!</span>", \
					    "<span class='danger'>You attack [src] with [I], but [gender_datums[src.gender].he] [gender_datums[src.gender].does] not respond... Maybe [gender_datums[src.gender].he] [gender_datums[src.gender].has] S.S.D?</span>")
	else if(client && willfully_sleeping)
		user.visible_message("<span class='danger'>[user] attacks [src] with [I], waking [gender_datums[src.gender].him] up!</span>", \
				    "<span class='danger'>You attack [src] with [I], waking [gender_datums[src.gender].him] up!</span>")
		sleeping = 0
		willfully_sleeping = 0
