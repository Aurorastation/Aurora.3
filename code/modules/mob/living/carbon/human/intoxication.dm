...
/mob/living/carbon/human/proc/handle_intoxication()
	...
	if(bac > INTOX_VOMIT*SR)//Vomiting starts here. 1% chance every 10 ticks, escalating with increased intoxication.
	...
	if(bac > INTOX_BALANCE*SR)//Can no longer walk properly. Will fall to the ground very often.
		...
		if (life_tick % 4 == 1 && !lying && !buckled_to && prob(10))
			...
			Paralyse(3 SECONDS)
			if(prob(10) && stat == CONSCIOUS && !src.reagents.has_reagent(/singleton/reagent/ethylredoxrazine))
	...
	if(bac > INTOX_BLACKOUT*SR && bac < INTOX_CONSCIOUS*SR && !src.reagents.has_reagent(/singleton/reagent/ethylredoxrazine))//Blacking out.
	...
	if(bac > INTOX_CONSCIOUS*SR && !src.reagents.has_reagent(/singleton/reagent/ethylredoxrazine))//Now in a coma.
		if (life_tick % 4 == 1 && prob(10) && stat == CONSCIOUS)
