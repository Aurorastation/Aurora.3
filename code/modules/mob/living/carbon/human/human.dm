	...
	if (href_list["secrecordadd"])
		if(hasHUD(usr,"security"))
			...
			if(istype(R) && istype(R.security))
				...
				if(istype(usr,/mob/living/carbon/human))
					var/mob/living/carbon/human/U = usr
					R.security.comments += "Made by [U.get_authentification_name()] ([U.get_assignment()]) on [time2text(world.realtime, "DDD MMM DD hh:mm:ss")], [game_year]<BR>[t1]"
				if(istype(usr,/mob/living/silicon/robot))
					var/mob/living/silicon/robot/U = usr
					R.security.comments += "Made by [U.name] ([U.mod_type] [U.braintype]) on [time2text(world.realtime, "DDD MMM DD hh:mm:ss")], [game_year]<BR>[t1]"
	...
	if (href_list["medrecordadd"])
		if(hasHUD(usr,"medical"))
			...
			if(istype(R) && istype(R.medical))
				...
				if(ishuman(usr))
					var/mob/living/carbon/human/U = usr
					R.medical.comments += "Made by [U.get_authentification_name()] ([U.get_assignment()]) on [time2text(world.realtime, "DDD MMM DD hh:mm:ss")], [game_year]<BR>[t1]"
				if(isrobot(usr))
					var/mob/living/silicon/robot/U = usr
					R.medical.comments += "Made by [U.name] ([U.mod_type] [U.braintype]) on [time2text(world.realtime, "DDD MMM DD hh:mm:ss")], [game_year]<BR>[t1]"
...
/mob/living/carbon/human/verb/check_pulse()
	...
	if (src.species.flags & NO_BLOOD)
		to_chat(usr, SPAN_NOTICE(self ? "Your species does not have a pulse." : "[src]'s species does not have a pulse."))
		return
