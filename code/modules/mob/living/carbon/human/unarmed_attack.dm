...
/datum/unarmed_attack/proc/apply_effects(var/mob/living/carbon/human/user,var/mob/living/carbon/human/target,var/armour,var/attack_damage,var/zone)
	...
	if(attack_damage >= 5 && armour < 100 && !(target == user) && stun_chance <= attack_damage * 5) // 25% standard chance
		switch(zone) // strong punches can have effects depending on where they hit
			...
			if(BP_GROIN)
				if(pain_message)
					target.visible_message(SPAN_WARNING("[target] looks like [gender_datums[target.gender].he] [gender_datums[target.gender].is] in pain!"), SPAN_WARNING((target.gender=="female") ? "Oh god that hurt!" : "Oh no, not your [pick("testicles", "crown jewels", "clockweights", "family jewels", "marbles", "bean bags", "teabags", "sweetmeats", "goolies")]!"))
...
/datum/unarmed_attack/proc/handle_eye_attack(var/mob/living/carbon/human/user, var/mob/living/carbon/human/target)
	...
	user.visible_message(SPAN_DANGER("[user] presses [gender_datums[user.gender].his] [eye_attack_text] into [target]'s [eyes.name]!"))
		...
		switch(zone)
			if(BP_HEAD, BP_MOUTH, BP_EYES)
				...
				switch(attack_damage)
					if(1 to 2)
						user.visible_message(SPAN_DANGER("[user] slapped [target] across [gender_datums[target.gender].his] cheek!"))
					if(3 to 4)
					...
					if(5)
						user.visible_message(pick(
							...
							40; SPAN_DANGER("[user] smashed [gender_datums[user.gender].his] [pick(attack_noun)] into [target]'s face!"),
				...
				switch(attack_damage)
					...
					if(1 to 4)	user.visible_message(SPAN_DANGER("[user] [pick(attack_verb)] [target] in [gender_datums[target.gender].his] [organ]!"))
					if(5)
						user.visible_message(pick(
							50; SPAN_DANGER("[user] smashed [gender_datums[user.gender].his] [pick(attack_noun)] into [target]'s [organ]!"),
...
/datum/unarmed_attack/kick/show_attack(var/mob/living/carbon/human/user, var/mob/living/carbon/human/target, var/zone, var/attack_damage)
	...
	switch(attack_damage)
		...
		if(3 to 4)	user.visible_message(SPAN_DANGER("[user] [pick(attack_verb)] [target] in [gender_datums[target.gender].his] [organ]!"))
