...
/mob/living/carbon/human/proc/commune()
	...
	if (target.is_diona())
		to_chat(src,span("alium", "The creature's mind is incompatible, formless."))
		return
	...
	if (target.can_commune())
		to_chat(H,span("psychic", "You instinctively sense [src] sending their thoughts into your mind, hearing:") + text)
	else if(prob(25) && (target.mind && target.mind.assigned_role=="Chaplain"))
		to_chat(H,span("changeling", "You sense [src]'s thoughts enter your mind, whispering quietly:") + text)
	else
...
/mob/living/carbon/human/proc/quillboar(mob/target as mob in oview())
	...
	visible_message(SPAN_WARNING("<b>\The [src]</b> launches a spine-quill at [target]!"))
...
/mob/living/carbon/human/proc/crush()
	...
	if(last_special > world.time)
		to_chat(src, SPAN_WARNING("Your arms are still recovering!"))
		return
	...
	if(G.state < GRAB_NECK)
		to_chat(src, SPAN_WARNING("You must have a stronger grab to crush your prey!"))
		return
	...
	if(ishuman(G.affecting))
		...
		visible_message(SPAN_WARNING("<b>\The [src]</b> viciously crushes [affected] of [G.affecting] with its metallic arms!"))
		...
		visible_message(SPAN_WARNING("<b>\The [src]</b> viciously crushes [G.affecting]'s flesh with its metallic arms!"))
