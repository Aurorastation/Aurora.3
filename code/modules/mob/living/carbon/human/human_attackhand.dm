...
/mob/living/carbon/human/attack_hand(mob/living/carbon/M as mob)
			...
			if(G.cell)
				if(M.a_intent == I_HURT)//Stungloves. Any contact will stun the alien.
					if(G.cell.charge >= 2500)
						...
						msg_admin_attack(text="[key_name_admin(M)] stungloved [src.name] ([src.ckey]) (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[M.x];Y=[M.y];Z=[M.z]'>JMP</a>)",ckey=key_name(M),ckey_target=key_name(src))
	...
	switch(M.a_intent)
		...
		if(I_HURT)
			...
			msg_admin_attack(text="[key_name(H)] [miss_type ? (miss_type == 1 ? "has missed" : "was blocked by") : "has [pick(attack.attack_verb)]"] [key_name(src)] (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[H.x];Y=[H.y];Z=[H.z]'>JMP</a>)",ckey=key_name(H),ckey_target=key_name(src))
		...
		if(I_DISARM)
			...
			msg_admin_attack(text="[key_name(M)] disarmed [src.name] ([src.ckey]) (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[M.x];Y=[M.y];Z=[M.z]'>JMP</a>)",ckey=key_name(M),ckey_target=key_name(src))
