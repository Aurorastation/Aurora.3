	...
	switch(M.a_intent)
		if(I_HELP)
			if(H != src && istype(H) && (is_asystole() || (status_flags & FAKEDEATH) || failed_last_breath))
				...
				if(!do_after(H, cpr - cpr * skill_time_reduction("medical", 0.1, H), src))
					cpr_time = 1
					return
				cpr_time = 1
...
/mob/living/carbon/human/proc/grab_joint(var/mob/living/user, var/def_zone)
	...
	if(do_after(user, 100 - 100 * (skill_time_reduction("combat", 0.1, user) + skill_time_reduction("medical", 0.1, user))))
		organ.dislocate(1)
