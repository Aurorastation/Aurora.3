/mob/living/carbon/teshari/proc/handle_strip(var/slot_to_strip, var/mob/living/user)
	...
	if(isteshari(user))
		// If you're a teshari and don't have hands, you shouldn't be able to strip someone.
	...
	switch(slot_to_strip)
		...
		if("mask")
			...
			if(do_after(user,teshari_STRIP_DELAY, act_target = src))
	...
	if(!do_mob(user,src,teshari_STRIP_DELAY))
		return 0
...
/mob/living/carbon/teshari/proc/empty_pockets(var/mob/living/user)
	if(!r_store && !l_store)
...
/mob/living/carbon/teshari/proc/toggle_sensors(var/mob/living/user)
	var/obj/item/clothing/under/suit = w_uniform
	if(!suit)
...
// Remove all splints.
/mob/living/carbon/teshari/proc/remove_splints(var/mob/living/user)
...
/mob/living/carbon/teshari/proc/toggle_internals(var/mob/living/user)
	if(internal)
