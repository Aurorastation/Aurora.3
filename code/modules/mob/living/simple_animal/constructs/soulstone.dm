...
/obj/item/device/soulstone/attack(mob/living/carbon/human/M as mob, mob/user as mob)
	...
	user.attack_log += text("\[[time_stamp()]\] <span class='warning'>Used [src] to capture the soul of [M.name] ([M.ckey])</span>")
	msg_admin_attack("[user.name] ([user.ckey]) used [src] to capture the soul of [M.name] ([M.ckey]) (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[user.x];Y=[user.y];Z=[user.z]'>JMP</a>)",ckey=key_name(src),ckey_target=key_name(M))
