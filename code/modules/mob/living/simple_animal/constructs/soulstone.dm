...
/obj/item/soulstone/attack(mob/living/target_mob, mob/living/user, target_zone)
	...
	M.log_message("Has had their soul captured with [src.name] by [user.name] ([user.ckey])", LOG_VICTIM, log_globally = FALSE)
	user.log_message("Used the [src.name] to capture the soul of [M.name] ([M.ckey])", LOG_ATTACK, log_globally = FALSE)
