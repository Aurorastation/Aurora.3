...
/datum/shuttle/autodock/ferry/specops/launch(var/user)
	...
	if (istype(user, /obj/machinery/computer))
		...
		if(world.time <= reset_time)
			C.visible_message(SPAN_NOTICE("[current_map.boss_name] will not allow the Special Operations shuttle to launch yet."))
	...
	spawn(20)
		if (!location)	//just arrived home
			for(var/turf/T in get_area_turfs(shuttle_area))
				...
				to_chat(M, SPAN_DANGER("You have arrived at [current_map.boss_name]. Operation has ended!"))
		else	//just left for the station
			launch_mauraders()
			for(var/turf/T in get_area_turfs(shuttle_area))
				var/mob/M = locate(/mob) in T
				to_chat(M, SPAN_DANGER("You have arrived at [current_map.station_name]. Commence operation!"))
