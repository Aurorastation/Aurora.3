...
/obj/machinery/computer/shuttle_control/multi/ui_data(mob/user)
	...
	var/datum/shuttle/multi/shuttle = SSshuttle.shuttles[shuttle_tag]
	if(istype(shuttle))
		. += list(
...
/obj/machinery/computer/shuttle_control/multi/handle_topic_href(var/mob/user, var/datum/shuttle/multi/shuttle, var/action, var/list/params)
	if(action == "pick")
...
/obj/machinery/computer/shuttle_control/multi/antag/ui_data(mob/user)
	...
	var/datum/shuttle/multi/antag/shuttle = SSshuttle.shuttles[shuttle_tag]
	if(istype(shuttle))
		. += list(
			"cloaked" = shuttle.cloaked,
		)

/obj/machinery/computer/shuttle_control/multi/antag/handle_topic_href(var/mob/user, var/datum/shuttle/multi/antag/shuttle, var/action, var/list/params)
...
/obj/machinery/computer/shuttle_control/multi/can_move(var/datum/shuttle/shuttle, var/user)
	if(istype(shuttle, /datum/shuttle/multi/antag))
		var/datum/shuttle/multi/antag/our_shuttle = shuttle
		if(our_shuttle.returned)
			...
			if(our_shuttle.return_warning_cooldown < world.time)
				...
				tgui_alert(user, "If you return to base, you won't be able to return to [SSmapping.current_map.station_short]. Launch again if you're sure about this.", "Shuttle Control", list("Acknowledged."))
				return FALSE
	return ..()
