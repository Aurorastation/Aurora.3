...
/obj/item/reagent_containers/food/snacks/grown/proc/update_desc()
	...
	if(SSplants.product_descs["[seed.uid]"])
		...
		if(reagents.has_reagent(/datum/reagent/bicaridine) || reagents.has_reagent(/datum/reagent/dylovene) || reagents.has_reagent(/datum/reagent/butazoline))
		...
		if(seed.get_trait(TRAIT_PRODUCES_POWER) && W.iscoil())
			...
			if(seed.kitchen_tag == "pumpkin") // Ugggh these checks are awful.
				...
				if(istype(W,/obj/item/material/hatchet) && !isnull(seed.chems[/datum/reagent/woodpulp]))
					...
					qdel(src)
					return
				else if(!isnull(seed.chems[/datum/reagent/drink/carrotjuice]))
					to_chat(user, "You slice \the [src] into sticks.")
