			...
			if(10)
				if(prob(degree*2))
					...
					if(get_trait(TRAIT_BIOLUM))
						...
						if(prob(degree*2))
							set_trait(TRAIT_BIOLUM_COLOUR,get_random_colour(0,75,190))
							source_turf.visible_message("<b>[display_name]</b>'s glow <font color='[get_trait(TRAIT_BIOLUM_COLOUR)]'>changes colour</font>!")
					else
						source_turf.visible_message(SPAN_NOTICE("<b>[display_name]</b>'s glow dims..."))
			if(11)
				set_trait(TRAIT_TELEPORTING,1)
