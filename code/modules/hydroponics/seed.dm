		...
		if(!isnull(force_amount))
			...
			if(GET_SEED_TRAIT(src, TRAIT_YIELD) > -1)
				if(isnull(yield_mod) || yield_mod < 1)
					...
					// Yield modifier will scale with actual yield of the plant; that is to say, plants with higher base yields will be more significantly impacted by fertilizer effects.
