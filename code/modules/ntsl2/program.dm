...
/datum/ntsl_program/proc/cycle(var/amount)
	if(ntsl2.connected)
		ntsl2.send("execute", list(id = id, cycles = amount))

/datum/ntsl_program/proc/get_terminal()
	return ntsl2.send("get_buffer", list(id = id))

/datum/ntsl_program/proc/topic(var/message)
	if(ntsl2.connected)
		ntsl2.send("topic", list(id = id, topic = message))

/datum/ntsl_program/proc/kill()
	if(ntsl2.connected)
		ntsl2.send("remove", list(id = id))
		ntsl2.programs -= src
		qdel(src)
...
/datum/ntsl_program/proc/tc_message(var/datum/signal/signal)
	if(ntsl2.connected)
		...
		ntsl2.send("message", list(id = id, sig_ref = "\ref[signal]", signal = list2params(list(
