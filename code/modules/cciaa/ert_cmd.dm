/client/proc/spawn_ert_commander()
	...
	if(istype(mob, /mob/abstract/new_player))
		src << "<span class='warning'>You can't be in the lobby to join as a commander.</span>"
		return
	...
	if(mob.mind && mob.mind.special_role == "ERT Commander")
		src << "<span class='warning'>You are already an ERT Commander.</span>"
	...
	for (var/obj/effect/landmark/L in landmarks_list)
		if(L.name == "ERTCommander")
			...
			if(M.age < 33 || M.age > 60)
				src << "<span class='warning'>The age you selected was not in a valid range for a Commander.</span>"
				if(M.age < 33)
					M.age = 33
				else
					M.age = 60
			...
			M.equip_to_slot_or_del(new /obj/item/weapon/melee/telebaton(M), slot_l_store)
			...
			M.equip_to_slot_or_del(new /obj/item/clothing/accessory/holster/armpit(M.back), slot_in_backpack)
