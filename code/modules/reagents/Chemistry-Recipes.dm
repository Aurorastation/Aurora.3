	...
	if(container && !ismob(container))
		...
		if(mix_message)
			...
			for(var/mob/M in seen)
				M.show_message("<span class='notice'>[hicon(container)] [mix_message]</span>", 1)
		if(reaction_sound)
...
/datum/chemical_reaction/slime/on_reaction(var/datum/reagents/holder)
	...
	T.Uses--
	if(T.Uses <= 0)
		T.visible_message("[hicon(T)]<span class='notice'>\The [T]'s power is consumed in the reaction.</span>")
