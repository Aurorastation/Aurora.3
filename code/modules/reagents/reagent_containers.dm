...
/obj/item/reagent_containers/proc/standard_splash_mob(var/mob/user, var/mob/target)
	...
	target.log_message("Has been splashed with [name] by [user.name] ([user.ckey]). Reagents: [contained] [temperature_text].", LOG_VICTIM, log_globally = FALSE)
	user.log_message("Used the [name] to splash [target.name] ([target.key]). Reagents: [contained] [temperature_text].", LOG_ATTACK, log_globally = FALSE)
...
/obj/item/reagent_containers/proc/standard_feed_mob(var/mob/user, var/mob/living/target) // This goes into attack
	...
	if(target == user)
		...
		var/contained = reagentlist()
		target.log_message("Has been fed [name] by [user.name] ([user.ckey]). Reagents: [contained]", LOG_VICTIM, log_globally = FALSE)
		user.log_message("Fed [name] to [target.name] ([target.ckey]). Reagents: [contained]", LOG_ATTACK, log_globally = FALSE)
