...
/obj/item/reagent_containers/pill/attack(mob/living/target_mob, mob/living/user, target_zone)
	...
	if(target_mob == user)
		...
		target_mob.attack_log += text("\[[time_stamp()]\] <font color='orange'>Has been fed [name] by [key_name(user)] Reagents: [contained]</font>")
		user.attack_log += text("\[[time_stamp()]\] <span class='warning'>Fed [name] to [key_name(target_mob)] Reagents: [contained]</span>")
