...
/obj/item/weapon/reagent_containers/pill
	...
	attack(mob/M as mob, mob/user as mob, def_zone)
		...
		if(M == user)
			...
			msg_admin_attack(text="[key_name_admin(user)] fed [key_name_admin(M)] with [name] Reagents: [contained] (INTENT: [uppertext(user.a_intent)]) (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[user.x];Y=[user.y];Z=[user.z]'>JMP</a>)",ckey=key_name(user),ckey_target=key_name(M))
	...
	afterattack(obj/target, mob/user, proximity)
		...
		if(target.is_open_container() && target.reagents)
			...
			msg_admin_attack(text="[user.name] ([user.ckey]) spiked \a [target] with a pill. Reagents: [reagentlist()] (INTENT: [uppertext(user.a_intent)]) (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[user.x];Y=[user.y];Z=[user.z]'>JMP</a>)",ckey=key_name(user),ckey_target=key_name(target))
