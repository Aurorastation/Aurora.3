	...
	afterattack(var/obj/target, var/mob/user, var/flag)
		...
		if(reagents.total_volume)
			...
			if(ismob(target))
				...
				msg_admin_attack(text="[user.name] ([user.ckey]) squirted [M.name] ([M.key]) with [name]. Reagents: [contained] (INTENT: [uppertext(user.a_intent)]) (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[user.x];Y=[user.y];Z=[user.z]'>JMP</a>)",ckey=key_name(user),ckey_target=key_name(M))
				return
