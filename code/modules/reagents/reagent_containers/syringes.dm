...
/obj/item/reagent_containers/syringe/afterattack(obj/target, mob/user, proximity)
	...
	switch(mode)
		if(SYRINGE_DRAW)
			...
			if(!reagents.get_free_space())
				to_chat(user, span("warning", "The pen is full."))
				mode = SYRINGE_INJECT
				return
			...
			if(ismob(target))
				...
				var/trans = target.reagents.trans_to_obj(src, amount_per_transfer_from_this)
				to_chat(user, span("notice", "You fill the pen with [trans] units of the solution."))
		...
		if(SYRINGE_INJECT)
			if(!reagents.total_volume)
				to_chat(user, span("notice", "The pen is empty."))
				mode = SYRINGE_DRAW
				return
			if(istype(target, /obj/item/implantcase/chem))
				return
