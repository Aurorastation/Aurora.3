	...
	proc/syringestab(mob/living/carbon/target as mob, mob/living/carbon/user as mob)
		...
		if(istype(target, /mob/living/carbon/human))
			...
			if (target != user && H.getarmor(target_zone, "melee") > 5 && prob(50))
				...
				msg_admin_attack(text="[key_name_admin(user)] attacked [key_name_admin(target)] with [src.name] (INTENT: HARM) (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[user.x];Y=[user.y];Z=[user.z]'>JMP</a>)",ckey=key_name(user),ckey_target=key_name(src))
