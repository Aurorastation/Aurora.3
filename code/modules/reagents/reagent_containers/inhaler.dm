...
/obj/item/reagent_containers/inhaler/afterattack(var/mob/living/carbon/human/H, var/mob/user, var/proximity)
	...
	if ( ((user.is_clumsy()) || (DUMB in user.mutations)) && prob(10))
		...
		if(H.reagents)
			...
			user.visible_message(SPAN_NOTICE("[user] accidentally sticks the [src] in [H]'s eyes!"),SPAN_NOTICE("You accidentally stick the [src] in [H]'s eyes!"))
	...
	if(user == H)
		...
		user.visible_message(SPAN_NOTICE("\The [user] injects \the [H] with \the [src]."),SPAN_NOTICE("You stick \the [src] in \the [H]'s mouth and press the injection button."))
