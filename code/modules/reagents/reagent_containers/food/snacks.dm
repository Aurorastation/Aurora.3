	...
	if (isanimal(target))
		...
		if (amount_eaten)
			...
			if (amount_eaten >= m_bitesize)
				user.visible_message("<b>[user]</b> feeds \the [target] \the [src].")
				if (!istype(target.loc, /turf))//held mobs don't see visible messages
					to_chat(target, "<b>[user]</b> feeds you \the [src].")
			else
				user.visible_message("<b>[user]</b> feeds \the [target] a tiny bit of \the [src]. <b>It looks full.</b>")
				if (!istype(target.loc, /turf))
					to_chat(target, "<b>[user]</b> feeds you a tiny bit of \the [src]. <b>You feel pretty full!</b>")
			return 1
	else
		var/fullness = 0
...
/obj/item/reagent_containers/food/snacks/attackby(obj/item/W, mob/living/user)
	...
	if(istype(W,/obj/item/pen))
		...
		if(selection == "Name")
			...
			if(input_clean_name)
				user.visible_message("<b>[user]</b> labels \the [name] as \"[input_clean_name]\".")
			...
			if(input_clean_desc)
				user.visible_message("<b>[user]</b> adds a note to \the [name].")
	...
	if(is_sliceable())
		...
		if(has_edge(W))
			...
			var/slices_lost = 0
			if(W.w_class > 3)
				user.visible_message("<b>[user]</b> crudely slices \the [src] with [W]!", SPAN_NOTICE("You crudely slice \the [src] with your [W]!"))
				slices_lost = rand(1,min(1,round(slices_num/2)))
			else
				user.visible_message("<b>[user]</b> slices \the [src]!", SPAN_NOTICE("You slice \the [src]!"))
...
/obj/item/reagent_containers/food/snacks/spreads/butter
	...
	bitesize = 2
	center_of_mass = list("x"=16, "y"=16)
	reagents_to_add = list(/datum/reagent/nutriment = 0, /datum/reagent/nutriment/triglyceride = 20, /datum/reagent/sodiumchloride = 1)
...
/obj/item/reagent_containers/food/snacks/truffle
	...
	icon_state = "truffle"
	reagents_to_add = list(/datum/reagent/nutriment = 0, /datum/reagent/nutriment/coco = 6)
	bitesize = 4
