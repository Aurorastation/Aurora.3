...
/obj/item/weapon/reagent_containers/food/snacks/attack(mob/M as mob, mob/user as mob, def_zone)
	...
	if(istype(M, /mob/living/carbon))
		...
		if(M == user)								//If you're eating it yourself
			...
			msg_admin_attack(text="[key_name(user)] fed [key_name(M)] with [src.name] Reagents: [reagentlist(src)] (INTENT: [uppertext(user.a_intent)])",ckey=key_name(user),ckey_target=key_name(M))
