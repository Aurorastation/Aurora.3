...
/obj/machinery/power/supermatter/Initialize()
	...
	filters += filter(type="rays", size=0, flags=FILTER_OVERLAY, factor=1)
...
/obj/machinery/power/supermatter/machinery_process()
	...
	animate(filters[filters.len], size=max(0, power+1), density=max(0, power/20), offset=++filter_offset, time=1 SECONDS, easing=ELASTIC_EASING|EASE_IN|EASE_OUT)
	return 1
