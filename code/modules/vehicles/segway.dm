/obj/vehicle/segway
	name = "segway"
	desc = "Nanotrasen's soultion to the crew weight crisis!"
...
/obj/vehicle/bike/update_icon()
	...
	add_overlay(image('icons/obj/segway.dmi', "[icon_state]_overlay", MOB_LAYER + 1))
...
/obj/vehicle/segway/attack_hand(var/mob/user as mob)
	if(user == load)
		...
		if(do_after(user, 8 SECONDS, act_target = src))
			unload(load)
			to_chat(user, "<span class='notice'>You unbuckle [load] from the src [src]!</span>")
			to_chat(load, "<span class='notice'>You were unbuckled from the [src]!</span>")
...
/obj/vehicle/segway/civilian
	name = "segway"
	desc = "A much higher end segway, designed for the upper class markets."
	icon = 'icons/obj/segway.dmi'
	icon_state = "civway"
	segway_icon = "civway"
	land_speed = 3
