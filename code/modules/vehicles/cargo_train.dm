...
/obj/vehicle/train/cargo/trolley/RunOver(var/mob/living/carbon/human/H)
	...
	if(HAS_TRAIT(H, TRAIT_LEANING))
		return
	H.log_message("Was run over by [src]", LOG_VICTIM, log_globally = FALSE)
...
/obj/vehicle/train/cargo/engine/RunOver(var/mob/living/carbon/human/H)
	...
	if(is_train_head() && istype(load, /mob/living/carbon/human))
		...
		D.log_message("Ran over [H.name] ([H.ckey]) with [src].", LOG_ATTACK, log_globally = FALSE)
		H.log_message("Was run over by [src], driven by [D.name] ([D.ckey])", LOG_VICTIM, log_globally = FALSE)
