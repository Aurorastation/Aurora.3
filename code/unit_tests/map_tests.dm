...
/datum/unit_test/map_test/areas_in_station_zlevels_must_be_marked_as_station_areas
	...
	var/list/do_not_check_areas_types = list(
		...
		/area/shuttle,
		/area/template_noop,
		/area/supply/dock, // this is exempt because runtime has this area in its z-level for the sake of not having an additional non-station z-level, since we care about quick boot time
	)
