...
/datum/unit_test/icon_test/all_initial_icon_states_shall_exist/start_test()
	...
	for(var/atom_type in typecacheof(/atom))
		var/failed = FALSE
		...
		if(atom_icon)
			...
			if(atom_icon_state in excepted_icon_states)
				continue
			else if(!atom_icon_state)
				failed = TRUE
				log_unit_test("[ascii_red]--------------- [atom_type] - Did not have an icon state set.")
			...
			failed = TRUE
			log_unit_test("[ascii_red]--------------- [atom_type] - Did not have an icon set.")
