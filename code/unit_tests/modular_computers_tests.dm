...
/datum/unit_test/modular_computers/communicator_end_to_end/start_test()
	...
	var/obj/item/modular_computer/handheld/communicator/landline/landline = new(test_turf)
	...
	var/datum/computer_file/program/communicator/landline_app = landline.get_communicator_program()
	...
	TEST_ASSERT_EQUAL(landline.communicator_tier, COMMUNICATOR_TIER_VIDEO, "The department landline has the wrong feature tier.")
	TEST_ASSERT_NOTNULL(landline.registered_id, "The department landline did not register its directory identity.")
	TEST_ASSERT(!landline.battery_module && istype(landline.tesla_link, /obj/item/computer_hardware/tesla_link), "The department landline is not APC-powered.")
