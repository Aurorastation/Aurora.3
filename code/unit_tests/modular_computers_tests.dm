...
/datum/unit_test/modular_computers/communicator_end_to_end/start_test()
	...
	TEST_ASSERT_NULL(video_app.get_video_target(), "A basic-tier call participant was exposed as a video target.")
	...
	TEST_ASSERT_EQUAL(length(available_video_targets), 1, "The video camera picker exposed the wrong number of compatible call participants.")
	TEST_ASSERT(!(landline_app in available_video_targets), "The video camera picker exposed a basic-tier call participant.")
	...
	TEST_ASSERT(holographic_app.toggle_hologram(), "The holographic projection request could not be sent.")
	TEST_ASSERT(holographic_peer_app.respond_feature_request(holographic_app, HOLOGRAM_REQUESTS, TRUE), "The holographic projection request could not be approved.")
