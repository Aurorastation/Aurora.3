	...
	for(var/turf/simulated/T in A)
		...
		if(expectation == UT_NORMAL || expectation == UT_NORMAL_COLD || expectation == UT_NORMAL_COOL)
			...
			if(expectation == UT_NORMAL)
				if(abs(temp - T20C) > 10)
					test_result["msg"] = "Temperature out of bounds: [temp] at [pressure] | [t_msg] | expectation: NORMAL"
					return test_result

			if(expectation == UT_NORMAL_COLD)
				if(temp > 120)
					test_result["msg"] = "Temperature out of bounds: [temp] at [pressure] | [t_msg] | expectation: NORMAL_COLD"
					return test_result

			if(expectation == UT_NORMAL_COOL)
				if(temp > 283)
					test_result["msg"] = "Temperature out of bounds: [temp] at [pressure] | [t_msg] | expectation: NORMAL_COOL"
					return test_result
