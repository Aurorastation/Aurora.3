...
/datum/wires/mulebot/UpdatePulsed(var/index)
	switch(index)
		if(WIRE_POWER1, WIRE_POWER2)
			holder.visible_message("<span class='notice'>[hicon(holder)] The charge light flickers.</span>")
		if(WIRE_AVOIDANCE)
			holder.visible_message("<span class='notice'>[hicon(holder)] The external warning lights flash briefly.</span>")
		if(WIRE_LOADCHECK)
			holder.visible_message("<span class='notice'>[hicon(holder)] The load platform clunks.</span>")
		if(WIRE_MOTOR1, WIRE_MOTOR2)
			holder.visible_message("<span class='notice'>[hicon(holder)] The drive motor whines briefly.</span>")
		else
			holder.visible_message("<span class='notice'>[hicon(holder)] You hear a radio crackle.</span>")
