...
/datum/wires/mulebot/UpdatePulsed(var/index)
	switch(index)
		if(WIRE_POWER1, WIRE_POWER2)
			holder.visible_message("<span class='notice'>[icon2html(holder, usr)] The charge light flickers.</span>")
		if(WIRE_AVOIDANCE)
			holder.visible_message("<span class='notice'>[icon2html(holder, usr)] The external warning lights flash briefly.</span>")
		if(WIRE_LOADCHECK)
			holder.visible_message("<span class='notice'>[icon2html(holder, usr)] The load platform clunks.</span>")
		if(WIRE_MOTOR1, WIRE_MOTOR2)
			holder.visible_message("<span class='notice'>[icon2html(holder, usr)] The drive motor whines briefly.</span>")
		else
			holder.visible_message("<span class='notice'>[icon2html(holder, usr)] You hear a radio crackle.</span>")
