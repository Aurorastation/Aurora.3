...
/datum/wires/tag_scanner/UpdatePulsed(var/index)
	...
	switch(index)
		if(TAG_WIRE_POWER)
			S.visible_message(SPAN_WARNING("[icon2html(S, usr)] <b>[capitalize_first_letters(S.name)]</b> beeps, \"BOOWEEEP!\""))

		if(TAG_WIRE_HACK)
			S.visible_message(SPAN_WARNING("[icon2html(S, usr)] <b>[capitalize_first_letters(S.name)]</b> beeps, \"BEEYUUP!\""))
