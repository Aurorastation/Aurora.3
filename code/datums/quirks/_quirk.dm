...
/datum/quirk/proc/is_species_appropriate(datum/species/mob_species)
	if(mob_trait in GLOB.all_species[mob_species].inherent_traits)
		return FALSE
	return TRUE

/// If a quirk is able to be selected for the mob's background
/datum/quirk/proc/is_background_appropriate(singleton/origin_item/origin_background)
	if(mob_trait in origin_background.origin_traits)
		return FALSE
	if(mob_trait in origin_background.blacklisted_traits)
		return FALSE
	return TRUE
...
/datum/quirk/item_quirk/proc/give_item_to_holder(obj/item/quirk_item, list/valid_slots, flavour_text = null, default_location = "at your feet", notify_player = FALSE)
	...
	var/where = human_holder.equip_in_one_of_slots(quirk_item, valid_slots, del_on_fail = FALSE, ignore_blocked = TRUE) || default_location

	if(notify_player)
		LAZYADD(where_items_spawned, SPAN_NOTICE("You have \a [quirk_item] [where]. [flavour_text]"))
