...
/datum/element/light_eaten/Attach(atom/target)
	...
	if(atom_target.light_system == COMPLEX_LIGHT)
		atom_target.set_light(0, 0, l_on = FALSE)
	else
		atom_target.set_light_power(0)
		atom_target.set_light_range(0)
		atom_target.set_light_on(FALSE)
		if(ismovable(atom_target))
			var/atom/movable/movable_target = atom_target
			movable_target.update_icon()
