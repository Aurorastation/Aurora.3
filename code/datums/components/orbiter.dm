...
/datum/component/orbiter/RegisterWithParent()
	...
	// RegisterSignal(parent, COMSIG_MOVABLE_UPDATE_GLIDE_SIZE, PROC_REF(orbiter_glide_size_update)) //Aurora doesn't vary glides

/datum/component/orbiter/UnregisterFromParent()
	// UnregisterSignal(parent, COMSIG_MOVABLE_UPDATE_GLIDE_SIZE) //Aurora doesn't vary glides
	var/atom/target = parent
...
/datum/component/orbiter/proc/end_orbit(atom/movable/orbiter, refreshing=FALSE)
	...
	orbiter.SpinAnimation(0, 0, parallel = FALSE) //TG has it with parallel = TRUE but it seems to not work here with that, for some reason
	...
	if(ismob(orbiter))
		var/mob/orbiter_mob = orbiter
		// orbiter_mob.updating_glide_size = TRUE //Aurora doesn't vary glides
