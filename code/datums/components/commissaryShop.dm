...
/datum/component/quikpay_shop/proc/card_pay(obj/item/attacking_item, mob/user)
	...
	if(transaction_amount <= E.worth)
		...
		if(istype(E, /obj/item/spacecash/ewallet/persistent_credit_card))
			log_and_message_admins("Persistent credit card [E] used for a transaction at [src] with amount [transaction_amount]. Remaining balance: [E.worth]", user, get_turf(src))
