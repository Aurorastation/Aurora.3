...
/proc/get_mobs_or_objs_in_view(turf/T, range, list/mobs = list(), list/objs = list(), checkghosts = GHOSTS_ALL_HEAR)
	var/list/hear = list()
...
/proc/get_hearers_in_radio_ranges_old(list/obj/item/radio/radios)
	set background = 1
...
/atom/proc/benchmark_new_old(seconds = 5)
	var/iterations = 0
	var/duration = seconds SECONDS
	var/end_time = world.timeofday + duration

	while(BENCHMARK_LOOP)
		SSspatial_grid.orthogonal_range_search(src, SPATIAL_GRID_CONTENTS_TYPE_HEARING, 10)
		iterations++

	message_admins("SSspatial_grid.orthangonal_range_search(src, SPATIAL_GRID_CONTENTS_TYPE_HEARING, 10) got [iterations] iterations in [seconds] seconds!")

	BENCHMARK_START

	while(BENCHMARK_LOOP)
		SSspatial_grid.orthogonal_range_search(src, SPATIAL_GRID_CONTENTS_TYPE_HEARING, 3)
		iterations++

	message_admins("SSspatial_grid.orthogonal_range_search(src, SPATIAL_GRID_CONTENTS_TYPE_HEARING, 3) got [iterations] iterations in [seconds] seconds!")

	BENCHMARK_START

	while(BENCHMARK_LOOP)
		get_hearers_in_view(10, src)
		iterations++

	message_admins("get_hearers_in_view(10, src) got [iterations] iterations in [seconds] seconds!")

	BENCHMARK_START

	while(BENCHMARK_LOOP)
		get_hearers_in_view(3, src)
		iterations++

	message_admins("get_hearers_in_view(3, src) got [iterations] iterations in [seconds] seconds!")

	BENCHMARK_START

	while(BENCHMARK_LOOP)
		get_mobs_or_objs_in_view(get_turf(src), 10)
		iterations++

	message_admins("get_mobs_or_objs_in_view(src.loc, 10) got [iterations] iterations in [seconds] seconds!")

	BENCHMARK_START

	while(BENCHMARK_LOOP)
		get_mobs_or_objs_in_view(get_turf(src), 3)
		iterations++

	message_admins("get_mobs_or_objs_in_view(src.loc, 3) got [iterations] iterations in [seconds] seconds!")

	var/list/all_radios = list()
	for(var/channel in SSradio.frequencies)
		all_radios += SSradio.get_devices(text2num(channel), RADIO_CHAT)
	BENCHMARK_START

	while(BENCHMARK_LOOP)
		get_hearers_in_radio_ranges(all_radios)
		iterations++

	message_admins("get_hearers_in_radio_ranges([length(all_radios)] radios in the world) got [iterations] iterations in [seconds] seconds!")

	BENCHMARK_START

	while(BENCHMARK_LOOP)
		get_hearers_in_radio_ranges_old(all_radios)
		iterations++

	message_admins("get_hearers_in_radio_ranges_old([length(all_radios)] radios in the world) got [iterations] iterations in [seconds] seconds!")
