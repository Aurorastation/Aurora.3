...
echo "Checking for density = 0/1 uses:" >> code_error.log
grep -r '\bdensity\s*=\s*\d' **/*.dm >> code_error.log
if [ $? -eq 0 ]; then
    ERROR_COUNT=$(($ERROR_COUNT+1))
    echo "FAIL: Found density = 0/1 in code" >> code_error.log
else
    echo "PASS: Found no density = 0/1 in code:" >> code_error.log
fi
