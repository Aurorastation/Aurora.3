...
MAP_ERROR_COUNT=0
find maps -name \*.dmm -type f | while read p; do if [ "$(sed -n 1p "$p")" != "$TGM_CHECK" ]; then MAP_ERROR_COUNT=$(($MAP_ERROR_COUNT+1)) && echo "FAIL: Found non-TGM mapfile in $p, $TGM_CHECK" >> code_error.log; fi;done
