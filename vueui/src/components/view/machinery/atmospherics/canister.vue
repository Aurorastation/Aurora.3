  ...
  <div>
    ...
    <vui-group>
      <vui-group-item label="Tank Label:">
        {{ name }}
        <vui-button class="float-right" icon="pencil-alt" :params="{ relabel: 1 }" :disabled="!canLabel"
          >Relabel</vui-button
        >
      </vui-group-item>
      <vui-group-item label="Tank Pressure:"> {{ tankPressure }} kPa </vui-group-item>
      ...
      <template v-if="hasHoldingTank">
        <vui-group-item label="Tank Label:">
          {{ holdingTank.name
          }}<vui-button class="float-right" icon="eject" :params="{ remove_tank: 1 }">Eject</vui-button>
        </vui-group-item>

        <vui-group-item label="Tank Pressure:">{{ holdingTank.tankPressure }} kPa</vui-group-item>
      </template>
      <vui-group-item v-else
        ><span class="average"><i>No holding tank inserted.</i></span></vui-group-item
      >
