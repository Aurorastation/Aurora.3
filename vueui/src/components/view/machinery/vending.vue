  ...
  <div>
    <template v-if="s.mode == 0 || s.sel_price == 0">
      <div class="cancel-button">
        ...
        <vui-button :disabled="!s.coin" :params="{ remove_coin: 1 }" icon="sign-out-alt">{{
          s.coin ? s.coin : 'No coin inserted.'
        }}</vui-button>
      </div>
      <div class="t-parent">
        <vui-button
          v-for="vend in output"
          :key="vend.key"
          :class="vend.amount > 0 ? '' : 'no-stock'"
          class="t-child tooltip"
          :disabled="vend.amount == 0 || s.mode == 1"
          :params="{ vendItem: vend.key }"
        >
