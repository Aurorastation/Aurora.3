<template>
  <div>
    <vui-group>
      <vui-group-item label="Designation:"> {{ s.doorArea }} - {{ s.doorName }} </vui-group-item>
      <vui-group-item label="Main power:">
        <vui-progress
          :class="{
            good: s.plu_main == 0,
            bad: s.plu_main == -1,
            average: s.plu_main > 0,
          }"
          style="width: 12em;"
          :value="gs.wtime"
          :min="s.plua_main"
      ...
      <vui-group-item label="Backup power:">
        <vui-progress
          :class="{
            good: s.plu_back == 0 || s.plu_main == 0,
            bad: s.plu_back == -1 && s.plu_main != 0,
            average: s.plu_back > 0,
          }"
          style="width: 12em;"
          :value="gs.wtime"
          :min="s.plua_back"
      ...
      <vui-group-item :set="(k = 'bolts')" :key="k" label="Bolts:">
        <vui-button
          style="min-width: 6em"
          :disabled="!!(!s.aiCanBolt && s.isai && !s.isAdmin)"
          :class="{ on: s[k] }"
          :params="{ command: k, activate: 0 }"
          >Raised</vui-button
        >
        <vui-button
          style="min-width: 6em"
          :disabled="!!(!s.aiCanBolt && s.isai && !s.isAdmin)"
          :class="{ on: !s[k] }"
          :params="{ command: k, activate: 1 }"
          >Dropped</vui-button
        >
        <vui-button
          style="min-width: 6em"
          v-if="s.boltsOverride && s[k]"
          class="danger"
          :params="{ command: 'bolts_override', activate: 1 }"
          >Drop Now</vui-button
        >
        <vui-button
          style="min-width: 6em"
          v-if="s.boltsOverride && !s[k]"
          class="danger"
          :params="{ command: 'bolts_override', activate: 0 }"
          >Raise Now</vui-button
        >
      </vui-group-item>
      <vui-group-item v-for="(c, k) in commands" :key="k" :label="c.name + ':'">
        <vui-button style="min-width: 6em" :class="{ on: s[k] }" :params="{ command: k, activate: c.i ? 1 : 0 }">{{
          c.et || 'Enabled'
        }}</vui-button>
        <vui-button
          style="min-width: 6em"
          :disabled="!!(c.a && s.isai && !s.isAdmin)"
          :class="{ on: !s[k] && !c.danger, danger: !s[k] && c.danger }"
          :params="{ command: k, activate: c.i ? 0 : 1 }"
          >{{ c.dt || 'Disabled' }}</vui-button
        >
      </vui-group-item>
    </vui-group>
  </div>
