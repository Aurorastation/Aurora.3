<template>
  <div>
    <div class="tagselector" v-show="!spawnpoint">
      <vui-button
        v-for="(amount, tag) in tags"
        :key="tag"
        v-on:click="current_tag = tag"
        v-bind:class="{ selected: current_tag == tag }"
        >{{ tag }} ({{ amount }})</vui-button
      >
    </div>
    ...
    <table>
      ...
      <tr v-for="(data, index) in spawners" :key="index" v-show="showEntry(data)">
        ...
        <td v-else>&infin;</td>
        <td class="action">
          <vui-button :disabled="data.cant_spawn !== 0" :params="{ spawn: index, spawnpoint: spawnpoint }" icon="star"
            >Spawn</vui-button
          >
          <vui-button v-if="data.can_edit == 1" :disabled="data.enabled == 1" :params="{ enable: index }"
            >Enable</vui-button
          >
          <vui-button v-if="data.can_edit == 1" :disabled="data.enabled == 0" :params="{ disable: index }"
            >Disable</vui-button
          >
        </td>
      </tr>
  ...
  methods: {
    showEntry: function(data) {
      if (!this.spawnpoint) {
        ...
        if (!Object.prototype.hasOwnProperty.apply(data, ['spawnpoints'])) return false
