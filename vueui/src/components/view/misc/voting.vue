    ...
    <template v-if="mode">
      ...
      <table>
        ...
        <tr v-for="(options, choice) in uichoices" :key="choice">
          <td style="text-align:left;"><vui-button :class="{on: choice == voted}" :params="{action: 'vote', vote: choice}">{{options.name}}</vui-button></td><td v-if="options.votes_visible">{{options.votes}}</td><td v-if="options.extra">{{options.extra}}</td>
        </tr>
      </table>
      <hr>
