  ...
  <div>
    <template v-if="mode">
      ...
      <table>
        <tr>
          <th>Choices</th>
          <th>Votes</th>
          <th v-if="mode == 'gamemode'">Minimum Players</th>
        </tr>
        <tr v-for="(options, choice) in uichoices" :key="choice">
          <td style="text-align:left;">
            <vui-button :class="{ on: choice == voted }" :params="{ action: 'vote', vote: choice }">{{
              options.name
            }}</vui-button>
          </td>
          <td>{{ options.votes }}</td>
          <td v-if="options.extra">{{ options.extra }}</td>
        </tr>
      </table>
    ...
    <template v-else>
      ...
      <ul>
        ...
        <li>
          <vui-button :disabled="!isstaff && !allow_vote_restart" :params="{ action: 'crew_transfer' }"
            >Crew Transfer</vui-button
          >
          <span v-if="(isstaff || allow_vote_restart) && is_code_red">(Disallowed, Code Red or above)</span>
          <vui-button :class="{ on: allow_vote_restart }" v-if="isstaff" :params="{ action: 'toggle_restart' }"
        ...
        <li>
          ...
          <vui-button :class="{ on: allow_vote_mode }" v-if="isstaff" :params="{ action: 'toggle_gamemode' }"
            >Toggle GameMode voting</vui-button
          >
        </li>
        <li>
          <vui-button :disabled="!allow_extra_antags" :params="{ action: 'add_antagonist' }"
