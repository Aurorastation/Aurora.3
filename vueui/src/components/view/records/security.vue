<template>
  <view-records-general v-if="active" hide-advanced>
    ...
    <vui-group-item label="Crimes:"
      ><view-records-field :editable="(editable & 4) > 0" path="active.security.crimes"
        ><textarea v-model="$root.$data.state.editingvalue"/></view-records-field
    ></vui-group-item>

    <vui-group-item label="Comments:">
      <div v-for="comment in active.security.comments" :key="comment">
        {{ comment }}
        <vui-button
          v-if="(editable & 4) > 0"
          :params="{
      ...
      <div v-if="active.security.comments.length == 0">
    ...
    <vui-group-item label="Notes:"
      ><view-records-field :editable="(editable & 4) > 0" path="active.security.notes"
        ><textarea v-model="$root.$data.state.editingvalue"/></view-records-field
    ></vui-group-item>
    <vui-group-item label="Incidents:">
      <div class="incidents" v-for="(incident, i) in active.security.incidents" :key="i">
        <view-records-incident :incident="incident" />
        <hr v-if="i + 1 < active.security.incidents.length" />
      </div>
      <div v-if="active.security.incidents.length == 0">
