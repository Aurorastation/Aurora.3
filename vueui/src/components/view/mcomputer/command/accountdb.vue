<template>
  <div>
    <vui-group>
      ...
      <vui-group-item v-if="s.access_level" label="Station funds:"
        >{{ s.accounts[`${s.station_account_number}`].money.toFixed(2) }}电</vui-group-item
      >
      ...
      <template v-if="new_name != null">
        ...
        <vui-group-item label="Initial Deposit:"
          ><vui-input-numeric v-model="new_funds" :button-count="0" width="7em" :max="1000000"
        /></vui-group-item>
      ...
      <template v-else-if="s.access_level">
        <template v-if="active">
          ...
          <vui-group-row>
            ...
            <vui-button
              v-if="active_acc.no != s.station_account_number"
              :params="{ revoke_payroll: { account: active } }"
              >Revoke Payroll</vui-button
            >
          ...
          <vui-group-row>
            <table class="table border" style="width: 100%">
              ...
              <tr v-for="(acc, number) in s.accounts" class="item border" :key="number">
                <td>
                  <vui-button @click="active = number">#{{ acc.no }}</vui-button>
                </td>
                <td>
                  <span :class="acc.sus ? 'bad' : ''">{{ acc.owner }}</span>
                </td>
              </tr>
            </table>
          </vui-group-row>
