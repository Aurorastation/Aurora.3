<template>
  <div>
    <i>Welcome to NT IT client manager</i>
    <hr />
    <div>
      This is the Client Management Application of the Nanotrasen IT Department. It is used ensure that the clients
      comply with the corporate IT policy, enroll new clients and manage clients remotely
    </div>
    <br />
    <h2>Status</h2>
    ...
    <template v-if="enrollment_status == 0">
      ...
      <vui-item v-if="device_type == 1" label="Device Preset:">
        <vui-button
          ...
          :key="id"
          :class="{ selected: device_preset == id }"
          @click="device_preset = id"
          >{{ preset.name }}</vui-button
        >
      </vui-item>
      <vui-item label="Enroll Device:">
        <vui-button
          ...
          >Confirm</vui-button
        >
        <div v-if="ntnet_status == 0" style="color: red;">
          NTNET unavailable. Unable to enroll device.
        </div>
      </vui-item>
    </template>
  </div>
