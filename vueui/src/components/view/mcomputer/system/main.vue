<template>
  <div>
    ...
    <div v-for="p in displayed_programs" :key="p.filename">
      ...
      <vui-button class="danger" icon="window-close" iconOnly :params="{ PC_killprogram: p.filename}" v-if="p.running"/>
    </div>
    ...
    <div v-for="p in services" :key="p.filename">
      ...
      <vui-button class="danger" icon="window-close" iconOnly :params="{ PC_toggleservice: p.filename}" v-if="p.service.enabled"/>
    </div>
  </div>
</template>
