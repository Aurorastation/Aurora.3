  ...
  <div>
    ...
    <div v-if="is_towing">
      ...
      <div v-if="has_proper_trolley">
        ...
        <div>
          Vacuum Capacity Remaining: <vui-progress :class="{ good: vacuum_capacity >= max_vacuum_capacity * 0.8, bad: vacuum_capacity <= max_vacuum_capacity * 0.4, average: vacuum_capacity < max_vacuum_capacity * 0.8 && vacuum_capacity > max_vacuum_capacity * 0.4 }" :value="vacuum_capacity" :max="max_vacuum_capacity" :min="0">{{ vacuum_capacity }}</vui-progress> <vui-button :disabled="vacuum_capacity >= max_vacuum_capacity" :params="{ empty_hoover: 1}">Empty</vui-button>
        </div>
        <div v-if="is_mopping">
          The trolley is currently mopping. <vui-button :disabled="!is_on || !has_bucket || bucket_capacity <= 0" :params="{ toggle_mops: 1}">Toggle</vui-button>
        </div>
        <div v-else>
          The trolley is not mopping. <vui-button :disabled="!is_on || !has_bucket" :params="{ toggle_mops: 1}">Toggle</vui-button>
        </div>
        <div v-if="has_bucket">
          ...
          <div>
            Container Reagents Remaining: <vui-progress :class="{ good: bucket_capacity >= max_bucket_capacity * 0.8, bad: bucket_capacity <= max_bucket_capacity * 0.4, average: bucket_capacity < max_bucket_capacity * 0.8 && bucket_capacity > max_bucket_capacity * 0.4 }" :value="bucket_capacity" :max="max_bucket_capacity" :min="0">{{ bucket_capacity }}u</vui-progress>
          </div>
        </div>
        <div v-else>
