<template>
  <div>
    <h3>Tank Control System</h3>
    <div v-if="s['input']">
      <vui-item :balance="0.65" label="Input:">
        <span v-if="s['input'].power">Injecting</span>
        <span v-else>On Hold</span>&nbsp;
        <vui-button :params="{ in_toggle_injector: 1 }" icon="power-off">Toggle Power</vui-button>
      </vui-item>
      <vui-item :balance="0.65" label="Flow Rate Limit:">{{ s['input'].rate }} L/s</vui-item>
      <vui-item :balance="0.65" label="Command:">
        <vui-input-numeric
          @keypress.enter="s({ in_set_flowrate: setRate })"
          width="3em"
          :button-count="3"
          v-model="setRate"
          :max="s.maxrate"
        />
        <br >
        <vui-button push-s :params="{ in_set_flowrate: setRate }">Set Flow Rate</vui-button>
      </vui-item>
    </div>
    <vui-button v-else :params="{ in_refresh_status: 1 }">Search for input port</vui-button>
    <div style="margin-top: 2em;" v-if="s['output']">
      <vui-item :balance="0.65" label="Output:">
        <span v-if="s['output'].power">Open</span>
        <span v-else>On Hold</span>&nbsp;
        <vui-button :params="{ out_toggle_power: 1 }" icon="power-off">Toggle Power</vui-button>
      </vui-item>
      <vui-item :balance="0.65" label="Max Output Pressure:">{{ s['output'].pressure }} kPa</vui-item>
      <vui-item :balance="0.65" label="Command:">
        <vui-input-numeric
          @keypress.enter="s({ out_set_pressure: setPressure })"
          width="5em"
          :button-count="4"
          :decimal-places="2"
          v-model="setPressure"
          :max="s.maxpressure"
        />
        <br >
        <vui-button :params="{ out_set_pressure: setPressure }">Set Pressure</vui-button>
      </vui-item>
    </div>
    <vui-button v-else :params="{ out_refresh_status: 1 }">Search for output port</vui-button>
  </div>
</template>

<script>
import Utils from '../../../../utils';
export default {
  data() {
    return {
      s: this.$root.$data.state,
      setRate: this.$root.$data.input?.setrate ?? 200,
      setPressure: this.$root.$data.output?.setpressure ?? 5066.25
    }
  }
};
</script>