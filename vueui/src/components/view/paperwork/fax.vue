  ...
  <div>
    <div class="item">
      ...
      <div class="itemContent">
        <vui-button :params="{ scan: 1 }" icon="eject"
          ><span v-if="state.idname">{{ state.idname }}</span
          ><span v-else>--------</span></vui-button
        >
      </div>
    </div>
    <div v-if="state.auth" style="margin-top: 24px; clear: both;">
      ...
      <span v-else
        ><b
          >Transmitter arrays realigning. Please stand by. {{ remaining_cooldown | roundRemaining }} seconds
          remaining.</b
        ></span
      >
    </div>
    ...
    <div v-for="pda in state.alertpdas" :key="pda.ref">
      {{ pda.name }}
      <vui-button :params="{ unlink: pda.ref }">Unlink</vui-button>
    </div>
