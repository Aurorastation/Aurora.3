<template>
  <div>
    <span v-if="state.toner">Current toner level: {{ state.toner }}</span
    ><br />
    ...
    <template v-if="state.toner">
      ...
      <template v-else> <h3>Please insert something to copy.</h3> </template><br />
      <vui-button v-if="state.isAI" :params="{ aipic: 1 }">Print photo from database</vui-button>
    </template>
    <template v-else>
      <h2>Please insert a new toner cartridge!</h2>
    </template>
  </div>
</template>
