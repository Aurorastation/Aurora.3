<template>
  <div>
    ...
    <vui-input-search
      style="float: right;"
      :input="players_filtered"
      v-model="search_results"
      :keys="['name', 'real_name', 'assigment', 'key', 'ip']"
      autofocus
      :threshold="threshold"
      include-score
    />
    <div class="table">
      ...
      <div
        class="player"
        v-for="p in search_results"
        :key="p.item.ref"
        :style="{ opacity: 1 - p.score * score_multiplier }"
      >
        <div class="item">
        ...
        <div class="item">
          <template v-if="!s.ismod"> {{ p.item.key }}<span v-if="!p.item.connected"> (DC)</span> </template>
          <vui-tooltip v-else>
        ...
        <div class="item" v-if="s.ismod">
          ...
          <span class="red" v-if="p.item.antag == 1"
            ><vui-tooltip label="ADD">This antag was added by admin.</vui-tooltip></span
          >
          <span class="red" v-if="p.item.antag == 2"
            ><vui-tooltip label="GM">This antag was added by gamemode.</vui-tooltip></span
          >
        </div>
        <div class="item" style="text-align: right;">
          ...
          <vui-button
            :unsafe-params="{
              src: s.holder_ref,
              adminplayerobservejump: p.item.ref,
            }"
            >JMP</vui-button
          >
          <vui-button
