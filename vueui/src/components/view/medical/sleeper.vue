<template>
  <div>
          ...
          <div class="column">
            <div>
              ...
              <div>
                ...
                <span :style="{ color: consciousnessLabel(stat) }">{{ consciousnessText(stat) }}</span>
              </div>
              <div><span class="text">Stasis Level:</span> {{ stasis }}</div>
              <div><span class="text">Species:</span> {{ species }}</div>
              <div>
                <span class="text">Brain Activity:</span>
                <vui-progress :class="progressClass(brain_activity)" :value="brain_activity"
                  >{{ brain_activity }}%</vui-progress
                >
              </div>
              <div><span class="text">Pulse:</span> {{ pulse }}</div>
              <div>
                <span class="text">BP:</span>
                <span :style="{ color: getPressureClass(blood_pressure_level) }">{{ blood_pressure }}</span>
              </div>
              <div>
                <span class="text">Blood Oxygenation:</span>
                <vui-progress :class="progressClass(brain_activity)" :value="Math.round(blood_o2)"
                  >{{ Math.round(blood_o2) }}%</vui-progress
                >
              </div>
              ...
              <div v-if="bloodreagents">
                <table>
                  <tr v-for="reagent in bloodreagents" :key="reagent.name">
                    <td>
                      <span class="text">{{ reagent.name }}:</span>
                    </td>
          ...
          <div class="column">
            ...
            <table>
              <tr v-for="reagent in reagents" :key="reagent.name">
                ...
                <td>
                  <vui-button :params="{ chemical: reagent.type, amount: 5 }">Inject 5</vui-button>
                </td>
                <td>
                  <vui-button :params="{ chemical: reagent.type, amount: 10 }">Inject 10</vui-button>
                </td>
              </tr>
            </table>
    ...
    <div v-if="beaker != -1">
      <span class="text">Beaker:</span> {{ beaker }} units of free space remaining.
      <vui-button :params="{ beaker: 1 }">Eject Beaker</vui-button>
    </div>
    <div v-else>
      No beaker inserted.
    </div>
