...
/proc/PerlinPermutate()
	...
	for(var/x = 1; x <= 512; x++)
		perlin_p[x] = perlin_permutation[min(max(1,(x % 256) + 1), 256)]
...
/proc/_perlin(var/x, var/y, var/seed)
	...
	var/A =  min(perlin_p[max(1, x0)] + y0, 512)
	var/AA = min(perlin_p[max(1, A)] + z0, 512)
	var/AB = min(perlin_p[max(1, A + 1)] + z0, 512)
	var/B =  min(perlin_p[max(1, x0 + 1)] + y0, 512)
	var/BA = min(perlin_p[max(1, B)] + z0, 512)
	var/BB = min(perlin_p[max(1, B + 1)] + z0, 512)
