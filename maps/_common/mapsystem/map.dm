...
/datum/map/proc/build_away_sites()
	...
	for (var/site_name in SSmapping.away_sites_templates)
		...
		if (site.template_flags & TEMPLATE_FLAG_SPAWN_GUARANTEED)
			guaranteed += site
			if ((site.template_flags & TEMPLATE_FLAG_ALLOW_DUPLICATES) && !(site.template_flags & TEMPLATE_FLAG_RUIN_STARTS_DISALLOWED) && (!(SSatlas.current_sector.name in site.sectors)))
